...
**Hard gate**: JSON sonic-rs parity, CSS lightningcss parity, workspace nextest health, fleet-wide IR/StructRegistry audits, bootstrap reproducibility, and the 17-entry post-AZ-II close matrix are refreshed on the tape-free StructDirect path.
**Status**: superseded by AZ-III.W2 - Semantic Parity and Bootstrap Canonicalization and AZ-III.W4 - Benchmark, Profile, and Workspace Truth

## Continuation Close

O6 did not run in AZ-II. Semantic parity moves to AZ-III.W2 - Semantic Parity and Bootstrap Canonicalization; workspace,
profile, and 17-entry benchmark truth move to AZ-III.W4 - Benchmark, Profile, and Workspace Truth. No AZ-II
performance or parity claim after O4 is terminal evidence.
...
7. Re-run fleet-wide structural audits after O5: payload coverage,
   StructRegistry closure, regen reproducibility, and workspace nextest
   health.
8. Update AZ-II progress docs with measured throughput and parity status.
...
10. Treat O3a-J1 as the JSON parity/throughput precondition: O6 may
    normalize numeric oracle witnesses and measure `data_xl`, but it
    may not mask materialization defects owned by O3/O4.
11. Treat O3a-S1 as the Sheets semantic precondition: O6 verifies
    Sheets health after S1-E1/S1-R1/S1-SER1 redress, and any remaining
    failure blocks O7 with an exact source owner.
12. Treat O3a-A1 as a live-product and bootstrap precondition:
    analysis/LSP directive failures must be green, and
    `bootstrap_parser.rs` must have a generated-self-host proof or an
    O7 blocker before terminal close.
...
| `crates/core/benches/{json/monolithic,css/l4,google_sheets/monolithic,bbnf/monolithic}.rs` | modify |
| `crates/core/benches/compile_pipeline.rs` | modify |
...
| `docs/benchmarks/archive/AZ-II/cutover/O6-structural-audit.txt` | create |
| `docs/benchmarks/archive/AZ-II/cutover/O6-workspace-nextest.txt` | create |
| `docs/benchmarks/archive/AZ-II/cutover/O6-bootstrap-parser-proof.txt` | create |
...
Sub-gate: `cargo nextest run -p bbnf --test sonic_rs_parity --cargo-profile ax-iter -- --nocapture` passes.
...
Sub-gate: `cargo nextest run -p bbnf --test lightningcss_parity --cargo-profile ax-iter -- --nocapture` passes. A failure blocks O7; O6 may not claim CSS parity with an active gap.
...
and `make az-ii-bench-close WAVE=O6` writes
`docs/benchmarks/post-AZ-II-O6-{json,css,sheets,bbnf,compile}.txt`
before assembling `docs/benchmarks/post-AZ-II.json`.
...
Files touched: `crates/core/benches/json/monolithic.rs`, `crates/core/benches/json/value.rs`, `docs/benchmarks/post-AZ-II-O6-json.txt`.
...
Files touched: `crates/core/benches/css/l4.rs`, `docs/benchmarks/post-AZ-II-O6-css.txt`.
...
Files touched: `crates/core/benches/google_sheets/monolithic.rs`, `docs/benchmarks/post-AZ-II-O6-sheets.txt`.
...
Files touched: `crates/core/benches/bbnf/monolithic.rs`, `crates/core/benches/compile_pipeline.rs`, `docs/benchmarks/post-AZ-II-O6-bbnf.txt`, `docs/benchmarks/post-AZ-II-O6-compile.txt`.
...
### AZ-II.cutover.O6.10 Structural Audit and Workspace Health

Mechanism: re-run the structural close gates that were marked
unverified in the hardening audit: payload coverage, StructRegistry
closure, generated reproducibility, BBNF bootstrap reproducibility, and
workspace nextest.

Files touched: `docs/benchmarks/archive/AZ-II/cutover/O6-structural-audit.txt`,
`docs/benchmarks/archive/AZ-II/cutover/O6-workspace-nextest.txt`.

Sub-gate: the structural audit artifact records 100% payload coverage,
closed StructRegistry for production grammars, clean regen check, green
BBNF bootstrap reproducibility, and workspace nextest counts. Any miss
blocks O7.

### AZ-II.cutover.O6.11 Benchmark JSON and Progress

Mechanism: assemble the close matrix and update progress docs with exact commands, dates, and residual named gaps.

Files touched: `docs/benchmarks/post-AZ-II.json`, `docs/tranches/AZ-II/PROGRESS.md`, `docs/tranches/AZ-II/waves/cutover/README.md`.

Sub-gate: `post-AZ-II.json` contains 17 parse entries plus the
close-selected compile_pipeline entries, with no
`placeholder`, `NOT_MEASURED`, `SIGABRT`, or `PARSE_FAILED` values.
Docs name O7 as FINAL conversion.

### AZ-II.cutover.O6.12 O3a Cohort Close Matrix
...
Sub-gate: `/tmp/az-ii-o3a-j1-o6-h1.txt` passes;
`post-AZ-II-O6-json.txt` and `post-AZ-II.json` contain `data_xl` with
no timeout marker, placeholder, or failed-parse marker.

### AZ-II.cutover.O6.14 O3a C1 CSS Admission and Parity Close
...
Files touched: `crates/core/tests/lightningcss_parity.rs`,
`crates/core/tests/css_l4*.rs`,
`crates/core/tests/ax_w0a2s_real_css_probe.rs`,
`crates/core/benches/css/l4.rs`,
`docs/benchmarks/post-AZ-II-O6-css.txt`, and
`docs/benchmarks/post-AZ-II.json`.

Sub-gate: `cargo nextest run -p bbnf --test lightningcss_parity
--cargo-profile ax-iter --no-fail-fast -- --nocapture` passes after
the C1 focused suites pass. Any failure blocks O6 CSS parity and O7;
O6 may not publish CSS throughput or semantic parity with
`lightningcss_parity_bootstrap` or `lightningcss_parity_tailwind` red.

### AZ-II.cutover.O6.15 O3a S1 Sheets Close Gate
...
Sub-gate: the focused commands `cargo nextest run -p bbnf --test
sheets_parity --cargo-profile ax-iter --no-fail-fast -- --nocapture`,
`cargo nextest run -p bbnf --test sheets_self_parity --cargo-profile
ax-iter --no-fail-fast -- --nocapture`, and `cargo nextest run -p bbnf
--test sheets_expr_parity --cargo-profile ax-iter --no-fail-fast --
--nocapture` pass on the post-O5 tree. If any fail, O6 blocks O7 and
names the responsible S1 child owner instead of downgrading the miss to
a stale bench or parity caveat.
...
1. `cargo nextest run -p bbnf --test sonic_rs_parity --cargo-profile ax-iter -- --nocapture` passes.
2. `cargo nextest run -p bbnf --test lightningcss_parity --cargo-profile ax-iter -- --nocapture` passes.
3. `cargo nextest run --workspace --cargo-profile ax-iter --no-fail-fast` passes or blocks O7 with a named, source-owned failure ledger.
4. `docs/benchmarks/archive/AZ-II/cutover/O6-structural-audit.txt` records 100% payload coverage, closed StructRegistry for production grammars, clean `cargo xtask regen --check`, and green BBNF bootstrap reproducibility.
5. All 17 close-matrix parse entries plus close-selected compile_pipeline entries are present in `docs/benchmarks/post-AZ-II.json` with no `placeholder`, `NOT_MEASURED`, `SIGABRT`, or `PARSE_FAILED` values.
6. `docs/benchmarks/post-AZ-II-O6-nm.txt` records tape symbols absent from close bench binaries.
7. `docs/benchmarks/profiles/AZ-II/cutover/O6/**` contains the required profile artifacts for later optimization claims.
8. O3a J1/C1/S1 cohorts are closed or block O7 with named successor
...
10. O3a C1 is closed: CSS admission, payload, selector, and
    lightningcss failures are green before CSS throughput or semantic
    parity is published.
11. S1-E1, S1-R1, and S1-SER1 are either green or listed as O7
    blockers with exact failing tests; O6 contains no Sheets
    compatibility shim, no synthetic serializer recovery from source
    spans, and no `Parsed<R>` return-model resurrection.
...
- `docs/benchmarks/post-AZ-II-O6-sheets.txt`
- `docs/benchmarks/post-AZ-II-O6-bbnf.txt`
- `docs/benchmarks/post-AZ-II-O6-compile.txt`
...
- **Depends on**: AZ-II.cutover.O5, O3a J1/C1/S1/A1 cohort dispositions
