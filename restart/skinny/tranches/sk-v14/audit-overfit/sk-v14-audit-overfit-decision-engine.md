...
- **Verdict: PASS for the resolver clause; FAIL for the scaffold-only-as-admit clause** (resolver is wired end-to-end and drives emission via fail-closed lowering; W8 + W9 remain SCAFFOLD-ONLY and were cited as admit evidence in SK-V13 W14.1-5 / W13.* rows — those rows are AUDIT-FALSIFIED in the SK-V14 contract, so the SK-V14 starting baseline reads CLEAN). Wave-level scaffold persistence is HIGH severity because PRUNE-5 must consume it; no row currently held at the SK-V14 starting baseline cites W8 / W9 as load-bearing evidence.
...
- **No-scaffold-only-admit clause: FAIL at SK-V13 close, PASS at SK-V14 starting baseline.** SK-V13 W14.1-5 (5 JSON parse_only) + W13.1-4 + W15.1 (typed + direct admits) rows cited W8 / W9 facts as evidence per the W8 / W9 research JSONs, but those rows are AUDIT-FALSIFIED in the SK-V14 baseline per SYNTHESIS §0.2 + §1.2 + §1.3. Per the SK-V14 contract, **zero current admits cite scaffold-only waves**: the baseline is `JSON parse_only 0/17, JSON direct 0/17, JSON typed 0/17, CSS L4 0/24`. PRUNE-1 + PRUNE-2 revert the scaffold-cited admits; PRUNE-5 (C-4) wires W8 / W9 to load-bearing before any new admit may cite them.

The resolver clause carries SK-V14 forward unchanged; the scaffold-clause obligation is the C-4 binding in SYNTHESIS §3.
...
| **Preserve the resolver's honest self-labelling** (`decision_csp.rs:160-164` block IDs). Post-C-4 these block IDs should clear; until they do, they correctly carry the SCAFFOLD-blocker signal end-to-end. No code change recommended pre-C-4. | `v4 §2` architectural-block citation chain | LOW | No-op pending C-4 |
...
The decision-engine fold is the architecturally honest layer of the SK-V13 close — W5 / W6 / W7 land as designed, the CSP solver drives emission via fail-closed lowering, and the resolver's own telemetry honestly self-labels the W8 / W9 row-movement block. The SK-V14 contract reads this correctly: the resolver clauses survive (SYNTHESIS §1.1 pillars 1-3); the W8 / W9 SCAFFOLD verdict is reaffirmed (SYNTHESIS §1.2 lines 196-197 + §3 C-4 + §4 PRUNE-5 wiring obligation). The SK-V14 starting baseline carries **zero load-bearing scaffold cite** at the row layer; C-4 is the wave that converts SCAFFOLD to LOAD-BEARING under measured row consumption.

The two HIGH findings (W8 / W9 SCAFFOLD persistence) are addressed by C-4; the MED finding (gate-layer-only footprint) is a quantification of the SCAFFOLD verdict and clears as C-4 wires the runtime consumer; the LOW finding (honest self-labelling) needs no action pre-C-4. No CRITICAL violation in the decision-engine layer at SK-V14 starting state.
