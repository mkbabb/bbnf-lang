...
| W6 - Measurement And Close | complete_with_misses | [`FINAL.md`](FINAL.md), this row | W6.3 close docs landed: `FINAL.md` cites resolving artefact for every Hard Gate (MET / MET_WITH_MISSES with named miss / ROUTED with named destination); every Non-Routable Carry resolves to landed evidence or routed follow-on against the post-AZ-IV residual. **Routed**: W6.1 fat-LTO `post-AZ-IV.json` measurement matrix + W6.2 workspace-gates aggregate run did not open inside this authoring window — routed to post-AZ-IV measurement cohort (5-sub-agent profiling per `docs/instructions/PROFILING.md`); AU floor anchored at `docs/benchmarks/post-AU.json`, AZ-III bench-iter at `docs/benchmarks/post-AW-IV.json`. Mechanism complete; matrix is the routed deliverable. Per-wave gate evidence is current (`W0-regen.txt` 9/9; `W1-nextest-pass.txt` 0 fail; W4 regen sync at `9cef79c5`). |
