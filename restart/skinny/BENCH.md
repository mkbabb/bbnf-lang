...
requires one-pass decode+sink materialization inside the parse-that/SinkOnly
primitive boundary.
...
| `alternate_fused_string_sink_plan` | Generated `SinkOnly` consumes raw string spans through `JsonSink::*_source`, but escaped strings are decoded, validated, and materialized into sink/hash facts in one pass inside parse-that rather than by a sink-local helper. | Required before direct string/Unicode close. REDRESS item 49 rejects a generic no-allocation decoded visitor, and item 54 rejects exact two-pass decoded stats in the sink. The admissible row must move the work into a fused primitive and must lift `unicode_mixed`, `unicode_escapes`, and `y_string_unicode`; merely removing allocation is not a pass condition. | Tests whether direct `N-direct` string rows are materialization-bound rather than retained-view or codegen-authority bound. |
