...
- **Codegen**: Grammar → `lower/` (5 sub-modules: `mod.rs`, `string_interner.rs`, `fn_table.rs`, `expression.rs`, `metadata.rs`) → bbnf-ir `Module` → `codegen/` (single monolithic path handling Arena, Owned, Span, and Prettify modes) + `ir_enums.rs`/`ir_types.rs`. `pipeline.rs` orchestrates the full lowering + codegen sequence.
...
- **Monolithic codegen**: `codegen/` is the sole codegen path. Handles Arena mode (`#[parser(arena)]`), Owned mode (default), Span mode (`#[parser(span)]`), and Prettify mode (`#[parser(prettify)]`). Generates direct recursive functions—zero combinator overhead. `MonoCtx` tracks fusion eligibility, single-site inline eligibility, dispatch-guaranteed-byte, `current_rule_name`, and hoisted leaf-parser bindings. Unified `SepByConfig` + `emit_mono_sep_by_core` handles all three sep_by variants (bare, ws-aware, delimited-with-terminator).
