...
/// Invoke `cargo xtask regen --grammar bbnf --output <dest_dir>` from
/// `workspace_root`. Panics with the captured stderr on non-zero exit
/// so the test failure surfaces actionable detail.
fn run_xtask_regen(workspace_root: &Path, dest_dir: &Path) {
    ...
    if !status.status.success() {
        panic!(
            "cargo xtask regen --grammar bbnf --output {dest:?} failed:\n\
             ── stdout ──\n{stdout}\n── stderr ──\n{stderr}",
            dest = dest_dir,
...
fn bbnf_regen_is_idempotent() {
    ...
    // Stage A: first regen onto stage-a/.
    run_xtask_regen(&workspace, &stage_a_dir);
    let stage_a_path = stage_a_dir.join("bbnf.rs");
    let stage_a_bytes = std::fs::read(&stage_a_path)
        .expect("read stage-a bbnf.rs");

    // Stage B: second regen onto stage-b/. The xtask process is fresh
    // (no cached IR-pipeline state across invocations); a divergence
    // here means the regen pass itself is non-idempotent.
    run_xtask_regen(&workspace, &stage_b_dir);
    let stage_b_path = stage_b_dir.join("bbnf.rs");
    let stage_b_bytes = std::fs::read(&stage_b_path)
        .expect("read stage-b bbnf.rs");

    if let Some(divergence) = first_divergence(&stage_a_bytes, &stage_b_bytes) {
        panic!(
            "BBNF regen is NOT idempotent: stage-a and stage-b emissions \
             diverge.\n\n{}",
            diff_snippet(&stage_a_bytes, &stage_b_bytes, divergence),
        );
    }

    // Belt-and-braces sanity: regenerated bytes should be non-empty.
    assert!(
        !stage_a_bytes.is_empty(),
        "stage-a bbnf.rs is empty — xtask regen produced no output"
    );
    assert_eq!(
        stage_a_bytes.len(),
        stage_b_bytes.len(),
