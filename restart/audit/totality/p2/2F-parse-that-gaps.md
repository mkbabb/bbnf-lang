...
prior_cycle_dispositions_folded:
  ...
  rejected: []
  revised: [PTG-SIMD-SPAN-SCAN, PTG-STRING-SCAN-UTF8, PTG-UNICODE-ESCAPE-CODEC, PTG-INTEGER-SWAR-DOTPROD, PTG-CSS-SCANNER-GAP, F-CH3-2F-08, F-CH5-V1-04]
...
v6_fold_authority:
  v6_fold_packet: restart/audit/totality/p2/hardening/HARDENING-T-P2-V1-CONSOLIDATED.md
  v6_fold_items: [F-CH3-2F-08 (LAC-2F-V5-01 CH3 pre-flight reflex), F-CH5-V1-04 (LAC-2F-V5-02 Lock 1 elevation to T-P3 §3C amendment surface)]
...
|---|---|
| **Q1.** Can the SK-V14 SPEC W0..W11 plan close R1-R10 acceptance without a runtime DFA dispatch surface in `bbnf-regex`? | Inspect grammar-authoritative migration Phase 3 host-function requirements per `[grammar-authoritative-status]`; if BBNF-self literal-regex matching (`grammar/bbnf/bbnf.bbnf:11-13`) is the only hot consumer, the 322-line classifier suffices. If CSS L4 `<at-rule>` block parsing or BBNF host-fn dispatch needs runtime DFAs, `bbnf-regex` must absorb the `regex-engine.md` pipeline (SK-V14 W11 the natural locus). **CH3 regression precondition (V6 F-CH3-2F-08):** before Q1 resolves toward absorption, scan `skinny/REDRESS.md` and `restart/skinny/tranches/sk-v{1..14}/` for any prior DFA / NFA / Aho-Corasick / regex-engine admission attempt; any prior measured rejection must be carried as a material-differential row (Lock 1 / Lock 16 manifest entry) before any admissibility row dispatches. |
...
|---|---|---|---|
| **LAC-2F-V5-01** | Lock 16 (SIMD/ASM allowlist + abstract-primitive declarations) | Add `bbnf-regex::Dfa` admissibility row IF Q1 resolves toward absorbing the `regex-engine.md` pipeline. Admissibility requires: (a) scalar reference (Hoehrmann/Thompson straightforward construction at `regex-engine.md:28-44`); (b) checkasm-parity equivalence to `regex-automata::meta::Regex::find` over byte stream; (c) same-wave consumer (any host-fn or leaf-parser dispatch site). **CH3 pre-flight reflex (V6 F-CH3-2F-08, LOW prophylactic):** (i) before any `bbnf-regex::Dfa` admissibility row dispatches, the absorption wave MUST execute a CH3-class REDRESS regression scan over `skinny/REDRESS.md` and `restart/skinny/tranches/sk-v{1..14}/` for any prior DFA / NFA / Aho-Corasick / regex-engine admission attempt, recording the result inline as a precondition; (ii) the amendment MUST carry an explicit REDRESS pre-block citation listing — at minimum, the routes the forward absorption MUST NOT re-open: REDRESS 96 (retained class-column substrate, falsified per `skinny/REDRESS.md:2797-2848`), REDRESS 97 (streaming structural cursor, falsified per `:2852-2906`), REDRESS 98 (`G-W3-UNION-SUBSTRATE` retired per `:2910-2950`), plus any prior regex-shaped admission attempt surfaced by clause (i). This restores parity with LAC-2A-V1-01 / LAC-2D-05 / LAC-2E-04 / LAC-2F-V5-02 / LAC-2F-V5-04, which all carry the REDRESS pre-block citation inside amendment text. | Q1 resolution + SK-V14 W11 wave admission. |
| **LAC-2F-V5-02** | Lock 1 (substrate-union v+1 manifest) — **load-bearing T-P3 §3C amendment surface** (V6 F-CH5-V1-04 elevation) | Add explicit `prev_in_string`-as-substrate refutation row: simdjson's cross-call retained-quote-mask design (`include/simdjson/arm64/simd.h find_quote_mask_and_bits`) is **inadmissible** under Lock 1 substrate-union; the per-call composed form (Gap PTG-PREV-IN-STRING-LOCK1) is the admissible primitive. Refutation is load-bearing because it caps the per-call SIMD ceiling below simdjson's published 1 GB/s. **V6 elevation (F-CH5-V1-04):** this LAC is no longer a per-cycle amendment candidate — it is promoted to a **T-P3 §3C Lock 1 substrate-union v+1 amendment surface** that T-P3 must emit verbatim as a permanent manifest entry. Binding contract: **no cross-call retained classifier state** (quote-mask, escape-mask, structural-mask, class-stream, prev-state byte, or any prefix-XOR carry word) is admissible under Lock 1 substrate-union; carry MUST stay within a single chunk-call boundary. Generalization: the closure of REDRESS 96 / 97 / 98 (retained class-column / streaming structural cursor / class-lane-only on M5 Max) extends to ALL transient classifier-state primitives, not just the three falsified shapes. The per-call composed form (Gap PTG-PREV-IN-STRING-LOCK1, `scan_string_with_carry_64`) is the admissible substrate-union-compatible primitive; the per-call SIMD ceiling sits structurally below simdjson's published 1 GB/s by construction. Any future SIMD primitive that proposes cross-call classifier-state retention is REJECT under Lock 1 v+1 without further measurement. | REDRESS 96/97/98 + this refutation + V6 F-CH5-V1-04 elevation to T-P3 §3C input. |
