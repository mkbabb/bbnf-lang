...
> plus fixed neutral structural-role ordinals (`Open`, `Close`,
> `Separator`, `PairDelimiter`, `StringDelimiter`, `Terminator`,
> `AtomBoundary`) and data-only `EscapeKind`. Grammar meaning is
> interpreted only inside generated grammar modules, never in generic
> substrate crates, so the union honours Lock 14.
...
- per structural byte, its **event class** — one fixed neutral role ordinal
  from `Open`, `Close`, `Separator`, `PairDelimiter`, `StringDelimiter`,
  `Terminator`, or `AtomBoundary` — so the typed event cursor can read a
  retained offset without embedding grammar meaning in generic crates;
...
The neutral role ordinal is intentionally not a grammar-semantic tag:
`PairDelimiter` may support JSON member pairs, CSS declaration pairs, or
Sheets ranges; `Terminator` may end a CSS declaration or any other grammar
item that lowers to the same structural role. The grammar-specific meaning
of each byte and role is interpreted only by generated grammar modules.
Generic crates see the fixed ordinals and the generated byte-set table.
...
proves byte-disjoint at its use site. The result is a `StructuralAlphabet`
record in `RecognizerFacts`,
...
StructuralAlphabet "json":
  structural bytes : { 0x7B '{', 0x7D '}', 0x5B '[', 0x5D ']',
                       0x3A ':', 0x2C ',', 0x22 '"' }
  event classes    : '{' -> Open             '}' -> Close
                     '[' -> Open             ']' -> Close
                     ':' -> PairDelimiter    ',' -> Separator
                     '"' -> StringDelimiter
  string delimiters: { 0x22 '"' }
  escape byte      : 0x5C '\'
  escape kind      : EscapeKind::Backslash
...
not become event-class names. The StructuralAlphabet's event-class field
is the fixed neutral role ordinal; the string-delimiter field is a *set*,
not a single byte. Both are data. The CSS L4 colour-function emit, the
comment `/* */` handling, and the at-rule prelude are *not*
structural-alphabet matters — they are grammar-source + metadata per Lock
14(b); the StructuralAlphabet carries only the byte-skeleton and neutral
roles, not semantic deviations.
...
StructuralAlphabet "google_sheets":
  ...
  escape byte      : 0x22 '"'   (Sheets escapes a quote by doubling it;
                     ...
                     idiom, see note)
  escape kind      : EscapeKind::DoubledDelimiter
  pad/clamp        : EOB_PAD_CLAMP, 64-byte tail pad
```
...
backslash-escape rule. *Which* rule is itself a small data-only enum field
on the alphabet (`EscapeKind ∈ { Backslash, DoubledDelimiter, None }`) —
still data, still grammar-neutral, still emitted by codegen. The Sheets
`:` pair delimiter and `{}` grouped spans are skeleton bytes; the
...
first-set is provably byte-disjoint at use, classifies each into one of
the fixed neutral role ordinals from the grammar's group/repetition/pair
structure, identifies string delimiters from rules whose body is a
delimited span, and emits the record. If the grammar has *no*
...
`ValueRef`, `path!`, and any retained-view/direct consumers touched by the
representation change are migrated in the same wave.
...
field would erode Lock 14 if every grammar contributed its own class kinds
instead of fixed neutral roles. Mitigation: §4 now uses only the fixed,
small, grammar-neutral set of structural roles
(`Open`, `Close`, `Separator`, `PairDelimiter`, `StringDelimiter`,
`Terminator`, `AtomBoundary`) plus data-only `EscapeKind`. CSS's
declaration terminator maps to `Terminator`; Sheets' range colon maps to
`PairDelimiter`; call, block, array, object, and attribute meanings stay
inside generated grammar modules.
