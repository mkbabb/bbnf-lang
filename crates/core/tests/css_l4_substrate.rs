...
/// the StructRegistry.
fn layout_for(rule_name: &str, kind: LayoutKind) -> StructLayout {
    StructLayout {
        rule_id: 0,
...
fn struct_builder_threads_color_function() {
    let mut builder = CssStructBuilder::<'static>::new();
    let layout = layout_for("colorFunction", LayoutKind::Struct);
...
fn document_get_path_query_resolves_string() {
    ...
    let mut builder = CssStructBuilder::<'static>::new();
    let sheet = layout_for("stylesheet", LayoutKind::Struct);
    let style = layout_for("qualifiedRule", LayoutKind::Struct);
    let decl = layout_for("declaration", LayoutKind::Struct);
