...
pub fn generate_enum(ctx: &IrCodegenCtx<'_>) -> TokenStream {
    ...
    //
    // Tranche W phase 5b: derive Copy on the parser enum. Every variant
    // generated by the BBNF backend is structurally Copy — its payloads
    // are some combination of `Span<'a>` (Copy), slab references
    // `&'a Enum<'a>` (Copy), slice references `&'a [_]` (Copy), tuples
    // of those, and `Option<_>` thereof. The `__Phantom` variant uses
    // `PhantomData<&'a ()>` which is also Copy. Adding the derive lets
    // `BumpSlab::alloc_slice_copy` (a single `copy_nonoverlapping`) take
    // over from `alloc_slice_clone` (a per-element `Clone::clone` loop).
    if has_sub_variants {
        quote! {
