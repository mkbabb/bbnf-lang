...
Type system algorithm: HM inference generates equality constraints and principal schemes; expected-type check/synth handles explicit signatures, annotations, directives, chain steps, and directed subsumption edges; first-order unification solves equality constraints. OutsideIn-style implication constraints are reserved for future local-equality constructs and are not a V1 generic-rule requirement. CSP solves finite non-HM choices for host overload, layout representation, recognizer eligibility, direct/tape materialization, recovery strategy, backend plan, and extraction legality. Host overloads with determining arguments emit explicit improvement constraints, CHR-shaped where applicable, before finite CSP selection.

Generic rule typing: V1 generic rules such as `Object<V>` are rank-1 HM parametric schemes. Validation materializes a finite `(RuleId, TypeArgs)` instance set reachable from a concrete entry or metadata-declared export; recursive generic cycles require a decreasing structural argument, an explicit return annotation, or rejection with `BBNF-GENERIC-CYCLE`. V1 has no GADT branch-local equality surface. A later indexed/local-equality feature must arrive by amendment with annotation rules and `BBNF-LOCAL-EQUALITY-ANNOTATION`. Record narrowing is limited to finite generated-shape coercions where source and target shapes are both known; row-polymorphism and open structural record subtyping are out of V1.
