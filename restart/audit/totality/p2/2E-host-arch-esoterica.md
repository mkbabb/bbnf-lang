...
counted_source_ids:
  ...
  - SRC-LEMIRE-PCLMUL-PREFIX-BLOG
  - SRC-LEMIRE-VBMI2-BLOG
  - SRC-MULA-MOVMASK-NEON
...
prior_cycle_dispositions_folded:
  accepted: [V4-CH1, V4-CH2, V4-CH3, V4-CH4, V4-CH5, V4-CH6, V5-confirm-all]
  rejected: []
  revised: []
...
Whitepaper 2014; Lemire 2016 prefix-XOR via PCLMUL blog), (ii) the
published LD4-interleaved classify primitive (`vld4q_u8` decomposition
per Arm Neon Intrinsics + Downs's interleaved-load blog), (iii) the
CSSC v8.9-A / v9.4-A spec anchor (Arm Architecture Reference Manual
§C7.2), (iv) the explicit refutation of "NEON `svmatch_u8` port" (SVE2-
only per Arm ARM §C2.2), (v) per-entry x86 AVX-512 secondary expansion
(VBMI2/GFNI/VPCLMUL/k-mask/AVX-IFMA/VNNI/BITALG) each tied to Intel
Intrinsics Guide + Mula/Lemire/Wikichip primary technique citations.
...
Intel SDM, and Wikichip GFNI page. Named-technique posts are Lemire's
simdjson paper + PCLMUL prefix-XOR blog + VBMI2 blog, Mula's NEON
movemask + AVX-512 VBMI + GFNI bit-manipulation, Validark's compress-
store work, Downs's interleaved-loads blog, and Arm's official Dot
Product technical blog. Local bbnf evidence is `SRC-BBNF-A64`,
...
| SRC-LEMIRE-PCLMUL-PREFIX-BLOG | Daniel Lemire, "Quickly computing a prefix-XOR (parallel prefix XOR) via carry-less multiplication", 2016 ([lemire.me/blog/2016/05/23](https://lemire.me/blog/2016/05/23/quickly-computing-a-prefix-xor-with-pclmulqdq/)) | Named-technique primary for PCLMUL/PMULL prefix-XOR; the algebra `M ⊗ (~0)` projecting prefix-XOR in one polynomial multiply. |
| SRC-LEMIRE-VBMI2-BLOG | Daniel Lemire, "AVX-512 VBMI2: shifting + extracting", 2019-2020 ([lemire.me/blog](https://lemire.me/blog/2019/06/19/avx-512-vpcompressb/)) | Named-technique primary for `VPCOMPRESSB` byte-level compress-store; the AVX-512 analogue of bulk-emit-positions. |
| SRC-MULA-MOVMASK-NEON | Wojciech Mula, "Bit shuffling with SSE / AVX-512 / NEON" + NEON pseudo-movemask emulation ([0x80.pl/articles/simd-pmovmskb.html](http://0x80.pl/articles/simd-pmovmskb.html)) | Named-technique primary for the SHRN/SRI movemask pattern at `bbnf-simd/src/aarch64/movemask.rs:1-25`. |
| SRC-MULA-AVX512-VBMI | Wojciech Mula, "AVX-512 VBMI - permuting bytes" ([0x80.pl/notesen/2019-01-05-avx512vbmi.html](http://0x80.pl/notesen/2019-01-05-avx512vbmi.html)) | `VPERMB` / `VPERMI2B` byte-table-lookup analogue of NEON TBL/TBX; informs cross-arch primitive vocabulary. |
| SRC-MULA-GFNI-BIT-MANIPULATION | Wojciech Mula, "GFNI for bit-level manipulation" ([0x80.pl/notesen/2022-10-18-avx512vbmi2-gfni-conversions.html](http://0x80.pl/notesen/2022-10-18-avx512vbmi2-gfni-conversions.html)) | GFNI as single-instruction byte-classifier kernel; the simdjson SK-V12-cited usage anchor. |
...
REDRESS-88/-89 consumers. The Lemire 2016 PCLMUL prefix-XOR algebra is
the algorithm-correctness anchor for *what the primitive computes*; the
SK-V7 W10/W10b measured rejection of the consumer remains binding.
...
|---|---|---|---|---|---|
| **C-P2C-2** `pmull_cssc_structural_union_emit64` | `PrefixXor64Pmull` + `NextSetBitCssc` composed | `+aes` + `+cssc` (FEAT_PMULL + FEAT_CSSC) | `SRC-LEMIRE-PCLMUL-PREFIX-BLOG` (Lemire 2016 PCLMUL prefix-XOR) + `SRC-LEMIRE-SIMDJSON-PAPER` §3.3 + `SRC-A64-ARM-ARM` §C7/§C7.2 + `SRC-INTEL-CLMUL-WP` (lineage) | **GROUNDED at abstract-primitive level; PRE-BLOCKED at consumer level.** The published technique is well-established (Lemire 2016 + simdjson VLDB 2019); the abstract primitive is admissible. The REDRESS 88+89 *consumer rejection* binds — only a SIMD-first union consumer that DELETES the scalar consume step opens this. Lock 16 manifest fields: substrate `existing_tape`/`direct_sink`/`admitted_fact_output`, retention `generated_function`, policy `generated_grammar`. | V6 surfaces: (a) **`BicXor3Bcax`** as an *adjacent* primitive (BCAX matches AND-NOT-XOR scanner algebra); (b) the *abstract-primitive lineage register* — `PrefixXorPmull` is parameterized by lane width (NEON 128b, x86 VPCLMULQDQ 256/512b); cross-arch primitive vocabulary is parameterized, not instruction-specialized. |
...
| **C-P2C-5** `string_special_64_context` | `ByteClassLookup64TBL` + `ByteContextExt` + movemask (SHRN/SRI per `SRC-MULA-MOVMASK-NEON`) | Advanced SIMD baseline | `SRC-A64-NEON` `vextq_u8` → EXT + `SRC-LEMIRE-SIMDJSON-PAPER` §3.2 string scan + `SRC-MULA-MOVMASK-NEON` | **GROUNDED; conditional as support primitive.** Folds into C-P2C-2 Union-C consumer (if landed) or C-P2C-4 64-byte oracle pre-step. Standalone admission requires measured string-mask consumer that does not exist at SK-V14 envelope-bound profile. | V6 surfaces: (a) **`BicXor3Bcax`** for the string-mask AND-NOT algebra; (b) cross-arch x86 **`VPMOVMSKB`** (`SRC-X86-INTEL-INTRIN`) is the single-instruction movemask vs aarch64's 5-instruction SHRN/SRI sequence. |
...
- **`Interleave4Classify` (LD4)** — `vld4q_u8` 4-way de-interleave load; abstract primitive for SoA byte-stream classification. Hardware gate: Advanced SIMD baseline. Citation: `SRC-A64-NEON` + `SRC-DOWNS-INTERLEAVED-LOADS`. Re-evaluation if UTF-8 / string deinterleave becomes hot leaf.
- **`BicXor3Bcax`** — `vbcaxq_u8` computes `a XOR (b AND NOT c)` in one cycle. Hardware gate: `+sha3` / FEAT_SHA3 (M-series advertises). Citation: `SRC-A64-NEON` + `SRC-A64-ARM-ARM` §C7. **Higher relevance than C-P2C-6 EOR3** because SK-V14 scanner uses AND-NOT chains.
- **`Crc32CHash`** — hardware-accelerated CRC32C; cross-arch (aarch64 FEAT_CRC32 + x86 SSE 4.2 CRC32). Hardware gate: `+crc` / `__ARM_FEATURE_CRC32`. Citation: `SRC-A64-ACLE` + `SRC-A64-NEON`. Re-evaluation if future digest / fact-stream consumer wants hardware hash.
...
| `movemask` (SHRN/SRI) | production-wired | Continue; consumed in NEON stripe; documented per `SRC-MULA-MOVMASK-NEON`. |
...
| LAC-2E-04 | refinement | Lock 1 / Lock 16 | For PMULL/CSSC reopen attempts (C-P2C-2 Union-C), require material-differential text that distinguishes new union/consumer shape from REDRESS 88/89 and REDRESS 96/97/98, plus a micro-prove-first artifact before S-P3 wave-scoping. V6 fold-up: require explicit citation of Lemire 2016 PCLMUL prefix-XOR + simdjson VLDB 2019 lineage as the abstract-primitive justification, distinct from the consumer-specific REDRESS rejection. | REDRESS 88/89 body-fill failures; REDRESS 96/97/98 union failures; SK-V13/V14 scoping Union-C risk table; Lemire 2016 + simdjson lineage. |
