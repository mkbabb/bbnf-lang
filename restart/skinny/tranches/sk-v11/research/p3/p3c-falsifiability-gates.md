...
| `G-W1-NONJSON-HARNESS` | W1 non-JSON generated-parser harness and telemetry gate | C9 accounting + Lock 14 enabling surface | A generated non-JSON bench lane exists for the selected grammar domain, records generated Track 1, independent Track 2/oracle, grammar id, output plane, run id, feature mask, same-wave consumer class, and strict semantic equality; missing required fields are rejected by the gate. No behavior row admission is allowed. | Revert harness/report/gate/codegen changes if the row is absent, Track 2/oracle is coupled to Track 1, grammar id/output plane are missing, or the gate accepts producer-only fields. |
| `G-W2-CSS-GENERATED-INTERVENTION` | W2 CSS L4 generated direct/typed intervention proof | C1/C2/C4/C5/C6 with C7 support; C8 oracle only; C9 accounting | At least one generated CSS L4 declaration-values direct or typed parser row is benchmarked and admitted with generated Track 1 before/after Mbps, independent Track 2/oracle Mbps, strict output equality, primitive self-time, no generic-crate JSON policy, and at least `floor(W1_css_open_mbps * 1.01)` on the selected non-JSON row. JSON companion rows `github_events >= 13403`, `update_center >= 10059`, `random >= 7878`, or `instruments >= 8969` may admit only if named and measured. | Revert generic/codegen/runtime changes if the CSS row is absent, unbenchmarked, oracle-coupled to Track 1, below the 1% improvement floor, leaks JSON policy into generic crates, or misses a named guard floor. |
| `G-W3-NUMERIC-SEQUENCE-DIRECT` | W3 numeric direct closure slice | C4 digit span/accumulate; D4 number-slot emit; optional C1/C5 support | Selected numeric/container rows must clear both-track floors: `canada` 10637, `mesh` 8675, `numbers` 2425, and `instruments` 8969 if selected. `marine_ik/direct_to_struct` is a direct guard, not a target, and must hold its guard floors. Generated numeric output must match Track 2/oracle exactly. | Revert on numeric semantic mismatch, f64/fallback policy drift, no selected row admits, direct guard miss, or non-JSON numeric proof failure when generic code is touched. |
...
| `G-W7-DIGEST-SINK` | W7 output digest/hash host sink | C8 output sink/oracle only; C9 accounting | A fresh post-W6 profile still names `output_digest_hash` as limiting on selected residual rows; selected rows must clear their §3.2 direct floors with strict Track 1/Track 2-or-oracle parity. C8 cannot enter generic parser crates or close a parser row without a product output sink consumer. | Revert on digest mismatch, hidden semantic string/hash side table, cache-hint-only route, no selected row admits, or guard-floor miss. |
...
- Non-JSON admission requires a generated direct or typed parser row, an
  ...
  `G-W2-CSS-GENERATED-INTERVENTION`, with W1 responsible for creating the
  baseline harness and telemetry lane.
...
- concrete Mbps threshold or same-wave captured non-JSON baseline threshold;
