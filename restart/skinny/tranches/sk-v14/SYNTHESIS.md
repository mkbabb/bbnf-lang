...
| **R3** | PRUNE waves before any new admit attempt | PRUNE-1 revert W14.1–.5; PRUNE-2 delete 7 CSS templates + revert 24 CSS rows; PRUNE-3 trait-dispatch + grammar-agnostic codegen template; PRUNE-4 67 hand-written per-grammar runtime files in `crates/core/src/runtime/{grammar}/` refactored to emitted output (9 sub-waves, W6.0..W6.8); PRUNE-5 wire W8 + W9 from SCAFFOLD to LOAD-BEARING. |
