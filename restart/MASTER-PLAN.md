...
| H.W4 (workload gates + direct-to-struct `SinkOnly` closure + 5-shape backend_shape per-rule selection) | **PARTIALLY LANDED.** The generated `SinkOnly` path is correctness-green, lowerer-authored from BIR, preserves raw string spans to `JsonSink::*_source`, and keeps retained view-walk digest as parity oracle. Direct receiver/source-shape redress lifts four rows (`citm_catalog`, `mesh`, `marine_ik`, `numbers`) but overall gate remains `N-direct / NoGo`. Generic decoded visitor, sink-local exact-stats helper, and quote-source streaming hasher are rejected. | SK-V6 Wave 3 targets remaining direct rows only after Wave 1 profiles; field-layout materializer or same-loop `SinkOnly` typed output is the admissible class. |
