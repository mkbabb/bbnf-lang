...
pub use number::{match_number_span, match_number_span_from_first, NumberSpan};
...
impl StringMode {
    ...
    const fn validates_utf8(self) -> bool {
        matches!(self, Self::Utf8 | Self::GrammarString)
    }
}
...
pub fn validate_number(input: &str) -> bool {
    match number::match_number_span(input.as_bytes(), 0) {
...
#[inline(always)]
pub fn match_string_at_quote_trusted_utf8(
    input: &[u8],
    offset: usize,
...
pub fn match_string_at_quote(
    ...
    debug_assert_eq!(input.get(offset), Some(&b'"'));
    let mut scan = skip_string_plain(input, offset + 1, mode)?;
    ...
    loop {
        ...
        match byte {
            ...
            b'\\' => {
                ...
                flags.insert(StringFlags::NEEDS_DECODE);
                cursor = validate_string_escape(input, cursor)?;
                scan = skip_string_plain(input, cursor, mode)?;
...
#[inline(always)]
fn validate_string_escape(input: &[u8], slash: usize) -> Result<usize, RegexError> {
    let escape = slash + 1;
    match input.get(escape).copied() {
        Some(b'"' | b'\\' | b'/' | b'b' | b'f' | b'n' | b'r' | b't') => Ok(escape + 1),
        Some(b'u') => validate_unicode_escape_run(input, slash),
...
pub fn decode_unicode_escape(input: &[u8], slash: usize) -> Result<(char, usize), RegexError> {
...
#[inline(always)]
fn validate_unicode_escape_run(input: &[u8], mut slash: usize) -> Result<usize, RegexError> {
    loop {
        let first_hex = slash + 2;
        let first = read_hex_unit_with_error_offset(input, first_hex, slash)?;
        let mut cursor = first_hex + 4;
...
#[inline(always)]
fn string_special_mask(block: u64) -> u64 {
...
pub fn classify_string_content(input: &[u8], start: usize, end: usize) -> Result<bool, RegexError> {
    debug_assert!(start <= end && end <= input.len());
    #[cfg(target_arch = "aarch64")]
    {
        return unsafe { neon_classify_string_content(input, start, end) };
    }

    #[allow(unreachable_code)]
    scalar_classify_string_content(input, start, end)
}

fn scalar_classify_string_content(
    input: &[u8],
    start: usize,
    end: usize,
...
#[cfg(target_arch = "aarch64")]
unsafe fn neon_classify_string_content(
    input: &[u8],
    start: usize,
    end: usize,
) -> Result<bool, RegexError> {
    ...
    scalar_classify_string_content(input, cursor, end)
}
...
pub fn validate_string(input: &str) -> bool {
    match match_string(input.as_bytes(), 0, StringMode::Utf8) {
        Ok(span) => span.raw_end == input.len(),
        Err(_) => false,
    }
}

pub fn unescape_string(raw_content: &str) -> Result<Cow<'_, str>, RegexError> {
    if !raw_content.as_bytes().contains(&b'\\') {
        classify_string_content(raw_content.as_bytes(), 0, raw_content.len())?;
            ...
            b'\\' => {
                ...
                match bytes.get(cursor).copied() {
                    ...
                    Some(b'u') => {
                        ...
                        let (ch, next) = decode_unicode_escape(bytes, slash)?;
                        out.push(ch);
                        cursor = next;
                    }
                    Some(_) | None => {
...
fn find_next_escape_or_control(input: &[u8], mut cursor: usize) -> usize {
    while cursor + 8 <= input.len() {
        let block = unsafe { std::ptr::read_unaligned(input.as_ptr().add(cursor).cast::<u64>()) };
        let mask = string_escape_control_mask(block);
        if mask != 0 {
...
#[inline(always)]
fn string_escape_control_mask(block: u64) -> u64 {
    const SLASHES: u64 = 0x5c5c_5c5c_5c5c_5c5c;
    const CONTROL_LIMITS: u64 = 0x2020_2020_2020_2020;
    const ONES: u64 = 0x0101_0101_0101_0101;
...
mod tests {
    ...
    fn numbers_match_json_shape() {
        for valid in ["0", "-0", "12", "-12.34", "6.02e23", "1E-9"] {
            assert!(validate_number(valid), "{valid}");
        }

        for invalid in ["", "-", "01", "1.", "1e", "+1"] {
            assert!(!validate_number(invalid), "{invalid}");
        }
    }

    #[test]
    fn strings_report_escape_state() {
        let plain = match_string(br#""abc""#, 0, StringMode::Utf8).unwrap();
    ...
    fn string_primitive_reports_flags() {
        let plain = match_string("\"abc\"".as_bytes(), 0, StringMode::Utf8).unwrap();
        ...
        let unicode = match_string("\"cafe\u{00e9}\"".as_bytes(), 0, StringMode::Utf8).unwrap();
    ...
    fn string_matcher_accepts_dense_unicode_escape_runs() {
        let input = br#""\u0041\u03a9\uD834\uDD1E""#;
        let escaped = match_string(input, 0, StringMode::Utf8).unwrap();
    ...
    fn string_matcher_validates_raw_utf8() {
        for input in [
            ...
            let error = match_string(input, 0, StringMode::Utf8).unwrap_err();
    ...
    fn string_matcher_reports_error_offsets() {
        for (input, offset, kind) in [
            ...
            let error = match_string(input, 2, StringMode::Utf8).unwrap_err();
            assert_eq!(error.kind, kind, "{input:?}");
    ...
    fn string_content_prefilter_detects_escapes_and_controls() {
        assert!(!classify_string_content(b"abc", 0, 3).unwrap());
        assert!(classify_string_content(br#"a\nb"#, 0, 4).unwrap());
        assert!(matches!(
            classify_string_content(b"a\nb", 0, 3),
            Err(RegexError {
                kind: RegexErrorKind::ControlCharacter,
                ..
            })
    ...
    fn unescape_handles_surrogate_pairs() {
        ...
        assert_eq!(
            unescape_string(r#"\uD834\uDD1E"#).unwrap().as_ref(),
            "\u{1d11e}"
        );
    }
    ...
    fn unicode_escape_primitive_decodes_boundaries() {
        assert_eq!(
            decode_unicode_escape(br#"\u0000"#, 0).unwrap(),
            ('\u{0000}', 6)
        );
        assert_eq!(
            decode_unicode_escape(br#"\uFFFF"#, 0).unwrap(),
        ...
        assert_eq!(
            decode_unicode_escape(br#"\uD800\uDC00"#, 0).unwrap(),
            ('\u{10000}', 12)
        );
        assert_eq!(
            decode_unicode_escape(br#"\uDBFF\uDFFF"#, 0).unwrap(),
            ('\u{10ffff}', 12)
        );
    }
    ...
    fn unescape_rejects_invalid_unicode_with_offsets() {
        for (input, offset, kind) in [
            ...
            let error = unescape_string(input).unwrap_err();
            assert_eq!(error.kind, kind, "{input}");
            assert_eq!(error.offset, offset, "{input}");
        }
    }
    ...
    fn string_matcher_rejects_lone_surrogates() {
        assert!(matches!(
            match_string(br#""\uD800""#, 0, StringMode::Utf8),
            Err(RegexError {
                kind: RegexErrorKind::InvalidSurrogatePair,
                ..
            })
    ...
    fn unescape_accepts_unicode_noncharacters() {
        assert_eq!(
            unescape_string(r#"\uDBFF\uDFFE"#).unwrap().as_ref(),
            "\u{10fffe}"
        );
    }
}
