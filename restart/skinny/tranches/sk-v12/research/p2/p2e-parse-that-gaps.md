...
| Candidate | Shape | Scalar-ref status | Arch | P1 antecedent | Layer | Same-wave consumer | Micro-proof need |
|---|---|---|---|---|---|---|---|
| `pt_byte_set_run_skip` | `fn skip_byte_set_run(input: &[u8], offset: usize, set: ByteSet) -> usize` returns first non-member offset. Comment/layout policy stays in generated grammar. | New scalar loop required; JSON `skip_ascii_whitespace` is a compatibility reference, not the generic API. | Scalar first; AArch64 may use `byte_class_from_eq_set_64` or `byte_class_from_table_64`; x86 out of SK-V12 scope. | `ascii_whitespace_skip`, `container_dispatch`. | Layer 1, optionally backed by Layer 0 class masks. | CSS `@ws` skipper for whitespace/comment boundary, with JSON whitespace as guard. | Empty/all-member spans, first/last miss, all offsets/tails, CSS comment boundary handoff. If SIMD is used: strict `checkasm_byte_class_from_eq_set_64`/`checkasm_byte_class_from_table_64` parity plus generated CSS equality. |
| `l0_byte_class_mask64` | `fn byte_class_mask64(block: &[u8; 64], table_or_set: ClassSpec) -> u64` returns transient membership mask only. | Existing scalar refs cover table and <=8-byte eq-set masks (`skinny/crates/bbnf-simd/src/lib.rs:235-271`), but CSS class specs need generated-table parity cases. | AArch64 TBL/eq-set eligible; wrappers that delegate to scalar remain support-only; x86 not targeted. | `container_dispatch`, `simd_movemask`, `ascii_whitespace_skip`. | Layer 0 vendored substrate. | CSS delimiter/whitespace/identifier-start classifiers called by `pt_css_token_classify_span` and `pt_byte_set_run_skip` in the same wave. | Generated ClassSpec corpus from CSS FIRST sets; checkasm across density, alignment, tails, non-ASCII bytes, and every CSS delimiter set. No retained position vector unless consumed by tape. |
| `pt_css_token_classify_span` | `fn classify_token_span(input, offset, token_table: TokenClassTable) -> TokenSpan { class, end, flags }`; flags are grammar-neutral facts such as escaped, non_ascii, numeric, or needs_host_decode. | New scalar DFA/reference required from grammar metadata. Existing JSON structural scan is not a CSS token oracle. | Scalar first; may call Layer-0 byte class masks and string-special blocks. | `container_dispatch`, `bounded_plain_string_scan`, `unicode_escape_hex_decode`, `number_digit_span`. | Layer 1 bbnf vocabulary. | Generated CSS L4 lexer/front-end for `ident`, `selectorIdent`, strings, `@` rules, hex colors, dimensions, percentages, and keyframe stops. | Golden CSS token/fact stream against independent oracle/lightningcss adapter; fuzz mixed comments/strings/escapes; prove no JSON structural alphabet hardcode. SIMD backing needs checkasm for each mask cell. |
| `pt_bounded_string_span` | `fn bounded_string_span(input, body_start, cap, policy: StringSpanPolicy) -> Result<StringSpan, ScanMiss>` returns closing delimiter/end and flags; delimiter/escape/control/UTF-8 policy is caller-owned. | New scalar public reference; existing generated JSON tiny loop and parse-that private plain loops are references (`skinny/crates/runtime/src/grammars/json/generated.rs:161-185`; `skinny/crates/parse-that-regex/src/lib.rs:462-547`). | Scalar first; AArch64 `scan_string_special_block` only after strict parity; x86 out of scope. | `bounded_plain_string_scan`, `string_escape_decode`. | Layer 1, with optional Layer 0 string-special block. | CSS single/double quoted strings and selector escaped identifiers; JSON string rows as guard only. | Every delimiter/escape/control position under caps, non-ASCII before terminator, unterminated/tail cases, quote modes. AArch64 backing needs 16/64-byte checkasm plus caller-level CSS equality. |
| `pt_hex_unit_decode` | `fn hex_unit_decode(input, offset, width: HexWidth) -> Result<HexUnit, HexError>` for fixed widths 3/4/6/8 and CSS variable-width policy supplied by caller. | Existing scalar `read_hex_unit_scalar` and `unescape_uxxxx_scalar` are references for width 4 (`skinny/crates/parse-that-regex/src/lib.rs:945-959`; `skinny/crates/bbnf-simd/src/aarch64/unescape_uxxxx.rs:40-47`). New scalar required for CSS hex widths. | Scalar first; AArch64 `unescape_uxxxx_x4_neon` can back width-4 batches only after strict x4 checkasm. | `unicode_escape_hex_decode`, `string_escape_decode`. | Layer 1 for decode API; Layer 0 for nibble/TBL kernels. | CSS hex colors `#RGB/#RRGGBB/#RGBA/#RRGGBBAA` and CSS/string escapes; JSON unicode escape guard. | Width 3/4/6/8 valid/invalid, mixed-case, short input, invalid offset, CSS termination policy. x4 SIMD needs invalid/mixed/alignment/surrogate tests, not smoke-only. |
| `pt_escaped_segments` | `fn escaped_segments(input, body: Range<usize>, policy: EscapePolicy, visit: impl FnMut(EscapeSegment)) -> Result<()>`; emits raw spans and decoded scalar/byte units without allocating. | New scalar segment-stream oracle; `unescape_string` is semantic reference but materializes `String`/`Cow` (`skinny/crates/parse-that-regex/src/lib.rs:718-809`). | Arch-neutral first; may call `pt_hex_unit_decode`; AArch64 quartet batching optional. | `string_escape_decode`, `unicode_escape_hex_decode`, `bounded_plain_string_scan`. | Layer 1 bbnf vocabulary. | CSS escaped strings/selectors and generated CSS fact stream; JSON unicode rows only guard. | Segment parity against materialized reference, invalid escape offsets, surrogate/Unicode policy delegated to caller, no decoded scratch/stat/hash sidecar. SIMD route requires strict `checkasm_parity`/`checkasm_utf8_block` plus CSS consumer. |
| `pt_digit_run_span_accumulate` | `fn digit_run_span_accumulate(input, offset, max_prefix_digits) -> DigitRun { end, digits, prefix_value, prefix_digits, truncated }`; sign/dot/exponent/unit policy stays outside. | New public scalar ref; private `scan_digit_run` and fixed-width parse helpers are references (`skinny/crates/parse-that-regex/src/number/mod.rs:106-223`). | Scalar first; AArch64 UDOT helper eligible after full checkasm; x86 out of scope. | `number_digit_span`, `typed_direct_projection`. | Layer 1, optionally backed by Layer 0 digit MAC. | CSS `<number>`, dimensions, percentages, keyframe stops; JSON numeric guard rows. | Runs 0..128, block/tail offsets, truncation/overflow, leading-dot caller contexts. UDOT needs x4/x16 parity, invalid-digit lanes, mixed valid/invalid, caller-level row movement. |
| `pt_number_parts` | `fn number_parts(input, offset, policy: NumberLexPolicy) -> Result<NumberPartsSpan, NumberError>` splits sign/int/fraction/exp/end while policy selects JSON/CSS/Sheets leading-dot and suffix rules. | New scalar ref over grammar policy; JSON `match_number_span_from_first` is one policy instance (`skinny/crates/parse-that-regex/src/number/mod.rs:38-90`). | Scalar; may consume `pt_digit_run_span_accumulate`. | `number_digit_span`, `typed_direct_projection`. | Layer 1. | CSS numeric values and units; Sheets numeric formulas; JSON guards. | Equality to current JSON spans under JSON policy, CSS `.5` acceptance under CSS policy, suffix/unit boundary proof, no f64 fallback or mantissa semantic change. |
| `pt_fact_event_emit` | `fn emit_fact_event(stream: &mut FactStream, class: u8, fact_id: FactId, span: Range<usize>, payload: FactPayload)` emits tape/fact events through grammar-derived ids. | New scalar oracle against generated event stream. Existing `EventGrammar`/`JsonFactId`/`SheetsFactId` witness the type shape but not CSS (`skinny/crates/runtime/src/tape/event_grammar.rs:4-24`; `skinny/crates/runtime/src/grammars/json/event_grammar_witness.rs:8-23`; `skinny/crates/runtime/src/grammars/sheets_witness/event_grammar_witness.rs:8-23`). | Arch-neutral. | `typed_direct_projection`, `container_dispatch`, `output_digest_hash`. | Layer 1 runtime/codegen vocabulary, not parse-that scanner. | Canonical CSS L4 fact stream shared by generated Track 1, independent oracle/Track 2, and lightningcss adapter. | Lock 1 proof that fact stream is the tape/projection, not a sidecar; strict fact ordering and payload equality; generated CSS same-plane oracle; no grammar-name arms in generic crates. |
| `pt_fact_stream_digest` | `fn digest_fact_stream(events: impl Iterator<Item = FactEvent>, config: DigestConfig) -> FactDigest` computes canonical output digest from fact events. | New scalar ref required. JSON `JsonDirectDigest` is an output-plane precedent, not reusable CSS policy (`skinny/crates/bbnf-bench/src/direct_struct.rs:16-48`, `:401-422`, `:717-734`). | Arch-neutral first; cache hints remain inventory-only unless a same-wave writer proves them. | `output_digest_hash`, `typed_direct_projection`, `serde_json_oracle_read_parse`. | Layer 1 bench/runtime vocabulary. | CSS fact-stream equality gate and lightningcss comparator; JSON direct digest rows are guard only. | Same event stream must digest identically across Track 1, oracle, and lightningcss adapter; reject missing comparator plane. Must not reopen sink-local decoded hash routes or add digest-only parser behavior. |
