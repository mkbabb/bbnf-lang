...
//!
//! Per AUDIT-2 §3 and AUDIT-6 §3.1 the per-grammar `*View` types
//! drifted in surface area (`JsonView` exposes kind/root/arena/is_*;
//! `CssView` / `SheetsView` were thinner). `typed_accessor_surface.rs`
//! and BA's path executor consume a UNIFORM surface — this trait
//! IS that surface.
//!
...
//!
//! # Backwards re-export of [`crate::backend::rust::view::color`]
//!
//! The pre-W2-act.close `crate::runtime::view` namespace surfaced
//! the [`Color`], [`ColorSpace`], and [`COLOR_PAYLOAD_BYTES`]
//! symbols generated by the typed-`->` backend. Those re-exports are
