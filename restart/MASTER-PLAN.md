...
| H.W1 (typed event cursor over tape projection — load-bearing) | Cost model in `passes::recognizers` derives `LayoutFacts.backend_shape[rule_id]` from existing Grammar IR facts (first-set disjointness, output mode, recovery, host decode, layout scope) per Lock 10 auto-detection mandate. Lowerer at `crates/codegen/src/lower/rust.rs` emits typed `EventCursor` consumption for `OffsetTape`/`EventTape` rules (`event.byte()` instead of `source[pos]`); direct field writes for `SinkOnly`; byte-position fallback for `EagerTape`. Whitespace boundary handling moves into the event cursor (no per-function `skip_ws`). String/number primitive source reads remain inside `parse-that/string` and `parse-that/number` as the only legal source-byte rescans. Parse-time retained projection aux side tables are explicitly rejected by SK-V5 redress item 50. SK-V5 redress item 51 rejects a byte-class whitespace `EventCursor` wrapper (focused `track1_generated` ≈ 7130 / 10291 / 14110 Mbps on twitter / citm_catalog / canada). SK-V5 redress item 53 rejects even a parser-local structural-mask cursor over source bytes (6156 / 8344 / 7139 Mbps on the same triad): consuming a live emit mask is still wrong if it is an additional scanner beside source-byte recursive descent. H.W1 must make structural projection the single parse substrate: scanner writes the tape/event stream and generated lowering consumes that stream, or `CollapsedStage` / `SinkOnly` consumes live masks in the same loop. No new BIR variant; no new BBNF directive. | `parse_value_at` falls below 20% self-time on the current expanded parse blockers (`twitter`, `random`, `unicode_mixed`, `unicode_basic`), or the wave records the exact remaining leaf that replaced it; Class A corpora stay above their S anchors; Unicode rows lose string/Unicode projection as the dominant samply leaf. |
| H.W2 (bbnf-simd kernel contract — host-aarch64 first; pathology-class fix kernels) | `bbnf-simd/aarch64/`: `vqtbl4q_u8` 4-table classifier; structural+terminator no-quote classifier; `bulk_emit_positions_64` set-bit emitter; `vshrn_n_u16` + `vsri` + `zip1` interleaved movemask; `vld1q_u8_x4` quad-load; cross-chunk `vextq_u8`; **Class B NEON TBL-driven `\uXXXX` hex decode** (replaces the scalar `sub/sub/sub/csel` cluster in `unescape_json_string` with `vqtbl1q_u8` 16-byte LUT; 3 ops/nibble vs 11). PASS-2 differential parity via `bbnf-simd/tests/checkasm_parity.rs` is the admission gate. **Class A NEON `match_tiny_plain_string` wiring INVALIDATED (cohort D6 at `restart/skinny/audit/SK-V5-COHORT/skv5-D6-class-ab-novelty.md`):** the kernel was previously wired and regressed `twitter` by ~25%; reverted per `skinny/REDRESS.md:301-313`. The 8-byte scalar early-out is not the hot UTF-8 boundary on twitter, so the kernel targets the wrong layer. The kernel itself stays in tree for a future grammar that targets the 8-byte plain-ASCII string layer; it is not the SK-V5 parse-G fix. The corrected parse-G diagnosis is the NEON UTF-8 codepoint pipeline fold at `skinny/crates/parse-that-regex/src/lib.rs:331-339` (replaces the 0x80 early-exit + scalar fall-through), which lands in SK-V5 Wave 3 (this MASTER-PLAN's H.W3 closure) per cohort B1. | Class B and trusted-UTF-8 work are necessary but did not close the current rows: `unicode_escapes`, `unicode_mixed`, `unicode_basic`, and `y_string_unicode` remain retained/direct blockers in `skinny/RESULTS.md`. SK-V5 redress item 56 restores the focused Canada structural scan to about 41833 Mbps against the 40000 Mbps floor; the checked-in full report still needs a matrix refresh before the stale L row is removed. Admission gate `BBNF_SIMD_STRICT=1 cargo test -p bbnf-simd --release --test checkasm_parity` remains required; Class A wiring claim withdrawn — `random` / `update-center` / `github_events` close routes through H.W1 typed cursor + H.W3 UTF-8 fold, not through `match_tiny_plain_string`. |
...
| H.W4 (workload gates + direct-to-struct `SinkOnly` closure + 5-shape backend_shape per-rule selection) | `bbnf-bench` adds the workload matrix from SK-V3 packet §7: `parse_only`, `parse_full_traversal`, `path_lookup`, `direct_to_struct`, `unicode_string_float`, `memory`, `cycles_per_byte`. Cost-model `derive_backend_shape` exercises all five `BackendShape` variants across the expanded corpora. The generated SinkOnly path is correctness-green, lowerer-authored from BIR, preserves raw string spans to `JsonSink::*_source`, and keeps the retained view-walk digest as a parity oracle; current redress makes only `numbers` pass. SOTA close requires a field-layout decoded-string sink primitive rather than the rejected generic decoded visitor, rejected sink-local exact-stats helper, or rejected quote-source streaming hasher; exact float/string/Unicode materialization inside the direct sink; event-stream consumption; and full-matrix incorporation of the item-56 Canada scan-floor fix. | Expanded corpus has no outcome-G or outcome-L rows and no `N-direct` rows; no correctness/schema rows fail; 5-shape per-rule selection emits to `LayoutFacts.backend_shape` with `BBNF-BACKEND-SHAPE-INCONSISTENT` covering rejected configurations; Track 1 ≥ S on the expanded parse corpus and direct tracks are within `sonic-rs * 1.10` time. |
