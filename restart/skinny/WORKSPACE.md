...
| Direct decoded-string sink delivery | Skinny now passes raw string spans plus decode flags through generated `JsonSink::*_source` hooks. | The source-hook seam is JSON-FAITHFUL and grammar-general; the attempted generic no-allocation decoded visitor is not. V1 closure must land a measured fused decode+sink primitive for grammars that need decoded direct fields, preserving lazy retained views and avoiding parser-side eager decode. |
