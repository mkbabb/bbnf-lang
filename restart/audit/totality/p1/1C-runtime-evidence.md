...
prior_cycle_dispositions_folded:
  ...
  revised:
    ...
    - Update the Lock 14 leak audit to the workspace surface: runtime/mod.rs hand-wires 9 grammar modules + 60+ grammar-named type reexports; verification cmd `find crates/core/src/runtime -mindepth 1 -maxdepth 1 -type d` returns 9 (Lock 14 requires 0).
...
| Generic crates carry ZERO grammar-named modules; ZERO grammar-specific types in their public APIs (Lock 14, `restart/locks/LOCKS.md:220`). | `crates/core/src/runtime/mod.rs:9-21` declares `pub mod bbnf; pub mod bnf; pub mod css_l4; pub mod css_pretty; pub mod csv; pub mod ebnf; pub mod google_sheets; pub mod json; pub mod math;`. `mod.rs:25-71` reexports 60+ grammar-named types (`JsonValue`, `JsonArena`, `CssRule`, `BbnfArena`, `SheetsValue`, `CsvValue`, …). | Unimplemented | The generic `runtime` crate's root is the leak surface. |
| Generic-crate scan `rg -n 'JsonParser\|CssL4Parser\|BbnfBootstrap\|GoogleSheetsParser' crates/runtime/src/` must return ZERO (Lock 14, `restart/locks/LOCKS.md:220`). | Cmd returns matches across `json/parse_with.rs`, `bbnf/parse_with.rs`, `css_l4/parse_with.rs`, `css_l4/document.rs`, `bbnf/document.rs`, `json/document.rs`, `json/mod.rs`, `bbnf/mod.rs`, `css_l4/mod.rs`, `bbnf/serialize.rs`, `json/builder.rs` — at minimum 19 matches (`crates/core/src/runtime/json/parse_with.rs:47,90,93`; `crates/core/src/runtime/bbnf/parse_with.rs:21,61,64`; `crates/core/src/runtime/css_l4/parse_with.rs:22,63`). | Unimplemented | Each `parse_with.rs` literally names `parse_<G>Parser_<rule>` and `__shape_support_<G>Parser` symbols. |
...
| Grammar-named type reexports in runtime root | `crates/core/src/runtime/mod.rs:25-71` reexports 60+ grammar-named types: `BbnfArena, BbnfCompound, …, JsonValue, CssRule, CssDeclaration, SheetsArena, …`. | Divergence (CRIT) | ~50 LOC of reexports to delete; consumers route through `bbnf::grammar::generated::<g>::*` instead. |
| Grammar parser-name leaks in runtime source | `JsonParser` / `CssL4Parser` / `BbnfBootstrap` / `GoogleSheetsParser` strings appear in 19+ sites under `crates/core/src/runtime/{json,bbnf,css_l4,google_sheets}/{parse_with,mod,document,builder,serialize}.rs`. | Divergence (HIGH) | ~120 LOC of doc strings + use statements + symbol references. |
...
| 1C-D4 | Runtime root `mod.rs` hand-wires 9 grammar modules + 60+ grammar-named type reexports — the Lock 14 root-leak. | `crates/core/src/runtime/mod.rs:9-21` + `25-71`. | 80 (root rewrite) | High | Same wave as 1C-D1; root replaced by `include!('manifest.rs')` or build-script registration. | 150 LOC | Generated manifest owner. | direct path:line. |
| 1C-D5 | Per-grammar `parse_with.rs` shims (4 grammars) leak `parse_<G>Parser_<rule>` and `__shape_support_<G>Parser` symbol names. | `crates/core/src/runtime/{json,bbnf,css_l4,google_sheets}/parse_with.rs` — 19+ symbol refs. | 480 | High | Same wave as 1C-D1; PathCursor wiring must move into generator template. | 250 LOC | `bbnf::grammar::generated::<g>::*` emitter + consumer. | per-file path:line. |
...
- `rg -n 'JsonParser|CssL4Parser|BbnfBootstrap|GoogleSheetsParser' crates/core/src/runtime/ | wc -l` → 19+ matches across {json,bbnf,css_l4,google_sheets}/{parse_with,document,mod,builder,serialize}.rs.
