...
prior_cycle_dispositions_folded:
  ...
  rejected:
    - "SKELETON triple DELETE proposal (FSM_DISPATCH_THREADED + FRAME_PUSH/POP_BOUNDED) — doubly inadmissible per T-P2 V5 LOCK refutation density 31:64"
...
| 3F-MIG-004 | LAC-1E-14, 1C-D5, CH2 V3 F2 | `restart/MIGRATION.md:41, 553-577` (non-JSON telemetry + generated code) | CSS L4 fact-stream lacks formal substrate category alongside OffsetTape/EventTape/SinkOnly/CollapsedStage; LAC-1E-14 proposes `FactStream` as 5th BackendShape variant — but this is G-Omega-gated per Lock 1 v+1 since it touches Lock 10 BackendShape canon too. MIGRATION row binds CSS L4 row to `admitted_fact_output` substrate_target with comparator provenance mandatory until T-P3 §3C disposes. |
...
4. **CSS L4 fact-stream telemetry row (3F-MIG-004).** Append to `restart/MIGRATION.md:41` (V1.1 Non-JSON telemetry): "CSS L4 declaration-values is admitted same-plane fact-stream row evidence (`skinny/RESULTS.md:94`) but lacks formal runtime substrate category alongside OffsetTape/EventTape/SinkOnly/CollapsedStage. LAC-1E-14 proposes `FactStream` as 5th BackendShape variant (gates Lock 1 + Lock 10 v+1); until T-P3 §3C disposes (alternative b: stays as `admitted_fact_output` substrate_target without taxonomy promotion), MIGRATION binds CSS L4 row to fenced telemetry with strict comparator provenance + gate-consumed telemetry per Lock 1 V+1 fact-stream wording (`restart/locks/LOCKS.md:66-71`)."
...
| positive | SKELETON triple DELETE refusal row formalises T-P2 V5 31:64 refutation density as a re-proposal gate (same-wave consumer + command + first consumer required). | Propagates to T-P2 V4-style non-shortlist criteria as the future-proposal template. |
...
| 3F-MIG-004 | 80-180 docs/report | 4 | Medium | T-P3 §3C disposition + W8 re-admit | Receiver: CSS L4 fact-stream telemetry plane. | Block forcing CSS fact stream into EventTape; bind to fenced telemetry until §3C disposes. |
...
| CH2 | Does LAC-1E-14 `FactStream` extension to BackendShape (5th variant) gate Lock 1 + Lock 10 amendment simultaneously, or does fact-stream stay as `admitted_fact_output` substrate_target without canon expansion? | T-P3 §3C disposition + Pass Omega Ω-C. | Lock 10 BackendShape canon coherence per `restart/prompts/totality/PASS-3-SYNTHESIS.md §8.2`. | G-Omega pins the BackendShape canon at 4 or 5 variants before SK-V14 W7/W8 dispatch. |
...
- 5-shape BackendShape canon coherent across 3A + 3B + 3E per `restart/prompts/totality/PASS-3-SYNTHESIS.md §8.2:211` (3F MIG-004 fact-stream row defers BackendShape expansion to §3C disposition; does not unilaterally propose 5th variant).
