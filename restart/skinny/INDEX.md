...
| Eager-tape substrate ceiling at ~1.6× sonic-rs; lazy-offset tape proposed as Lock 1 amendment per `restart/skinny/audit/LAZY-TAPE-DESIGN.md` and implemented per its §10. | skinny/REDRESS.md Sonic Closeness analysis (lines 158-215); skinny/RESULTS.md NO-GO outcome G across all three corpora prior to lazy-tape implementation | Lazy-offset tape **REFUTED at outcome G post-implementation** (twitter T1 = 11780 Mbps < 13K threshold; `skinny/RESULTS.md:5-7`; `skinny/REDRESS.md` item 20). Eager-tape canonical stands; the V9.2 Lock 1 amendment is discarded per the conditional staging protocol. | Lock 1 unchanged. Substrate is not the bottleneck; the codegen template shape is. Surviving lever recorded at row below. **ANTI-MECHANICAL** under Lens N (clean lock-composition but failed empirical claim). |
| Four substrate-shape perturbations cluster-rejected: dispatch-table (REDRESS-17), 12-byte skipless token (REDRESS-18), pair-token fusion (REDRESS-16), lazy-offset tape (REDRESS-20). Every perturbation preserved correctness; none validated against the throughput gate. | skinny/REDRESS.md items 16-20; skinny/RESULTS.md outcome G three iterations | The eager 16-byte aligned `TapeToken` with stored sibling skip and explicit pair tokens is the canonical V1 substrate. No further perturbation candidates remain; further substrate-shape work requires a new architectural insight beyond what these four routes explored. | Lock 1 verbatim canonical; the four-perturbation cluster bounds the substrate-amendment search space empirically. **MECHANICAL-FROM-NEGATIVE-EVIDENCE** under Lens N. |
| Empirical hot-leaf-count asymmetry: skinny carries 5+ hot leaves at ≥10% self-time on twitter (`skinny/profile/PROFILE-REPORT.md`) while sonic-rs carries 1 (`skinny/profile/sonic-rs-v2/PROFILE-REPORT.md` confirms LTO fuses entire kernel into `parse_object`) and simdjson carries 2 (`skinny/profile/simdjson-v2/PROFILE-REPORT.md` stage1 + stage2). The asm dump (`skinny/profile/ASM-REPORT.md`) names the cause: `parse_value` does `skip_ws` and `peek` against raw bytes for every value boundary; the SIMD structural index is consulted only inside `parse_string`. | six-agent comparative-profile cohort 2026-05-12 | The architectural lever is the **structural-index-driven typed-parse template** (per `restart/skinny/audit/SOTA-BEAT-DESIGN.md` §2): generated `parse_*` consumes the offset stream via `cursor`, dispatching on `source[offsets[cursor]]`; whitespace and value boundaries never re-scan source. This is a codegen template change, not a substrate amendment. | Lock 1 unchanged; SUBSTRATE.md §1.6 captures the additive `ValueRef.cursor: u32` field; COMPILER.md §3.5 captures the normative codegen contract; new BIR variant `CursorDispatch` at ARCHITECTURE.md §7.2. **ADDITIVE-MECHANICAL** under Lens N. |
| Per-target SIMD primitive layer needed: NEON intrinsics on arm64 (host), AVX-512 VBMI2 on x86_64 (Intel Ice Lake+/Zen 4+), SWAR fallback for portability. Sonic-rs ships a minimal NEON surface (6 intrinsics); known-unused leverage identified by intrinsics agent (`skinny/profile/sonic-rs-v2/PROFILE-REPORT.md` + research catalogs): `vqtbl4q_u8` 4-table classifier, `vshrn_n_u16` interleaved-vector movemask (Validark 2024), `vld1q_u8_x4` quad-load on arm64; `_mm512_mask_compressstoreu_epi8` one-shot offset emission on AVX-512 (simdjson explicitly leaves this on the table at `icelake/simd.h:157`). | DAVID/asmjson research agent; handwritten ASM catalog agent; SIMD intrinsics agent | New crate `crates/bbnf-simd/` with per-target submodules (`aarch64/`, `x86_64/{avx512_vbmi2,avx2}/`, `scalar/swar_8byte`). CPUID dispatch at parser construction returns the right classifier; cost model (Lock 10) selects per grammar. Lock 16 (admissibility allowlist) governs admissible primitives + handwritten `asm!` blocks. | Lock 14 generalisation preserved (no grammar-specific code in `bbnf-simd`); Lock 16 new (admissibility allowlist with citations); see `SOTA-BEAT-DESIGN.md` §3 for full primitive table per architecture. **ADDITIVE-MECHANICAL with new Lock 16** under Lens N. |
