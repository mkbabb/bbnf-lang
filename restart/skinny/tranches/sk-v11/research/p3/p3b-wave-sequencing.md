...
The resulting bracket is 11 waves: W0, W1a, W1b, W2-W9. It stays inside the
<=12 skinny ceiling with exactly one spare slot for either a CHALLENGE-accepted
W8a source-last-mile split or one behavior-wave rescue; any second split
escalates instead of silently expanding scope (`SKINNY-TRIUMVIRATE.md:102-110`).
It consumes at most eight candidate surfaces: C1-C7 plus C8. C9 is carried in
every wave as output-plane accounting, not a row-moving candidate.
`HEX_QUARTET_X4_PROOF`, movemask, PMULL/CTZ, EOR3/BCAX, PRFM/STNP, and cache
hints remain support or inventory unless a later wave names a complete source
delta, scalar oracle, strict parity/checkasm, feature/fallback, same-wave
consumer, and measured row gate (`HANDOFF.md:118-127`,
`p2f-grammar-neutral.md:58-68`).
...
W1a and W1b are both CHALLENGE-gated because they establish the first non-JSON
gate/report and baseline authority.
...
| W1a | Non-JSON Gate/Report Schema Lane | C9 accounting; Lock 14 gate surface, not a parser row mover | `skinny/crates/bbnf-bench/src/*gate*`, report/render schema, non-JSON companion gate/report fixtures | W0 closes with gate-json consuming all SK-V11-open fields | Conditional on W0; blocks W1b | <=260 handwritten source/test/gate LOC; generated output 0 unless fixtures are named; <=90 min | Splits the first-of-class non-JSON work. W1a teaches the gate/report layer to reject missing non-JSON fields and producer-only telemetry without claiming a parser baseline. |
| W1b | Generated Non-JSON Baseline And Oracle Lane | C9 accounting; generated CSS/Sheets/BBNF-self baseline harness, not an intervention | `skinny/crates/codegen/src/*`, `skinny/crates/runtime/src/grammars/*`, `skinny/crates/bbnf-bench/src/*`, `grammar/css/l4/*` preferred, independent oracle/Track 2 | W1a closes; CHALLENGE selects one grammar/workload, preferably CSS L4 declaration values | Conditional on W1a; blocks W2 and later generic-crate primitive waves | <=360 handwritten source/test/gate LOC; regenerated output capped to the selected generated parser and named inputs; <=90 min | W1b creates exactly one generated non-JSON baseline row plus independent oracle. No behavior row admits; the baseline Mbps becomes W2's seed. |
| W2 | CSS L4 Generated Direct/Typed Intervention Proof | C1/C2/C4/C5/C6 as scalar-first generated-parser intervention; C7 support only | CSS L4 grammar templates, generated runtime, generated direct/typed benchmark, non-JSON oracle/Track 2, gate-json non-JSON consumer | W1b closes; CSS L4 declaration-values baseline exists; scalar references and throwaway microproofs are recorded | Conditional on W1b; blocks JSON generic primitive waves that touch `parse-that-regex`, `bbnf-simd`, or codegen-generic surfaces | <=430 handwritten source/test/gate LOC; regenerated output capped to named grammar/generator inputs; <=90 min | Non-JSON generality must be exercised before generic C1-C7 claims. W2 consumes the W1b baseline and cannot create the first measurable non-JSON row. |
| W3 | Numeric Direct Closure Slice | C4 digit span/accumulate; D4 number-slot emit shape; optional C1/C5 support | `skinny/crates/parse-that-regex/src/number/*`, `skinny/crates/bbnf-simd/src/aarch64/digit_mac.rs`, generated JSON direct/typed numeric callers, direct Track 2/oracle benches | W2 closes or rejects with non-JSON route preserved; numeric scalar oracle and same-host caller microbench pass | Conditional on W2 disposition; first JSON direct behavior wave | <=360 handwritten source/test/gate LOC; regenerated output capped to named JSON generated callers; <=90 min | Numeric/near-floor rows are low-order risk and give fast falsification. Redress selects one or two rows unless existing microbench data justifies more. |
| W4 | Generated Dispatch And Byte-Set Control Slice | C1 byte-set/class masks, C5 byte-set layout skip, C6 FIRST/prefix/lookahead, C7 movemask support; D1/D2 consumer shape | generated JSON direct dispatch, codegen lowering for `SinkOnly`, optional `bbnf-simd` TBL/TBX classifier only inside same-loop consumer, direct Track 2/oracle benches | W3 admits or rejects with REDRESS; W2 non-JSON proof remains valid for any generic edit; same-host classifier/dispatch microbench passes | Conditional on W3 disposition | <=430 handwritten source/test/gate LOC; regenerated output capped to named generator inputs; <=90 min | Dispatch/control follows numeric because it can affect more rows and must not be confused with W3 structural substrate. It consumes transient masks only in same-loop direct/typed callers. |
| W5 | Bounded String Span And Special-Byte Scan Slice | C2 bounded string scan; C7 movemask support; D3 borrowed string span | `skinny/crates/parse-that-regex/src/lib.rs`, `skinny/crates/bbnf-simd/src/aarch64/string_block.rs`, generated JSON direct/typed string callers, direct Track 2/oracle benches | W4 admits or rejects; caller microbench passes on named string rows; strict checkasm passes if any native body is used | Conditional on W4 disposition | <=360 handwritten source/test/gate LOC; regenerated output capped to named generated string/key callers; <=90 min | String scan work waits until dispatch/control settles. Redress selects one string/key consumer and at most two target rows, with `y_string_unicode` floor-bearing if selected. |
| W6 | Escaped Segment And Hex Decode Slice | C3 escape segment / hex decode; `HEX_QUARTET_X4_PROOF` support only if new segment source delta exists | `skinny/crates/parse-that-regex/src/lib.rs`, `skinny/crates/bbnf-simd/src/aarch64/unescape_uxxxx.rs`, generated direct/typed escaped-string callers, CSS/BBNF escaped-string or host hex consumer if used | W5 admits or rejects; new escaped-segment consumer is named; x4 scalar oracle/checkasm pass before any native body | Conditional on W5 disposition | <=360 handwritten source/test/gate LOC; regenerated output capped to named escaped-string callers; <=90 min | Escape work depends on the string span seam. It may not reuse the already-consuming `unescape_string` path as production; a real source delta and row gate must land same-wave. |
...
| W8 | Direct Residual Fixpoint And Row Reclamation | docs/gate/result accounting by default; no new primitive unless split as W8a | direct row benches, `skinny/RESULTS.md`, `skinny/REDRESS.md`, gate-json row-state updates | W3-W7 all admitted or rejected; W2 non-JSON intervention admitted or explicitly routed | Conditional on all behavior dispositions | <=250 docs/gate/result LOC; source work requires CHALLENGE-accepted W8a and consumes the only remaining spare bracket slot; <=90 min | This wave prevents paper close. Every remaining direct row either becomes `A / GO` under strict measured evidence or receives a per-row REDRESS uncloseable proof naming exhausted candidates. |
...
- W1a blocks W1b because non-JSON telemetry must be gate-consumed before a
  generated baseline row can become authority.
- W1b blocks W2 because the first non-JSON baseline and independent oracle must
  exist before an intervention can claim improvement.
...
| W1a | Non-JSON telemetry/gate harness exists and rejects missing grammar domain, comparator/oracle, output plane, Track 2/oracle, run id, feature mask, and same-wave consumer fields. No behavior row admission and no generated baseline authority. |
| W1b | Exactly one generated non-JSON direct/typed baseline row exists, preferably CSS L4 declaration values, with independent Track 2/oracle, strict semantic equality, run id/host/flags/sample evidence, and gate consumption. No intervention and no row admission. |
| W2 | CSS L4 declaration-values generated direct/typed row: W1b baseline Mbps is the seed; exit requires strict semantic equality, generated Track 1 and independent Track 2/oracle, at least `ceil(W1b_css_baseline_mbps * 1.01)` on one admitted non-JSON intervention row, and no named JSON guard regression beyond P3-C's maintain floor. If W1b cannot produce the baseline row, W2 is REVISE before redress. |
| W3 | Numeric/near-floor direct targets: `canada >= 10637`, `mesh >= 8675`, `numbers >= 2425`, `instruments >= 8969`; redress selects one or two rows unless existing microbench data proves all four fit. Optional numeric guard/target `marine_ik` direct stays admitted and typed numeric guards stay within maintain floors. |
...
| W5 | String-heavy direct targets: `twitter >= 13740`, `github_events >= 13403`, `update_center >= 10059`, `random >= 7878`, `gsoc-2018 >= 3737`, `distinct_values >= 2658`, `y_string_unicode >= 3950`; redress selects one string/key consumer and at most two target rows. Unicode rows are not admitted guards; `y_string_unicode` is a floor-bearing residual whenever selected. |
| W6 | Escape/Unicode direct targets: `unicode_escapes >= 3441`, `unicode_mixed >= 2588`, `y_string_unicode >= 3950`; optional CSS/BBNF escaped-string proof row uses W2/W1b non-JSON baseline formula. Existing `unescape_string` reuse alone is a gate failure. |
| W7 | Residual output-sink targets: only rows where a fresh post-W6 profile still names `output_digest_hash` as limiting, with the plan required to select a bounded row subset before redress. C8 cannot close a parser row by itself. |
...
| W1a | JSON-provider emission as non-JSON proof; hidden directives/BIR variants; producer-only non-JSON telemetry; generated baseline or intervention authority. |
| W1b | Old non-JSON hand runtimes as generated-parser proof; coupled oracle; behavior intervention or row admission during baseline creation. |
| W2 | Prose-only Lock 14 proof; generic CSS/Sheets/BBNF policy branches; non-JSON benchmark without generated Track 1 and independent Track 2/oracle; first-baseline creation in the intervention wave. |
...
| W9 | Close document drift, future-phase promises, or G-Alpha presentation while any W0, W1a, W1b, or W2-W8 wave lacks admitted/rejected/measured status. |
