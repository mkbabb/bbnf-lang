...
| A4 | Inter-document narrative coherence | PASS-3 says every perf gate names competitor, dataset, and platform per Lock 14 at `PASS-3.md:375`; the actual lock is Lock 8 at `restart/locks/14-LOCKS.md:48`. | REINVENT | Replace the lock citation with Lock 8 and a correct line target. |
...
| 2 | `restart/audit/pass-3-runtime/PASS-3.md:375` | Replace "Lock 14" / `restart/locks/14-LOCKS.md:207` with Lock 8 / `restart/locks/14-LOCKS.md:48` for competitor/dataset/platform evidence. | Citation check points to Lock 8 and the SOTA gate rows still pass command 9. | A4, H1 |
