...
| `json/direct_to_struct` (expanded workload gate) | sonic-rs serde struct direct rows in the same `skinny/RESULTS.md` run. | no `N-direct` rows; Track 1 and Track 2 direct are each within `sonic-rs * 1.10` in time. The first generated SinkOnly direct path is correctness-green and now passes only `numbers`; the remaining 16 rows require true BIR-lowered SinkOnly, decoded string delivery, exact float/string/Unicode materialization, and event-consumption work. | M5 Max macOS arm64 NEON. | H.W4, J.W1. |
