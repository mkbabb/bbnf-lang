...
static RUNTIME_PROFILE: GrammarProfile = GrammarProfile::new(
    ...
    RuntimeProvider::CssL4AtRulesAndMedia,
);
...
pub(crate) fn emit_runtime_files() -> BTreeMap<String, String> {
    BTreeMap::from([
        (
            "config.rs".into(),
            render(include_str!("css_l4_at_rules_and_media_templates/config.rs")),
        ),
        (
            "generated.rs".into(),
            render(include_str!(
                "css_l4_at_rules_and_media_templates/generated.rs"
            )),
        ),
        (
            "mod.rs".into(),
            render(include_str!("css_l4_at_rules_and_media_templates/mod.rs")),
        ),
        (
            "parser.rs".into(),
            render(include_str!("css_l4_at_rules_and_media_templates/parser.rs")),
        ),
        (
            "sink.rs".into(),
            render(include_str!("css_l4_at_rules_and_media_templates/sink.rs")),
        ),
    ])
}
