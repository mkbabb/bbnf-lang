# Alpha-F Contract Draft: SK-V8 to SK-V9

Date: 2026-05-18.

Scope: alpha-F draft only. This slice writes the SK-V9 Pass Alpha contract
surface:

- `restart/skinny/tranches/sk-v9/SYNTHESIS.md`
- `restart/skinny/tranches/sk-v9/HANDOFF.md`

It does not create `SPEC.md` or `DISPATCH-PROMPT.md`. V9 implementation is not
dispatched. After alpha challenge convergence the orchestrator must present
G-Alpha, and only after `G-Alpha closed` can skinny passes begin.
...
- W6 routed Apache/CITM measured row-table admission, retained class/event
  grammar plus `ValueRef` cursor proof, and direct output/control-path
  contracts to SK-V9 Alpha planning candidates.
- W6 routed SC-6-L1-R1 and broad lock/path cleanup to Pass Omega.

## 3. Contract Materialized

`SYNTHESIS.md` now carries:

- Section 0 alpha close condition and SK-V9 cycle close condition.
- Section 4.1 per-row goalset for current parse and direct misses, current GO row
  maintain floors, and typed Apache/CITM row-table candidates.
...
- Next move through alpha challenge, G-Alpha presentation, and only then skinny
  pass dispatch.
...
- any sentence dispatching SK-V9 implementation before G-Alpha;
- Apache/CITM presented as measured SK-V8 rows;
- W3 or W4 represented as admitted source work;
- `parse_only` treated as SOTA admission under the current borrowed-view /
...
After challenge convergence, present G-Alpha. Only after `G-Alpha closed` can
the skinny pass sequence begin, with downstream S-P3 responsible for the future
wave plan after its own entry conditions are met.
