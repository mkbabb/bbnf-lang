...
mod __csvparser_emit_impl {
    ...
    pub const GRAMMAR_CsvParser: [&'static str; 1usize] = [
        include_str!("/Users/mkbabb/Programming/bbnf-wt-b2-w1/grammar/misc/csv.bbnf"),
    ];
    ...
    /// `RuleKind` enum.
    #[inline(always)]
    fn project_rule_kind_CsvParser(variant_idx: u8) -> CsvParserRuleKind {
        match variant_idx {
            0u8 => CsvParserRuleKind::escaped,
            1u8 => CsvParserRuleKind::textdata,
            2u8 => CsvParserRuleKind::record,
    ...
    fn project_frame_CsvParser<'p>(
        ...
        match project_rule_kind_CsvParser(frame.variant_idx) {
            ...
            CsvParserRuleKind::record => {
                let mut children: ::std::vec::Vec<CsvParserValue<'p>> = ::std::vec::Vec::with_capacity(
                    frame.child_count as usize,
                );
                for (child_off, _child_frame) in output.value_children(offset) {
                    children.push(project_frame_CsvParser(output, input, child_off));
                }
