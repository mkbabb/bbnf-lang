...
| 15 | Fat-LTO `post-AZ-IV.json` carries floor + deltas + zero watchdog rows | ROUTED | W6.1 not landed in this wave window. Routed forward: post-AZ-IV measurement matrix opens as a follow-on benchmark dispatch under the W6.1 plan; AU floor anchored at `docs/benchmarks/post-AU.json`; AZ-III bench-iter anchored at `docs/benchmarks/post-AW-IV.json` and `docs/benchmarks/archive/AZ-III/`. Post-AZ-III deltas were not regressed into the AU floor by any W3/W4/W5 commit (no bench thresholds touched in any wave) |
| 16 | Same-harness sonic-rs floor (`bbnf_value_*` parity-or-better; `bbnf_get_*` ≤ 5x) | ROUTED | Same as Hard Gate 15: routed to W6.1 follow-on. The W3 cursor-threading + per-iteration decision consult ships the mechanism (`33184651` + `715747db`); the close-matrix measurement closing the floor is the routed deliverable |
...
| 6 | Cross-profile watchdog rows (fat-LTO + bench-iter) | ROUTED | W6.1 measurement matrix not landed in this authoring window. Routed forward to post-AZ-IV measurement cohort (5-sub-agent profiling per `docs/instructions/PROFILING.md`); zero watchdog rows is the close-matrix gate |
...
| F12 | bbnf-buddy (BC tranche) | Tracked in `docs/tranches/BC/`; SVG mascot + skeleton-animation per project memory | **post-AZ-IV** (BC tranche; opens after BA rule-discovery) |
...
Recycled BA opens after this close; BC follows BA per project memory.
