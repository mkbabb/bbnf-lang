...
| `alternate_fused_string_sink_plan` | Generated `SinkOnly` consumes raw string spans through `JsonSink::*_source`, but escaped strings are decoded, validated, and materialized into typed field facts inside parse-that/SinkOnly rather than by a sink-local hash helper. | Required before direct string/Unicode close, but the first SK-V6 family is rejected for the digest stressor. REDRESS item 49 rejects a generic no-allocation decoded visitor, item 54 rejects exact two-pass decoded stats in the sink, item 55 rejects a quote-source one-pass streaming hasher, item 66 rejects source-hook folding, item 67 rejects parser-owned decoded scratch, item 68 rejects byte-output `unescape_json_string`, and item 69 rejects semantic string facts as a `semantic_full_digest_stressor` close. The next admissible row is a real typed-output DirectBuild gate that proves owned typed structs first and keeps the digest stressor visible. | Tests whether direct `N-direct` string rows are materialization-bound rather than retained-view or codegen-authority bound, and separates a representative typed output from a maximal semantic digest stressor. |
