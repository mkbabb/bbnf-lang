...
fn validate_skv14_visible_admits(
    ...
    for line in results_text.lines() {
        ...
        {
            bail!(
                "SK-V14 visible JSON A/GO row lacks W9 typed, W10 parse, W11A direct, W11L/W11N/W11O token-product, or W11U raw-lexeme authority; {} {} remains admitted",
                cells[0],
                cells[1]
            );
        }
...
fn is_skv14_w8r_css_row(row_id: &str) -> bool {
    row_id
        .strip_prefix("css_l4/")
        .and_then(|tail| tail.strip_suffix("/direct_to_struct/main"))
        .is_some_and(|feature| SKV13_CSS_FEATURES.contains(&feature))
}
...
fn skv14_parse_only_admit_fields(row_id: &str) -> (&'static str, &'static str, &'static str) {
    ...
    } else if row_id == "json/instruments/parse_only/main" {
        (
            "SK-V14-W10T",
            "none:SK-V14-W10T-admit",
            "admitted:SK-V14-W10T-parse-only-open-sweep",
        )
    } else {
        (
            "SK-V14-W10",
...
fn validate_skv13_rolling_delta(results_text: &str, rolling_path: &Path) -> Result<()> {
    ...
    if !rolling_text.contains("run_id: SK-V13-open")
    ...
    for corpus in SKV13_JSON_CORPORA {
        for workload in SKV13_JSON_WORKLOADS {
            ...
            if row.tranche_admitted == "ADMITTED"
                ...
                && !SKV14_W11U_UNICODE_ESCAPES_TYPED_ROWS.contains(&row_id.as_str())
    ...
    for feature in SKV13_CSS_FEATURES {
        ...
        if row.plane != "css_l4_parity" && row.plane != "css_l4_full_parse" {
            bail!(
                "{row_id} rolling plane {} is not css_l4_parity or css_l4_full_parse",
                row.plane
            );
        }
        validate_rolling_status(row)?;
