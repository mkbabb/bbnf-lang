...
Downstream effect: W4 rejection blocks W5B-FRONTEND/W5C-GEN/W5D-DELETE/W6/W7
by the PRUNE chain and blocks W8 because CSS L4 re-admit requires the PRUNE
ledger at 0/24. W5B-FRONTEND inherits frontend/import/IR closure after W4 and
W5A admit. W5C-GEN inherits provider-free generator body replacement after
W5B-FRONTEND admits. W5D-DELETE inherits CSS provider/template deletion after
W5C-GEN admits. W9/W10 remain globally blocked until PRUNE-1..PRUNE-5 close.
...
- `skinny/crates/bbnf-bench/src/lock14_baseline.rs` (temporary W5A guard: provider/template deletion remains forbidden before W5D-DELETE)
...
- W5A plan does NOT delete CSS provider modules, CSS template directories, or `crates/core/src/runtime/` (W5D-DELETE and W6 scope).
...
6. Add a temporary W5A Lock 14 guard that forbids provider/template deletion until W5D-DELETE and rejects new provider/template directories.
...
- W5A closed at 921 C-1 part-A source/test LOC and cannot borrow from W5B-FRONTEND, W5C-GEN, W5D-DELETE, or W6.
...
(REDRESS 36-38, 85-86 watch-list); borrowing W5B-FRONTEND/W5C-GEN/W5D-DELETE/W6 budget to paper over a
...
- `skinny/crates/grammar/src/lib.rs` (generic frontend/import/IR closure; CSS L4 compatibility constructs lower into canonical IR, not new public syntax)
...
- Executable frontend construct coverage proves the CSS L4 compatibility constructs compile or lower through the request-owned generic frontend into canonical IR.
- `@ws` remains retired as a public directive and is lowered into the layout contract.
- JSON unchanged-output proof still holds.
- Sheets/BBNF-self proof remains fail-closed or generated-role-witnessed.
- Provider/template topology is unchanged from W5A:
...
proof, and the Lock 14 parent-diff unit test consume the W5B-FRONTEND closure in
the same commit.

Pre-blocked routes: provider/template deletion; provider-free generator-body
replacement before frontend closure; static centralization of hand-written CSS
runtime bodies; reading committed generated output as source truth; standalone
`@ws` public syntax; grammar-name branches in generic crates; borrowing W5C-GEN,
W5D-DELETE, or W6 budget.
...
## Section 8C — W5C-GEN PRUNE-3C (Provider-Free Runtime Generator Body)

Owner paths:

- `skinny/crates/codegen/src/lib.rs` (remove live provider-backed runtime generation dispatch)
- `skinny/crates/codegen/src/grammar_provider.rs` or successor neutral generator module named by the plan (provider-free generator body consuming W5A request facts plus W5B-FRONTEND IR)
- `skinny/xtask/src/main.rs`, `skinny/xtask/src/regen.rs`, and `skinny/xtask/src/regen_css.rs` (confirm `regen-css` consumes source + workspace metadata through the provider-free body)
- `skinny/crates/bbnf-bench/src/lock14_baseline.rs` (add W5C-GEN owner-path and parent-diff subject routing before source redress)
- `skinny/RESULTS.md` (W5C-GEN row attribution)
- `skinny/REDRESS.md` if rejected.

Entry gate:

- W5B-FRONTEND admitted.
- `skinny/crates/bbnf-bench/src/lock14_baseline.rs` has `SK_V14_W5C_GEN_OWNER_PATHS`, parent-diff subject routing for `sk-v14-waveW5C-GEN` / `sk-v14-waveW5C-GEN-redress`, and a unit test proving the roster admits only those paths before W5C-GEN source redress.
- W5C-GEN plan names the provider-free runtime generator body, the removal path for live `render_runtime_profile` / `RuntimeProvider` / `GrammarProfile` registry production dispatch, CSS/JSON source+metadata emission proof through W5B-FRONTEND IR, W5A/W5B-FRONTEND Sheets/BBNF-self proof carry, and provider-reachability grep gates.
- W5C-GEN plan explicitly forbids provider/template deletion; that deletion belongs to W5D-DELETE after the provider-free generator body admits.
- Any new neutral module path is forbidden until the plan names the exact path and the Lock 14 gate patch adds that path plus parent-diff unit test.

Tasks:

1. Add the Lock 14 owner-path and parent-diff gate patch for W5C-GEN before touching generator source owner paths.
2. Replace provider-backed runtime emission with one request/frontend-IR-driven provider-free generator path consuming grammar source + workspace metadata.
3. Ensure CSS L4 and JSON bytes come from grammar source + workspace metadata through W5A's request and W5B-FRONTEND IR, not providers/templates or committed generated output.
4. Retire live production dispatch dependency on `render_runtime_profile`, `RuntimeProvider`, `GrammarProfile` registry, `json_provider`, and `css_l4_*_provider`.
5. Run `cargo xtask regen-css`, all seven exact `check-css-l4-*` companions, `cargo xtask check-json`, and `cargo xtask gate-json --check-results --skv14-existing-results-capture`.
6. Preserve W5A/W5B-FRONTEND JSON unchanged-output proof and Sheets/BBNF-self fail-closed or generated-role proof.
7. Add a provider-reachability / generic-codegen grep that allows legacy provider/template files only as unused W5D-DELETE residue.
...
- `cargo xtask regen-css` remains byte-deterministic through the provider-free source/frontend-consuming body.
...
- JSON unchanged-output proof still holds.
- Sheets/BBNF-self proof remains fail-closed or generated-role-witnessed.
- `find skinny/crates/codegen/src -name '*_provider.rs' \! -name 'grammar_provider.rs' | wc -l` may remain non-zero in W5C-GEN, but those files are unreachable deletion residue owned by W5D-DELETE.
- Lock 14 parent-diff unit test covers W5C-GEN owner paths.
- Full-table maintain: ±1.0% on all rows.
- W5C-GEN source/test LOC cap is ≤1.0k C-1 part-A and cannot borrow from W5D-DELETE or W6.

Same-wave consumer: `cargo xtask regen-css`, the seven `check-css-l4-*`
companions, `cargo xtask check-json`, `cargo xtask gate-json`, provider-
reachability grep, no-grammar-name grep, and the Lock 14 parent-diff unit test
consume the W5C-GEN provider-free body in the same commit.

Pre-blocked routes: provider/template deletion before W5D-DELETE; static
...
grammar-name branches in generic crates; borrowing W5D-DELETE or W6 budget.

Revert protocol: revert provider-free generator body wiring plus Lock 14 gate
patch as one slice; preserve W5A's request-boundary work, W5B-FRONTEND closure,
and the old provider/template mesh; add REDRESS naming the failing source
construct, dispatch path, or provider reachability proof.

Downstream effect: W5C-GEN rejection blocks W5D-DELETE, W6, and W7. W8/W9/W10 remain
globally blocked until PRUNE-1 through PRUNE-5 close.

## Section 8D — W5D-DELETE PRUNE-3D (Provider/Template Deletion And Lock 14 Baseline Close)

Owner paths:

- `skinny/crates/codegen/src/{json,css_l4_*,bbnf,google_sheets,csv,ebnf,math,bnf,css_pretty}_provider.rs` except `grammar_provider.rs` (delete or retire old per-grammar provider mesh after W5C-GEN replacement is load-bearing)
- `skinny/crates/codegen/src/css_l4_*_templates/` (delete 7 CSS template directories)
- `skinny/crates/codegen/src/json_templates/` if W5C-GEN proves it is no longer production-consumed
...
- `skinny/xtask/src/regen.rs` and `skinny/xtask/src/regen_css.rs` (confirm `regen-css` consumes only the provider-free W5C-GEN path)
- `skinny/crates/bbnf-bench/src/lock14_baseline.rs` (close post-W5 forward invariant; add W5D-DELETE owner-path/parent-diff routing if W5C-GEN did not already make deletion paths executable)
- `skinny/RESULTS.md` (W5D-DELETE row attribution)
- `skinny/REDRESS.md` if rejected.

Entry gate:

- W5C-GEN admitted.
- W5D-DELETE plan proves all seven CSS L4 profiles and companions already pass through the W5C-GEN provider-free body.
- W5D-DELETE plan names the exact provider modules, template directories, old provider dispatch residue, Lock 14 baseline update, JSON unchanged-output proof, and Sheets/BBNF-self proof carry.
- If deletion paths are not already executable under the W5C-GEN Lock 14 gate patch, W5D-DELETE first adds its own owner-path and parent-diff subject routing before deleting providers or templates.
...
5. Run `cargo xtask check-json` and preserve Sheets/BBNF-self non-JSON proof from W5A/W5B-FRONTEND/W5C-GEN.
...
- Retired JSON template or grammar-profile registry residue is absent if W5C-GEN proves it is no longer production-consumed.
- Lock 14 baseline gate passes: grammar-name scan returns ZERO matches in generic crates per `LOCKS.md:220-238`.
- Generic production code contains no grammar-name dispatch arms:
  `cd skinny && ! rg -nU 'match\s+[^{]+\{[^}]*\b(Json|CssL4\w*|Bbnf\w*|GoogleSheets\w*)\b\s*=>' crates/{codegen,runtime,passes,bbnf,grammar}/src --glob '!**/tests/**'`.
- `cargo xtask regen-css` remains byte-deterministic through the W5C-GEN provider-free path.
- All seven exact `check-css-l4-*` companions pass.
- `cargo xtask check-json` passes.
- JSON unchanged-output proof still holds.
- Sheets/BBNF-self proof remains fail-closed or generated-role-witnessed.
- Full-table maintain: ±1.0% on all rows.
- W5D-DELETE source/test LOC cap is ≤400 C-1 part-A and cannot borrow from W6.

Same-wave consumer: `cargo xtask regen-css`, the seven `check-css-l4-*`
companions, `cargo xtask check-json`, and the Lock 14 baseline gate consume the
W5D-DELETE deletion in the same commit.

Pre-blocked routes: provider/template deletion before W5C-GEN admits;
...
boundary, W5B-FRONTEND closure, and W5C-GEN provider-free generator body; add
REDRESS naming the failing deleted surface or missing consumer.

Downstream effect: W5D-DELETE rejection blocks W6 and W7. W8/W9/W10 remain
globally blocked until PRUNE-1 through PRUNE-5 close.
...
- W5D-DELETE admitted (W5A source-consuming request boundary exists, W5B-FRONTEND frontend/import/IR closure is load-bearing, W5C-GEN provider-free generator body is load-bearing, and W5D-DELETE provider/template deletion plus Lock 14 baseline close has landed; provides the collapse target).
- W6 plan enumerates 9 sub-waves W6.0..W6.8 by grammar name: `css_l4, math, csv, bnf, ebnf, css_pretty, google_sheets, bbnf, json`. PRUNE-4 sub-wave count is 9 NOT 8 per S-P0 §2.3 (`css_pretty` is the +1 over the SK-V13 baseline's 8). Pass Omega V3 W2R assigns CSS L4 root-runtime collapse to W6.0 after W5D-DELETE.
...
1. Refactor the per-grammar runtime files under `crates/core/src/runtime/<G>/` into emitted output via the W5C-GEN provider-free generator body consuming `<G>.bbnf` + workspace metadata through W5B-FRONTEND IR.
...
- W5B-FRONTEND + W5C-GEN + W5D-DELETE + W6 admitted (C-1 PRUNE-3A + PRUNE-3B + PRUNE-3C + PRUNE-3D + PRUNE-4 BEFORE C-4 per S-P0 §2.2 sequencing constraint; the frontend closure, provider-free generator, deleted provider residue, and per-grammar runtime collapse must exist before W7 wires W8/W9 against the post-collapse mesh).
...
- `skinny/crates/codegen/src/` (emit parse_only path via the W5C-GEN provider-free generator body; per workspace metadata `parse_only` shape)
...
2. Emit the parse_only path via the W5C-GEN provider-free generator body + workspace metadata `parse_only=true` shape (per `[no-orthogonal-codepaths]` — one collection strategy).
...
- **P-6** — Per-grammar provider modules in generic codegen. SK-V14 PRUNE-3 splits into W5A source-consuming request boundary, W5B-FRONTEND frontend/import/IR closure, W5C-GEN provider-free generator body, and W5D-DELETE provider/template deletion; W5C-GEN proves ONE grammar-agnostic generator body consuming grammar source + workspace metadata through W5A's request and W5B-FRONTEND IR, and W5D-DELETE deletes provider/template clusters only after the replacement path is load-bearing.
