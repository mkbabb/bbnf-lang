...
/// Pre-compile the grammar once; returns the IR (needed for formatting) and the bytecode program.
...
fn aot_format_pathological(b: &mut Bencher) {
    ...
    b.bytes = PATHOLOGICAL.len() as u64;
    b.iter(|| {
        prettify_formula(black_box(PATHOLOGICAL), &config).unwrap();
    });
}
...
fn aot_format_1kb(b: &mut Bencher) {
    ...
    b.bytes = input.len() as u64;
    b.iter(|| {
        prettify_formula(black_box(&input), &config).unwrap();
    });
}
