...
The throughput evidence, using the authoritative `Delta vs sonic-strict`
column, is a substrate-guard signal rather than SOTA admission: bbnf is positive
on number-heavy corpora (`canada` +27.9%, `mesh` +21.4%, `marine_ik` +37.0%,
`numbers` +51.2% — corpora where structural density is low and the scalar
number scanner dominates) and loses string/structural-heavy corpora badly
(`gsoc-2018` -53.3%, `distinct_values` -61.2%, `y_string_unicode` -54.1%,
`apache_builds` -28.2% — corpora dense in `"` and `{}`/`[]`/`,`/`:` where the
per-byte structural rediscovery dominates). Larger simdjson DOM sidecar
magnitudes may explain planning pressure but are not strict admission evidence.
The split is the structural-byte-density axis. That is the substrate ceiling
...
| Generated Track 1 retained JSON parser (touched) | Templates: `skinny/crates/codegen/src/json_templates/generated.rs`, `skinny/crates/codegen/src/json_templates/parser.rs`, and `skinny/crates/codegen/src/json_templates/view.rs`. Generated output: `skinny/crates/runtime/src/grammars/json/scan.rs`, `skinny/crates/runtime/src/grammars/json/generated.rs`, `skinny/crates/runtime/src/grammars/json/parser.rs`, and `skinny/crates/runtime/src/grammars/json/view.rs`. | +60 templates / −30 regenerated | `cargo xtask check-json`; generated diff must contain only class-table emission, structural-index move, `consume_structural` deletion, and `json.fact.0` rewrite. | Retained JSON parse plane: all `json/<fixture>/track1_generated` strict rows remain valid; number-heavy guard rows `canada`, `mesh`, `numbers` no worse than -2.0%; structural-heavy rows `twitter`, `apache_builds`, `gsoc-2018`, `distinct_values`, `y_string_unicode` report the scalar rediscovery leaf removed. Any threshold copied from S-P2 must be recomputed from post-W0 same-run strict rows, not sidecar magnitudes. | This is the Tier A production consumer. It must validate UTF-8/control/escape work in the measured row; no `tape_vs_tape` substitute. | `cargo xtask check-json`, `cargo xtask check-conformance`, `cargo bench -p bbnf-bench --bench json_parity`, `cargo xtask gate-json --advisory` only for pre-W0 research telemetry; W3 admission must run the enforcing post-W0 `gate-json` path required by SPEC Section 0.4 / Section 6. | Revert codegen templates and regenerated JSON output together; never hand-patch generated files as the revert boundary. |
| Retained view / `ValueRef` (touched) | `skinny/crates/runtime/src/grammars/json/view.rs` and `skinny/crates/runtime/src/grammars/json/value.rs` generated from `skinny/crates/codegen/src/json_templates/view.rs` and `skinny/crates/codegen/src/json_templates/value.rs`; `skinny/crates/runtime/src/tape/mod.rs` | +20 templates/runtime | Covered by `cargo xtask check-json`; generated output must keep public view API stable. | Retained-view plane: `bbnf_bench::parity::assert_parity` token stream stays equal to Track 2 for all fixtures; payload counters remain reported. | Same retained `Tape` as parser; no view-owned cursor/fact cache. | `cargo xtask check-conformance`; `cargo bench -p bbnf-bench --bench json_parity -- json/twitter/track1_generated`. | Revert with Track 1 parser slice; a view-only fallback cursor is forbidden. |
...
The advisory gate commands above are pre-W0 research telemetry only. A W3
admission proof must run the enforcing post-W0 `gate-json` path from SPEC
Section 0.4 / Section 6 after W0 has installed the required schema and refusal
rules.
