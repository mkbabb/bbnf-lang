...
fn recover_binary_op<'a>(input: &'a str, lhs_end: u32, rhs_start: u32) -> Option<&'a str> {
    ...
    // Try two-character operators first so `<` / `>` don't shadow
    // `<<` / `>>`. `-` is sought via `find_unquoted` last because
    // it can appear inside a `->` map arrow which is NOT a
    // binary_factor operator.
    for &op in &["<<", ">>"] {
        if let Some(off) = super::find_unquoted(gap, op) {
            // No leading `<` / `>` before `op` (otherwise it would
            // be a longer operator that we already considered).
            let _ = off;
            return Some(op);
        }
    }
    // `-` is special: only treat it as a binary operator when it
    // is NOT immediately followed by `>` (which would form `->`,
    // the map arrow).
