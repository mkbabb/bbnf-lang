...
2. **The upstream BIR alphabet count is 19 variants** post Phase-8.4 α3 fold (`restart/audit/hardening/HARDENING-CONSOLIDATED-V8.md` §3 α3). The PASS-1 BIR base of 22 variants collapses three semantically-redundant pairs — `(DispatchAlt, SpeculativeAlt) → Alt { mode: Dispatch | Speculative }`, `(LayoutPush, LayoutPop) → LayoutScope { kind: Push | Pop }`, `(CallHost, HostChain) → CallHost` (chains express as `Seq`-of-`CallHost`) — landing on 19 variants. PASS-1 + ARCH §7.2 own the alphabet; PASS-2 cites without re-owning. PASS-2's payload-refiner table below carries the alphabet plus the BC additions (`Rule`, `Keyword`, `CharClass`, `MapExpr`, `FoldResult`, `EnumDiscriminator`, `Lookbehind`, `DebugMarker`); after the α3 collapse and `Lookbehind` addition the refiner table sits at 22 rows. PASS-2 keeps Unicode inside the regex-program payload carried by `RegexProgram`. The `RegexProgram` spelling is the canonical BIR alphabet entry per ARCH §7.2; it does not require full-DFA codegen for every regex (the payload is a regex-program contract; VM, lazy-DFA, full-DFA, and prefilter are all legal execution plans).
...
| 3 | `Alt { mode: Dispatch \| Speculative }` | dispatch keys, arms (Dispatch) or arms + checkpoint policy (Speculative) | `passes::extract` using `CostDecision` evidence from `cost-model` | match/PHF/scan dispatch (Dispatch) or cursor/tape checkpoint (Speculative) | same core | scaffold |
| 4 | `Repeat` | body, separator, bounds | rule lowering | loop with progress guard | same core | scaffold |
| 5 | `Optional` | body, absence policy | rule lowering | optional branch | same core | scaffold |
| 6 | `Ref` | target rule, args | rule lowering | rule call | same core | scaffold |
| 7 | `Lit` | bytes, case policy | rule lowering | byte compare | same core | scaffold |
| 8 | `Keyword` | keyword set, case policy | keyword detector | branch/PHF | same core | scaffold |
| 9 | `CharClass` | byte class | rule lowering | predicate | same core | scaffold |
| 10 | `Scanner` | scan plan | shape miner | scanner callback | scanner callback | scaffold |
| 11 | `RegexProgram` | regex program handle, execution plan, Unicode metadata, verifier contract | regex compiler | regex call or verifier | wasm-compatible regex call or verifier | scaffold |
| 12 | `Span` | span projection | rule lowering | span value | offset/len | scaffold |
| 13 | `Layout` | skip policy | `@layout` analysis | layout consume | same core | scaffold |
...
| 14 | `MapExpr` | projection expression | type/e-graph lower | typed conversion | same core | scaffold |
| 15 | `HostCall` | single host call (chains express as `Seq`-of-`CallHost`), generics, error policy | host inference | `host` registry call | primitive or extern import | scaffold |
| 16 | `FoldResult` | accumulator, fold op | rule lowering | accumulator state | same core | scaffold |
| 17 | `EnumDiscriminator` | enum arm/table | type lower | enum tag | numeric tag | scaffold |
| 18 | `PrattSpine` | operator table, associativity | Pratt detector | Pratt loop | same core | scaffold |
| 19 | `SimdScan` | structural alphabet, kernel shape | SIMD detector | `simd-scan` call | wasm/scalar scan | scaffold |
| 20 | `Lookbehind` | bounded predicate | lookbehind analysis | reverse predicate | reverse predicate | scaffold |
| 21 | `ErrorRecovery` | strategy, diagnostic | `@error` analysis | diagnostic/recovery edge | same core | scaffold |
| 22 | `DebugMarker` | source map marker | debug config | optional metadata | optional metadata | scaffold |
...
Cardinality defence: BC's research anchors put the useful compiler-IR band at roughly 20-30 variants, comparing MLIR `arith` at 60, Cranelift `InstructionData` at 40, rustc HIR `ExprKind` at 35, rustc HIR `ItemKind` at 16, and chalk `TyKind` at 23 (`docs/tranches/BC/audit/research-anchors.md:12-18`, `docs/tranches/BC/audit/W0-typed-ir-variant-table.md:319-329`). The upstream 19-variant alphabet (post Phase-8.4 α3 fold) sits well inside that band; PASS-2's 22-row payload-refiner table (alphabet plus the BC additions `Rule`, `Keyword`, `CharClass`, `MapExpr`, `FoldResult`, `EnumDiscriminator`, `Lookbehind`, `DebugMarker`) likewise stays inside the band. swc is kept as backend-separation inheritance rather than a cardinality bound because local README cites swc for WASM/codegen pipeline shape (`restart/README.md:369`); swc compiles JavaScript AST into per-domain enums (`Stmt` / `Expr`) with different cardinality pressure than parser IR (`restart/corpora/SOTA.md:186` — parol's typed-AST cardinality reference is the closest auditable corpus line for the AST-cardinality argument; the swc rustdoc URL citation is retired in favour of corpus path:line discipline).
...
|---|---|
| Payload field widths, alignment, and packing for each variant. | The 19-variant upstream alphabet itself (post Phase-8.4 α3 fold); new variants and retirements return to PASS-1. |
...
|---|---|---|---|
| BC.W0 | Typed IR boundary and variant-table discipline (`docs/tranches/BC/waves/W0.md:10-28`). | Expand final BIR to the 19-variant upstream alphabet (post Phase-8.4 α3) plus PASS-2's 22-row payload-refiner table including Tape shapes. | 7-variant placeholder. |
...
1. Implement `ir::backend_ir` with the 19-variant upstream alphabet (post Phase-8.4 α3) projected through PASS-2's 22-row payload-refiner table and the snapshot printer.
...
| α3 BIR alphabet 22 → 19 (three pair-collapses) | architectural cardinality | ACCEPT | The three pairs per PASS-1 V8 surfacing: (`AltDispatch`, `AltSpeculative`) → `Alt { mode }`; (`LayoutPush`, `LayoutPop`) → `LayoutScope { kind }`; (`CallHost`, `HostChain`) → `CallHost` (chains express as `Seq`-of-`CallHost`). PASS-2's existing table at lines 50-79 already collapses Layout (single variant) and HostCall (chains folded); only the Alt collapse applies on PASS-2's refiner table. Authoritative alphabet count (post-fold) = 19; PASS-2 cites the upstream count without re-owning. |
...
| ε1 23 vs 24 alphabet count reconciliation | hygiene | SUPERSEDED-BY-α3 | The 23-vs-24 reconciliation dissolves under α3 because both sides converge on the post-fold 19-variant alphabet (PASS-2's `Layout` collapse + ARCH's `LayoutScope` collapse name the same single variant). |
