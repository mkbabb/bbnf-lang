...
## Architecture state (post-Tranche O–T)
...
(`crates/core/src/pipeline/compile.rs:430`), enforced by a
...
│   ├── egraph/                         egraph — general-purpose e-graph substrate (shared by ir + bbnf-regex)
...
parse_that  (re-exports bbnf-regex)
   ↑
egraph ──────────────────────────────────────── general-purpose e-graph substrate
   ↑        (consumed by bbnf-ir grammar tier + bbnf-regex HIR tier)
egraph-derive  (proc-macro: #[derive(Language)])
   ↑
...
**Leaf crates** (`bbnf-ser`, `bbnf-regex`, `egraph`) have zero bbnf dependencies.
...
- **sp_method eligibility**: `bbnf_ir::passes::span::compute_sp_method_rules` iteratively decides which rules project to a zero-alloc Span method. The Rust emitter consults `has_sp_method` on the rule meta to emit an `__rule_span(state)` entry point alongside the slab constructor. Literal unescape via `unescape_literal()` + `proc_macro2::Literal::string()`.
- **Codegen**: Grammar → `lower/` (`mod.rs`, `string_interner.rs`, `fn_table.rs`, `expression.rs`, `value_expr.rs`, `metadata.rs`) → bbnf-ir `Module` → `backend/` (`emitter.rs` trait + per-target `rust/`, `ts/`, `wasm/` emitters, shared `driver/`, `patterns/`, `prettify/`, `strategy/` sub-modules). `pipeline/compile.rs` orchestrates the full lowering + emission sequence.
...
- **Single pipeline entry point**: `crates/core/src/pipeline/compile.rs` owns the full IR pass ordering. `bbnf-derive` consumes it via `bbnf::pipeline::compile_paths_request`, so proc-macro and library builds cannot drift. Notable ordering: `sort_alt_branches` → `refine_span_eligibility` → `compute_scc` → `compute_follow_sets` → `factor_regex_with_lookahead` → `fuse_token_dispatch` → `generate_dispatch_tables` → `compute_regex_info` → `recognize_patterns`.
...
- **Prettify codegen**: `@pretty` directives control Doc emission. Hint vocabulary: `group`, `indent`, `dedent`, `block`, `blankline`, `nobreak`, `softbreak`, `hardbreak`, `compact`, `fast`, `off`. The `emit_prettify_*` methods on `RustEmitter` (defined across `backend/rust/emitter/prettify/{alt,attempt,grammar,literal,repeat,seq}.rs`) produce `to_doc()` + `source_range()` impls, driven by the shared policy types in `backend/prettify/`. Sub-variant coercion handles heterogeneous alternation branches. Shared hint definitions in `analysis/src/directives/hints.rs` — single source of truth for codegen + LSP.
...
- **Monolithic codegen**: `backend/rust/` is the sole Rust codegen path. Generates direct recursive functions with slab allocation—zero combinator overhead. The heavy inline trait-method bodies were extracted from `backend/rust/emitter/mod.rs` into per-kind siblings (`leaves.rs`, `seq.rs`, `binary.rs`, `operator_chain.rs`, `map_value.rs`, `grammar.rs`); mod.rs retains a single `impl Emitter` block that delegates. `MonoCtx` tracks fusion eligibility, single-site inline eligibility, dispatch-guaranteed-byte, `current_rule_name`, and hoisted leaf-parser bindings. Unified `SepByConfig` + `emit_mono_sep_by_core` handles all three sep_by variants (bare, ws-aware, delimited-with-terminator). Optional `backend/rust/emitter/prettify/` sub-path emits fused parse+format for `#[parser(prettify)]`.
- **Span-only codegen**: Triggered by `#[parser(span)]` and by per-rule `sp_method` eligibility. The Rust emitter generates `fn __rule_span(state) -> Option<Span>` alongside the slab constructor—zero allocations, no enum variants, no Vec.
- **Inline optional Span codegen**: Optional Span expressions (Literal/Regex with fast-path) in the Rust emitter's `repeat.rs` sibling emit inline byte checks instead of constructing a SpanParser per call.
- **Delimiter-driven flat scanning**: `backend/patterns/delim_scan.rs`. Grammar-agnostic codegen optimization for `Wrap(Repeat(Alt))` patterns where the Alt's FIRST sets overlap. Detects single-byte "pivot" Literals that distinguish branches, emits a forward `memchr` scanner loop. Span path replaces the descent entirely; slab path uses speculative dispatch (scan selects branch, then calls existing recursive descent for typed construction). Pseudo-class guard: if value after pivot terminates at the open byte, falls back to block branch. When the pivot rule returns Span (`is_token` or `TypeDesc::Span`), constructs the result directly from scanner offsets, eliminating speculative rewind + re-parse.
- **`factor_regex_with_lookahead` pass**: `ir/src/passes/sets/factor_lookahead.rs`. Detects Alt branches with overlapping regex FIRST sets but disjoint continuation FIRST sets. Factors common prefix and builds dispatch table on continuation. Runs after `compute_follow_sets`, before `generate_dispatch_tables`.
- **Direct memchr emission**: `generate::regex::emit::emit_regex_direct_call` handles negated character class patterns `[^XYZ]+` and `[^XYZ]*`, emitting inline `memchr::memchr1/2/3` calls that bypass SpanParser enum dispatch. Also handles positive character classes (`[a-z]`, `[abc]`) and their repetitions (`[a-z]+`, `[a-z]*`) via direct inline byte-range/set checks.
...
- **`@ws` directive**: `@ws /regex/ ;` overrides what `?w` compiles to. Stored as `GrammarIR::ws_pattern: Option<StringId>`. Codegen uses `emit_ws_trim()` which checks `ir.ws_pattern` and emits `generate::regex::emit::emit_regex_direct_call` for known SIMD fast paths (e.g., CSS comment-aware whitespace → `css_ws_comment_fast`).
...
- **`@debug` directive**: `@debug ruleName ;` / `@debug * ;` instruments rules for trace output. Stored as `RuleMeta::debug: bool` and `GrammarIR::debug_all: bool`. Trace codegen: `backend/rust/trace.rs` emits `#[cfg(feature = "parser-trace")]` calls for monolithic paths.
...
- **IR-backed LSP analysis**: `try_compile_ir()` in `analysis/src/state/diagnostics/ir_analysis.rs` runs the full IR pipeline per document change, extracts `IrRuleMeta` (FOLLOW sets, dispatch, memo, span eligibility, projected type) for hover enrichment.
...
- **Map-transparent all-literal detection**: `backend/driver/alt.rs` + the per-emitter `alt.rs` siblings see through `Map(Literal, Constant)` wrappers. An alternation of mapped literals (`"px" -> 0u8 | "em" -> 1u8`) uses the sequential literal fast path — byte-compare first, then emit the constant.
- **Regex structural classifier**: `bbnf-regex::classify` (imported as `parse_that::regex::classify`) uses the bespoke HIR for structural classification — detects Numeric/HexDigits/Identifier/QuotedString patterns. Drives `FnDescriptor` specialization in `try_specialize_map_fn` and inline scanner selection in `generate/regex/emit/`.
- **Inline byte scanners**: `generate/regex/emit/` emits direct inline code for patterns beyond JSON: `css_ident_fast` for `[a-zA-Z_][\w-]*`, `css_string_fast` for quoted strings, `css_ws_comment_fast` for comment-aware whitespace, comma-or-whitespace `,|\s+`, and generalized char-class/negated-class loops via `emit_generalized_regex_direct`.
