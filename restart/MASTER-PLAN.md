...
| `json/twitter` + `json/random` + `json/unicode_mixed` + `json/unicode_basic` (current expanded SOTA-BEAT parse blockers) | sonic-rs / simdjson per-corpus M5 Max anchors in `skinny/RESULTS.md` and sidecar profiles. | `parse_value_at` < 20% self-time on every row; outcome-G eliminated. | M5 Max macOS arm64 NEON. | H.W1 (typed event cursor) + H.W2 (Class A/B NEON), J.W1. |
| `json/direct_to_struct` (expanded workload gate) | sonic-rs serde struct direct rows in the same `skinny/RESULTS.md` run. | no `N-direct` rows; Track 1 and Track 2 direct are each within `sonic-rs * 1.10` in time. The first sink-only digest parser is landed and now passes 6 of 17 rows after the UTF-validation and integer-classification redress, so H.W4 owns exact float/string/Unicode materialization inside `SinkOnly` for the remaining 11 rows. | M5 Max macOS arm64 NEON. | H.W4, J.W1. |
