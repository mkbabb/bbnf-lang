...
Date: 2026-05-14 (post-SK-V5 implementation redress)
Status: **SK-V5 IMPLEMENTATION IN PROGRESS. Current measured authority is `skinny/RESULTS.md`: full gate `N-direct / NoGo`; retained parse has 13 G rows plus one stale Canada L row from the prior structural-scan floor miss; only `mesh`, `marine_ik`, and `numbers` are retained A / GO; direct correctness is green but only `numbers` passes the sonic-rs 1.10x direct slack. SK-V5 redress item 56 restores the focused Canada structural-only scan to about 41833 Mbps against the 40000 Mbps NEON floor; full `bench-json` / `gate-json` still must refresh the stale L row. Strictness/output-plane columns are disclosed. Waves 0-5 are implemented at the Rust-state level with measured redress; the remaining close is retained G rows plus `N-direct`, not codegen-authority absence.**
...
12. `skinny/RESULTS.md` — current measured skinny gate: original triad passes historically; expanded parse has 13 G rows plus one stale Canada L row pending item-56 refresh; direct workload is correctness-green but `N-direct / NoGo`; strictness/output-plane columns are disclosed.
13. `skinny/REDRESS.md` — accepted wins and rejected-route ledger; item 56 records the admitted structural-scan floor fix, while items 49, 54, and 55 reject the three decoded-string hash/materializer routes.
...
| Skinny expanded gate | The same implementation measured across 17 rows, including Unicode and shape-stress corpora | Checked-in full report has 13 parse/tape G rows plus one stale Canada L row; item 56 restores the focused Canada structural floor; overall gate remains `N-direct / NoGo` because direct throughput is red on 16 rows |
...
**Current operating verdict: `skinny/RESULTS.md` is the measured authority for the runnable skinny, amended by `skinny/REDRESS.md` item 56 for the focused scan-floor row.** It records full-gate **`N-direct / NoGo`**. The original triad still passes as historical substrate evidence. The checked-in expanded parse/tape plane has 13 hard G rows plus one stale Canada L row; the focused item-56 redress restores Canada structural-only scan to about 41833 Mbps, so the live parse blocker is event/tape consumption plus string/Unicode projection rather than the Canada scanner floor. The direct workload is correctness-green with generated Track 1 `SinkOnly` and structurally different hand Track 2, but only `numbers` passes the 1.10 sonic-rs time slack; the other 16 rows remain the binding direct-typed-emission block. Strictness and output-plane columns are disclosed.
...
- Lazy-offset tape with sparse flags is the measured JSON skinny substrate for the historical triad; focused item-56 scan-floor redress restores Canada structural scan; expanded SOTA-BEAT remains blocked on retained G rows plus `N-direct`.
...
| 3 | Wave 2 | Number lever + generated SinkOnly + bench rewire + bench-private SinkParser nuke | `parse-that-regex/src/number/`, `codegen/src/lower/sink_only.rs`, `runtime/src/grammars/json/sink.rs`, `bbnf-bench/src/direct_struct.rs` | Track 1 calls generated runtime; Track 2 is structurally different; `numbers` direct passes after later redress, while canada/mesh/marine_ik remain near-miss direct rows | SK-V5 packet §4 |
| 4 | Wave 3 | UTF-8 fusion + Class B `_x4` batched + utf8_block module | `parse-that-regex/src/lib.rs:331-339`, `parse-that-regex/src/unicode/`, `bbnf-simd/src/aarch64/utf8/`, `bbnf-simd/src/aarch64/unescape_uxxxx.rs` | duplicate UTF-8 validation removed and source hooks admitted; parse-G and string-bound direct rows remain open | SK-V5 packet §5 |
...
**Implementation discipline**: dispatch order is sequential. The M5 Max close
condition has not fired: parse-G rows and `N-direct` rows remain. Wave 6 is the
next reporting/full-matrix surface, and further implementation must remove the
remaining retained and direct rows rather than claim durability on top of a
measured win. Wave 7 is the x86 asmjson-beat successor — conditional on Zen 4
silicon access AND declared NASM author AND checkasm-green Layer 1; it cannot
be used to close the M5 Max SK-V5 gate.
...
Hereupon the SK-V3 and SK-V4 grand syntheses are historical, not sufficient for dispatch. The SK-V5 cohort plus implementation redress ledger is the receiver authority. The skinny JSON triad is green as historical evidence; the checked-in expanded parse plane has 13 G rows plus one stale Canada L row now corrected in the focused scan-floor row, and the direct workload is `N-direct / NoGo` with only `numbers` passing. Waves 0-5 have landed enough Rust state to close strictness disclosure, BackendShape/codegen authority, generated `SinkOnly`, trusted UTF-8 source hooks, Lock 14 cleanup, consumed primitive admission, and the Canada scan-floor redress. The next close is not another directive or side substrate: it is full matrix refresh plus retained event/tape consumption and direct field-layout materialization that remove the remaining G and `N-direct` rows.
