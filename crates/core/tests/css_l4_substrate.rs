...
fn struct_builder_assembles_simple_stylesheet() {
    ...
    let sheet_layout = layout_for(149, "ruleList", LayoutKind::Struct);
    let style_layout = layout_for(144, "qualifiedRule", LayoutKind::Struct);
    // `declaration` (the Alt dispatch wrapper) no longer opens a
    // Declaration frame; the concrete declaration frame is opened by the
    // matched leaf rule (here `genericDecl`). Routing the wrapper *and*
    // the leaf both to a Declaration frame double-counted every
    // declaration, so only the leaf rule opens one.
    let decl_layout = layout_for(64, "genericDecl", LayoutKind::Struct);
...
fn struct_builder_assembles_typed_dimension() {
    let mut builder = CssStructBuilder::<'static>::new();
    let length_layout = layout_for(84, "length", LayoutKind::Struct);
...
fn struct_builder_threads_color_function() {
    let mut builder = CssStructBuilder::<'static>::new();
    let layout = layout_for(87, "colorFn", LayoutKind::Struct);
...
fn document_get_path_query_resolves_string() {
    ...
    let mut builder = CssStructBuilder::<'static>::new();
    let sheet = layout_for(149, "ruleList", LayoutKind::Struct);
    let style = layout_for(144, "qualifiedRule", LayoutKind::Struct);
    let decl = layout_for(64, "genericDecl", LayoutKind::Struct);
