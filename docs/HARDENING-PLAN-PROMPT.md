...
## Gestalt — twelve locks

The plan must reflect these twelve architectural commitments faithfully. Any wave that violates one is a fault.

1. **Tape and its columnar variants are fully dead**. No tape crate, no `TapeRec`, no `TapeCursor`, no kind-partitioned columnar SoA, no variant-strip arenas, no "tape rebranded as fast-path". The substrate IS the typed-enum + slice-borrow (per Lock 9), with bumpalo and owned as opt-in escape hatches. Direct-to-struct is *both* the visible API *and* the underlying memory layout — sonic-rs / lightning-css discipline. The columnar approach was tried at Tranche Y (worked at AU peak; reverted at AY-I.W1) and again at AV's `04-columnar-soa.md` (designed; never fully activated; joined Era V's substrate-first/consumer-later dead pile). sonic-rs at 5 GB/s on twitter beats AU's tape-era 1.97 GB/s by 2.5×; direct-to-struct without tape is the demonstrably faster path. Plans that resurrect tape, columnar SoA, variant-strip arenas, or any substrate the typed enum does not directly express are faults.
