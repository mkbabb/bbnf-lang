...
fn main() {
    ...
    let arena = BumpArena::<CssPrettyParserEnum<'_>>::with_capacity(64);

    let (r, s) =
        CssPrettyParser::selectorSpan().parse_return_state_with_context(input, &arena);
    ...
    let (r, s) =
        CssPrettyParser::qualifiedRule().parse_return_state_with_context(input, &arena);
    eprintln!("qualifiedRule: ok={} offset={}", r.is_some(), s.offset);

    let (r, s) = CssPrettyParser::ruleItem().parse_return_state_with_context(input, &arena);
    eprintln!("ruleItem: ok={} offset={}", r.is_some(), s.offset);

    let (r, s) = CssPrettyParser::ruleList().parse_return_state_with_context(input, &arena);
    eprintln!("ruleList: ok={} offset={}", r.is_some(), s.offset);

    let (r, s) = CssPrettyParser::stylesheet().parse_return_state_with_context(input, &arena);
    eprintln!(
