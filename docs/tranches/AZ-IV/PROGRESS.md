...
| W3 - Lazy Bail-Out Parse | in_progress | `audit/W3-*.{txt,md}` | W3.0 path-egraph-seed landed (commits `c727df9e`, `d186efcc`); 3 hand-authored rewrites (DuplicatePrefixElimination, RedundantDowncastRemoval, AdjacentAccessorFusion) with 6 fixture tests, encoded through `Pattern` named atoms (bbnf-ir cannot import bbnf-core; cycle direction). W3.1 (path executor + cursor + schema) + W3.3 (codegen path-plan emitter) under integration; W3.2/W3.4/W3.5 sequenced after Phase-1 cherry-pick. |
