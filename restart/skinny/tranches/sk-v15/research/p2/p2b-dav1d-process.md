...
| `EOB_PAD_CLAMP` | Scalar zero-pad/clamp block under `scalar::eob_pad_clamp`. | Current path may delegate; hand ASM would require missing-intrinsic proof. | `checkasm_eob_pad_clamp` and primitive-checkasm floor. | Existing JSON scan tail handling. | Tail handling for `scan/string` or structural scanner rows. | No specific block, but it cannot become an auxiliary sidecar or retained cursor under Lock 1. |
