...
- **IR-based codegen**: Grammar → `lower.rs` → bbnf-ir `Module` → `ir_codegen/` (6 sub-modules: `mod.rs`, `alt.rs`, `seq.rs`, `repeat.rs`, `wrap.rs`, `infer.rs`) + `ir_enums.rs`/`ir_types.rs`/`ir_span.rs`/`ir_pretty.rs`. Old direct-AST codegen (`codegen.rs`, `alternation.rs`, `concatenation.rs`, `patterns.rs`, `type_inference.rs`) removed. `pipeline.rs` orchestrates the full lowering + codegen sequence.
