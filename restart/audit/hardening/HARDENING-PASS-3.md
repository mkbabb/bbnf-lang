...
| Required context read | `restart/README.md`, `restart/locks/14-LOCKS.md`, precepts, PASS prompts, corpora, inheritance index, PASS-3 outputs |
...
| `restart/audit/pass-3-runtime/PASS-3.md:293`; `restart/locks/14-LOCKS.md:48` | Lock citation | PASS-3 cites `restart/locks/14-LOCKS.md:207` for competitor baselines. | The intent is right. | Line 207 is a failure-mode clause in the lock file, not Lock 8. | Replace with `restart/locks/14-LOCKS.md:48` and README/corpus line citations. | REINVENT |
...
| `agent-5-error-recovery-incremental-parsing.md:61-62`; `restart/locks/14-LOCKS.md:36` | Layout-lowering error | Existing message is about unused formatter metadata. | It catches a real doc/formatter case. | It does not cover "rule X has no resolvable layout because Y." | Add `error[BBNF-LAYOUT002]: rule \`X\` has no resolvable layout; reason: ...`. | REINVENT |
