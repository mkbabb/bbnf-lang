...
fn validate_skv14_manifest_rows(rows: &[SkV14ManifestRow]) -> Result<(), String> {
    ...
    if pending != 25 || falsified + sustained != 50 {
        return Err(format!(
            "SK-V14 audit overlay expected pending=25 and falsified+sustained=50 after authorized W9/W10/W10R/W10S admits, saw {falsified} / {pending} / {sustained}"
        ));
    }
    Ok(())
}
...
fn validate_native_comparator_source(
    ...
    if workload == "parse_only" {
        let mode = match comparator_id {
            "sonic_rs_strict" => "parse_only_sonic",
