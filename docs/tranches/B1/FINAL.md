...
| 3 | Every public routine command is seconds-scale on the declared routine surface or removed | green | `docs/benchmarks/archive/post-B1-W0-routine.txt` rows: `iter-check` 3.88 s, `iter-check-lsp` 3.27 s, `iter-check-prettify` 33.36 s warm; `iter-check-bootstrap` AZ-I-routed (not on routine surface). |
...
| 6 | No stale `#[allow(dead_code)]`, `#[ignore]`, `#[cfg(false)]`, dead workflow prose introduced | green | W2.b ledger deletes 5 ABROGATE scripts in the same commit as their replacement aliases; `docs/benchmarks/archive/post-B1-W2-abrogation-ledger.txt`. |
