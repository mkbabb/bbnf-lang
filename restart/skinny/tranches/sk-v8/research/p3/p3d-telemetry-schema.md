...
Output: this file.
Pass Alpha goalset: SK-V8 must create `SK-V8-open`, populate required telemetry on all 38 current main rows, make `gate-json` reject missing telemetry, bind CostFacts before behavior admission, preserve typed GO rows, keep direct digest rows as guard rows, and pass Lock 14/Lock 15 at close (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/HANDOFF.md`).
Candidate pool: S-P2 substrate-ceiling survivors after V7 consolidation; V7 accepted 6/6 and authorizes S-P3, not any implementation wave (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/hardening/HARDENING-S-P2-V7-CONSOLIDATED.md`).

## §1 - Synthesis

P3-D is a gate-binding plan, not a source plan. S-P3 is read-only against `skinny/` source, and implementation belongs only to later wave redress phases (`restart/prompts/skinny/PASS-3-SYNTHESIS-PLAN.md`). This artifact therefore names schema requirements, allowed values, row bindings, fixtures, and failure states only.

The current authority is schema-v3 `skinny/RESULTS.md`: 38 main rows, current overall `N-direct / NoGo`, same-run sonic strict/lossy provenance, sidecar C++ values only where populated, and missing hot-leaf attribution (`skinny/RESULTS.md`, `skinny/RESULTS.md`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md`). The live rendered header has 26 columns, even though the P3 prompt shorthand says "24-column schema"; this plan preserves the live 26-column header and treats the prompt wording as shorthand for the schema-v3 surface (`skinny/RESULTS.md`, `restart/prompts/skinny/PASS-3-SYNTHESIS-PLAN.md`).

The schema has two jobs before behavior waves can dispatch. First, W0 must make every current row observably tied to a run, profile artifact, host/build metadata, comparator plane, strictness, sidecar freshness, and `SK-V8-open` delta (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/SPEC.md`). Second, W1 must make CostFacts chosen/rejected evidence consumed by `gate-json --with-cost-facts`, without changing parser behavior or generated JSON output unless a later plan explicitly owns that behavior (`restart/skinny/tranches/sk-v8/SPEC.md`).

The W3 tape plus structural-projection union needs extra observability but no new directive, BIR variant, or substrate. S-P2 preserves the union as a lead hypothesis only: Tier A is structural-class cursor migration inside the single retained `Tape`; Tier B owns string-boundary, quote/backslash/parity, CostFacts-template, and non-JSON production migration (`restart/skinny/tranches/sk-v8/SYNTHESIS.md`). W3 remains blocked until W0/W1 close and a fresh plan names owner paths, same-wave production consumer, revert protocol, thresholds, measured-path strict validation, and challenge acceptance (`restart/skinny/tranches/sk-v8/HANDOFF.md`, `restart/skinny/tranches/sk-v8/SPEC.md`).
...
| `Workload` | existing | Current main: `parse_only`, `direct_to_struct`, `real_typed_struct`; routed residual only: `tape_vs_tape`; non-main probe: `simd_structural_scan` | `tape_vs_tape` is not default W0/W1 scope and cannot satisfy W3 production-consumer proof (`restart/skinny/tranches/sk-v8/SPEC.md`). |
| `Outcome` | existing / SPEC 0.3 | `A`, `C`, `G`, `K`, `L`, `N-direct`; optional post-W0 amendment `S` | Reject any other outcome after W0; `K` and `S` never support strict SOTA admission (`restart/skinny/tranches/sk-v8/SPEC.md`). |
...
| `Strictness` | existing | `strict`, `deferred`, `permissive`, `lossy`, `n/a` | Strict admission requires `strict`; current rows are `deferred` (`skinny/RESULTS.md`). |
...
| `row_output_plane` / `Output plane` | normalized W0 addition over existing prose | `borrowed_view_offset_tape`, `dom_value_tree`, `digest`, `typed_direct`, `offset_index`, `structural_index`, `sink_only_transient`, `n/a` | Required comparator-plane equality key. Current parse rows are borrowed view over offset tape vs DOM; direct rows are digest; real typed rows are typed direct (`skinny/RESULTS.md`). |
| `Track 1 Mbps`, `Track 2 Mbps` | existing | Positive numeric Mbps or `n/a` only when workload has no track | W0 telemetry-only rows must remain within +/-1.0 percent versus `SK-V8-open` (`restart/skinny/tranches/sk-v8/SPEC.md`). |
...
| `comparator_plane` | SPEC 0.4 | `dom_value_tree`, `digest`, `typed_direct`, `offset_index`, `structural_index`, `lazy_iterator`, `sax_dpda`, `none` | Strict admission rejects when this does not equal `row_output_plane` (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md`). |
| `comparator_strictness` | SPEC 0.4 | `strict`, `permissive`, `lossy`, `unknown`, `n/a` | Strict admission requires `strict`; sonic lossy and permissive rows remain flaw probes (`restart/skinny/tranches/sk-v8/research/alpha/alpha-B-competitor-deltas.md`). |
| `Sidecar freshness` | SPEC 0.4 | `same-run`, `same-run-native`, `sidecar-same-run`, `stale:<reason>`, `absent:<reason>`, `historical:<id>`, `n/a` | Strict admission allows same-run native strict anchors or same-run sidecars only; missing sidecars require explicit absence reason (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/SPEC.md`). |
| `Profile artifact` | SPEC 0.4 | Existing path under a W0 profile directory, plus symbol/profile format | Reject placeholder `unprofiled`; W0 must name profile command and artifacts (`restart/skinny/tranches/sk-v8/SPEC.md`). |
...
| `CostFacts rule id` | SPEC 0.4 / W1 | `costfacts:<grammar_id>:<rule_id>` or `none:<reason>` before W1 | Required after W1 for materialized JSON rule rows (`restart/skinny/tranches/sk-v8/SPEC.md`). |
| `CostFacts chosen shape` | SPEC 0.4 / W1 | Existing five shapes only: `OffsetTape`, `EventTape`, `EagerTape`, `SinkOnly`, `CollapsedStage`; `none:<reason>` before W1 | No `UnionTape` or new `BackendShape` value. The union is a representation replacement, not a sixth shape (`restart/skinny/tranches/sk-v8/SYNTHESIS.md`). |
...
| `substrate_cardinality` | P3-D addition | `one`, `two_forbidden`, `zero_or_inert`, `unknown` | W3 rejects `two_forbidden` or `unknown`; SC-6's discriminant is cardinality (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-6-lock1-amendment-generalisation.md`). |
...
| Gate | Binding |
|---|---|
| W0 `SK-V8-open` | All 38 current main rows must have row ids, run ids, profile artifacts, hot leaves, host/build/feature metadata, cycles-per-byte or sample-cost equivalent, sidecar freshness, comparator metadata, and `SK-V8-open delta=baseline`. Parser/scanner/SIMD/asm/codegen behavior changes reject W0 (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/HANDOFF.md`). |
| W0 malformed manifest | `gate-json` must reject at least one intentionally malformed sidecar manifest and must reject stale/sidecar-only comparator evidence as strict admission (`restart/skinny/tranches/sk-v8/SPEC.md`). |
| W1 CostFacts | After W1, `gate-json --with-cost-facts` rejects missing `CostFacts rule id`, chosen shape, rejected alternatives, evidence source, REDRESS reference, or wave id (`restart/skinny/tranches/sk-v8/SPEC.md`). |
| W2 typed product | New typed rows require explicit host/API schema facts, generated Track 1, structurally independent Track 2/oracle, full-table maintain, and no digest-stressor promotion (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-D-validated-invalidated.md`). |
| W3 union | Selected parse rows must prove strict validation, comparator evidence, structural cursor work, and admitted tape facts occurred in the measured row. The schema rejects sidecar, view-boundary, post-parse, comparator-only, old offset append, and `tape_vs_tape`-as-consumer proofs (`restart/skinny/tranches/sk-v8/SPEC.md`). |
| W4 direct guard | Direct rows stay guard rows unless a W4 plan names rows and output-plane proof. Digest rows do not become product-plane proof (`restart/skinny/tranches/sk-v8/SPEC.md`). |
| W5 Lock 14 | Generic-crate edits must pass public API, grammar branch, primitive/table, role/fact boundary, template/provider, and non-JSON proof scans (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/SPEC.md`). |
| W6 close | `skinny/RESULTS.md`, `skinny/REDRESS.md`, `restart/skinny/tranches/sk-v8/SPEC.md`, and `restart/skinny/tranches/sk-v8/HANDOFF.md` agree; no row remains with missing required telemetry or unrouted failure (`restart/skinny/tranches/sk-v8/SPEC.md`). |
...
Failure to answer any row-level question is a W3 plan failure, not a reason to relax the schema. `tape_vs_tape` may be added later as W0/W1 gate-binding telemetry only with named owner files, tests, LOC, and rerun budget; it is never a W3 production same-wave consumer (`restart/skinny/tranches/sk-v8/SPEC.md`, `restart/skinny/tranches/sk-v8/HANDOFF.md`).
...
W0 fixture expectation: all 38 current main rows are present and telemetry-bound. That is 17 `parse_only`, 17 `direct_to_struct`, and 4 `real_typed_struct` rows (`restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md`, `skinny/RESULTS.md`).
...
- Do not relabel stale sidecars as same-run strict anchors; REDRESS 77 and 78 made strict/lossy provenance explicit (`skinny/REDRESS.md`).
- Do not turn `K` or optional `S` substrate-guard rows into SOTA admission rows; SC-5 requires demotion plus full residual visibility (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md`).
- Do not treat `tape_vs_tape`, `simd_structural_scan`, sidecar manifests, or comparator-only rows as W3 production consumers (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md`, `restart/skinny/tranches/sk-v8/SPEC.md`).
- Do not introduce `UnionTape`, `BackendShape::Union`, a new BIR variant, a new BBNF directive, a public substrate API, parser-owned cursor/facts, or a parallel substrate (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/hardening/HARDENING-S-P2-V7-CONSOLIDATED.md`).
- Do not reopen REDRESS 28+33, 50-55, 60-72, 80, 82-84, 88, 89, or historical blocked routes without fresh W0 evidence, same-wave consumer, no-regression gate, REDRESS citation, and challenge acceptance (`restart/skinny/tranches/sk-v8/HANDOFF.md`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-C-redress-digest.md`).
