...
|---|---|---|
| Track 2 ≤ S × 0.95 AND Track 1 ≤ Track 2 × 1.10 | Substrate viable, codegen viable | Dispatch tranches A-J; SOTA-beat at V1 likely |
...
`S = min(sonic_rs_anchor_time, simd_json_borrowed_time, simd_json_owned_time)` for the corpus row, using the pinned API/mode recorded in BENCH.md. Full matrix in `BENCH.md` §6.
...
| Cost-model + e-graph rewrites | Skinny pre-selects one canonical plan and bounds that cut with a non-egraph alternate-plan stub bench | H.W2/H.W3 cost-driven rewrite and recognizer tuning body; legality/normalization rewrites remain V1 correctness work in `passes::normalize` |
...
4. **Single-plan extraction.** No CSP, no e-graph, no cost-model selection. COMPILER.md §5.3. BENCH carries a small alternate-plan stub to bound whether this cut masks JSON throughput cost.
...
| `Tape<'input>` seals tokens as `Box<[TapeToken]>` for codegen quality. V1 I tranche's incremental reuse map (`ReparsePlan`) requires append-after-parse. | SUBSTRATE.md §1.2 | The skinny's sealed view is the committed-snapshot projection of the future V1 `TapeBuilder<'input>` (Vec or chunked storage). | I tranche adds `TapeBuilder` upstream; the read-side `Tape<'input>` and `ValueRef` shapes do not change. **MECHANICAL with named inversion** under Lens N. |
...
7. Write `restart/skinny/RESULTS.md` recording the verdict + reproducibility schema rows.
