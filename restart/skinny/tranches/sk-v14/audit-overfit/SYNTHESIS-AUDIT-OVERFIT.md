...
| **C-3** = R4 + R5 | CSS L4 generative pipeline (xtask regen-css from .bbnf) + production corpus | A1 CRIT (24 CSS rows below 1 KB floor); A1 CRIT (W10.2 / W10.3 sub-Mbps clusters); A1 HIGH (W2 / W10.1 sub-Mbps clusters); A1 MED (missing `skinny/corpora/css-l4-sk-v14/`); A4 findings 2 + 3 + 4 + 5 + 6 + 7 (no regen-css subcommand, 4 scanners are fixture lookups, 14/15 .bbnf orphan); A4 finding 1 (CSS templates as include_str only) | 11 |
| **C-4** = PRUNE-5 | Wire W8 per-grammar policy + W9 same-substrate union from gate-only into compile / lower / runtime | A5 finding 1 (W8 SCAFFOLD persists); A5 finding 2 (W9 SCAFFOLD persists); A5 NEW-MED (gate-layer-only footprint quantified); A5 LOW (resolver honest self-labelling — preserved through PRUNE-5 as C-4 entry-gate invariant) | 4 |
