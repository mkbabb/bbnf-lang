...
build_flags_regime: **`RUSTFLAGS unset`** (default aarch64 baseline; no `-C target-cpu=native` override either at the cargo invocation level or via `skinny/Cargo.toml [profile.release]`, which does not pin target-cpu). This is the regime under which every Mbps + c/B row in §2-§3 below was captured. The S-P1 V1 CH4 hardening lens (`hardening/V1/CH4.md §3 CF-1`) records that P1-C / P1-D were captured under `RUSTFLAGS="-C target-cpu=native"` and that P1-A's "native target CPU per Cargo.toml" phrasing is misleading because the workspace Cargo.toml does not in fact pin target-cpu. Per the S-P1 V1 hardening aggregator's cross-regime delta rule, **no Mbps or c/B cell in this artefact is directly comparable to any P1-C / P1-D cell**; the build-flag regimes differ on the NEON intrinsic codegen path (`bbnf-simd` gets aarch64-baseline codegen here vs Apple-Silicon-specialised codegen under P1-C / P1-D). Cross-artefact deltas (e.g. the P1-B Track 1 direct `twitter` 11037 Mbps vs P1-D Track 1 direct `twitter` 11627 Mbps figure cited in `hardening/V1/CH4.md §3 CF-1`) are **regime-mismatch; not directly comparable** and must be refused by the aggregator until both regimes are unified or the row carries an explicit per-row `RUSTFLAGS` qualifier.
...
**Build-flags regime guard (cross-artefact comparator rule).** The §3 delta columns above compare P1-B rows against the SK-V13 RESULTS.md baseline; that comparison is regime-consistent because the SK-V13 baseline runs under the same default-aarch64 `[profile.release]` build that P1-B's `RUSTFLAGS unset` capture uses. **No P1-C / P1-D cross-artefact comparison is computed in §3 because the build-flag regime differs** (P1-C / P1-D both pin `RUSTFLAGS="-C target-cpu=native"`, per `hardening/V1/CH4.md §2.3 + §2.4`). Any reader attempting to subtract a P1-B Mbps / c/B figure here from a P1-C or P1-D figure for the same corpus must first equalise the `build_flags_regime` field (see frontmatter); per the S-P1 V1 hardening aggregator's cross-regime delta rule, such deltas are **regime-mismatch; not directly comparable** and must be refused at the aggregator boundary. The illustrative `twitter` Track 1 direct example documented in `hardening/V1/CH4.md §3 CF-1` (P1-B 11037 Mbps / 3.00 c/B vs P1-D 11627 Mbps / 2.938 c/B) is the canonical refusal target: until V2 unifies the regime (either re-runs this artefact under `target-cpu=native` or re-runs P1-C/D under `RUSTFLAGS unset`), the 5.3 % Mbps + 2.1 % c/B drift is not a parser-level signal — it is a build-flag-regime drift confound.
