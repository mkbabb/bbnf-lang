# T-P3 V5 Final Confirmation CHALLENGE Context
...
Prior revise cycle: `f61202b60` (`docs(sk-v15-t-p3): record V4 hardening citation revise`).
Cycle: `V5`.
Output directory: `restart/audit/totality/p3/hardening/V5/`.
...
5. Prior hardening:
   ...
   - `restart/audit/totality/p3/hardening/V4/CH1.md`

## Final Confirmation Purpose

V3 was clean-cycle 1. V4 was a citation-only `REVISE`. V5 folds that single
defect and is the V≤5 ceiling cycle. If V5 returns seven `ACCEPT` lenses with
zero orphan `REVISE`, T-P3 locks under §3Z-equivalent final convergence for this
cohort: V3 clean-cycle 1 plus V5 clean confirmation after a citation-only V4
repair, with no remaining orphan `REVISE`. If any V5 lens returns `REVISE` or
`REJECT`, do not paper-close; surface the ceiling condition to the orchestrator.
...
- G3 auto-passes only after T-P3 lock under the active user pin; G-Omega is the
  only mandatory user gate.
...
- `CH1.md` CORRECTNESS: verify `CH1-V4-001` is closed, then independently verify
  evidence citations, path resolution, path:line range, 3C live 1E/2X coverage,
  extractable LOCKS diff, and target-packet scope.
...
- `CH4.md` COST: verify the V3 CH4 coverage matrices still close `CH4-V2-001`
  and `CH4-V2-002` after the citation fold, without W12, broad CSSOM parity,
...
git apply --check /tmp/tp3-locks-v5.diff
...
V5 locks T-P3 only if all seven lenses return `ACCEPT`, there are no orphan
