...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(path) = skv13_css_visual_functions_report_path(&args[1..])? {
        if !has_explicit_json_check {
            return Err(format!("{} requires --check-results", path.display()).into());
        }
        let text = fs::read_to_string(&path)?;
        let report = SkV13CssVisualFunctionsReport::from_json_str(&text)
            .and_then(|report| report.validate_gate().map(|_| report))
            .map_err(|error| format!("{}: {error}", path.display()))?;
        validate_skv13_css_visual_functions_report(&report, &criterion_root(), &workspace)
            .map_err(|error| format!("{}: {error}", path.display()))?;
        println!(
            "G-W4-CSS-VISUAL-FUNCTIONS {} {} feature_rows={}",
            report.rows[0].admission_status,
            path.display(),
            report.covered_feature_rows.len()
        );
    ...
    if let Some(path) = skv13_css_at_rules_and_media_report_path(&args[1..])? {
        if !has_explicit_json_check {
            return Err(format!("{} requires --check-results", path.display()).into());
        }
        let text = fs::read_to_string(&path)?;
        let report = SkV13CssAtRulesAndMediaReport::from_json_str(&text)
            .and_then(|report| report.validate_gate().map(|_| report))
            .map_err(|error| format!("{}: {error}", path.display()))?;
        validate_skv13_css_at_rules_and_media_report(&report, &criterion_root(), &workspace)
            .map_err(|error| format!("{}: {error}", path.display()))?;
        println!(
    ...
    if let Some(path) = skv13_same_substrate_union_report_path(&args[1..])? {
        if !has_explicit_json_check {
            return Err(format!("{} requires --check-results", path.display()).into());
        }
        let text = fs::read_to_string(&path)?;
        let report = SkV13SameSubstrateUnionReport::from_json_str(&text)
            .and_then(|report| report.validate_gate().map(|_| report))
            .map_err(|error| format!("{}: {error}", path.display()))?;
        validate_skv13_same_substrate_union_report(&report, &workspace)
            .map_err(|error| format!("{}: {error}", path.display()))?;
        println!(
    ...
    for fixture in fixtures {
        ...
        if w0_real_typed_metadata_expected(&fixture.name) && estimates.real_typed_track1.is_some() {
            ...
            } else if typed_decision.w13_unicode_basic_added {
                mark_w13_unicode_basic_typed_admission(&mut typed_row);
            } else if typed_decision.w13_random_added {
...
fn skv13_css_visual_functions_report_path(
    args: &[String],
) -> Result<Option<PathBuf>, Box<dyn Error>> {
    companion_report_path(args, "--skv13-css-visual-functions-report")
}

fn skv13_css_at_rules_and_media_report_path(
    args: &[String],
) -> Result<Option<PathBuf>, Box<dyn Error>> {
...
fn skv13_same_substrate_union_report_path(
    args: &[String],
) -> Result<Option<PathBuf>, Box<dyn Error>> {
...
fn validate_skv13_css_visual_functions_report(
    ...
    let golden = read_css_l4_lane_in_group(
        criterion_root,
        "nonjson_css_l4_w4",
        "track2_golden_visual_functions_oracle",
        357,
    )?;
    let lightningcss = read_css_l4_lane_in_group(
        criterion_root,
...
fn validate_skv13_css_at_rules_and_media_report(
    ...
    let golden = read_css_l4_lane_in_group(
        criterion_root,
        "nonjson_css_l4_w10_1",
        "track2_golden_at_rules_and_media_oracle",
        85,
    )?;
    let lightningcss = read_css_l4_lane_in_group(
        criterion_root,
...
fn validate_css_l4_visual_functions_retained_artifacts(
    row: &bbnf_bench::report::SkV13CssVisualFunctionsRow,
    workspace: &Path,
) -> Result<(), String> {
    let track1_path = resolve_workspace_path(workspace, &row.track1_artifact);
    let oracle_path = resolve_workspace_path(workspace, &row.oracle_artifact_path);
    ...
    let equality_dir = track1_path
        .parent()
        .ok_or_else(|| "W4 CSS Track 1 artifact has no parent directory".to_string())?;
    let strict_path = equality_dir.join("strict-equality.txt");
    let lightning_eq_path = equality_dir.join("lightningcss-strict-equality.txt");
    let track1 = fs::read(&track1_path)
        .map_err(|error| format!("failed to read {}: {error}", track1_path.display()))?;
    ...
    let lightning = fs::read(&lightning_path)
        .map_err(|error| format!("failed to read {}: {error}", lightning_path.display()))?;
    if track1 != oracle || track1 != lightning {
        return Err("W4 CSS retained fact streams differ".to_string());
    }
    if sha256_hex(&track1) != row.fact_stream_sha256 {
        return Err("W4 CSS fact_stream_sha256 mismatch".to_string());
    }
    let fact_text = std::str::from_utf8(&track1)
        .map_err(|error| format!("W4 fact stream is not UTF-8: {error}"))?;
    for needle in [
        ...
        if !fact_text.contains(needle) {
            return Err(format!("W4 CSS fact stream missing `{needle}`"));
        }
    }
    validate_css_l4_named_equality_artifact(
        &strict_path,
        "css_l4/visual_functions/direct_to_struct/main",
        "sk-v13-w4:fixture-fnv64-c73dd65ad964e9b0",
        false,
    )?;
    validate_css_l4_named_equality_artifact(
        &lightning_eq_path,
    ...
    if resolve_workspace_path(workspace, &row.lightningcss_artifact) != lightning_eq_path {
        return Err("W4 CSS lightningcss equality path mismatch".to_string());
    }
    Ok(())
}

fn validate_css_l4_at_rules_and_media_retained_artifacts(
    row: &bbnf_bench::report::SkV13CssAtRulesAndMediaRow,
    workspace: &Path,
) -> Result<(), String> {
    let track1_path = resolve_workspace_path(workspace, &row.track1_artifact);
    let oracle_path = resolve_workspace_path(workspace, &row.oracle_artifact_path);
    ...
    let equality_dir = track1_path
        .parent()
        .ok_or_else(|| "W10.1 CSS Track 1 artifact has no parent directory".to_string())?;
    let strict_path = equality_dir.join("strict-equality.txt");
    let lightning_eq_path = equality_dir.join("lightningcss-strict-equality.txt");
    let track1 = fs::read(&track1_path)
        .map_err(|error| format!("failed to read {}: {error}", track1_path.display()))?;
    ...
    let lightning = fs::read(&lightning_path)
        .map_err(|error| format!("failed to read {}: {error}", lightning_path.display()))?;
    if track1 != oracle || track1 != lightning {
        return Err("W10.1 CSS retained fact streams differ".to_string());
    }
    if sha256_hex(&track1) != row.fact_stream_sha256 {
        return Err("W10.1 CSS fact_stream_sha256 mismatch".to_string());
    }
    let fact_text = std::str::from_utf8(&track1)
        .map_err(|error| format!("W10.1 fact stream is not UTF-8: {error}"))?;
    for needle in [
        ...
        if !fact_text.contains(needle) {
            return Err(format!("W10.1 CSS fact stream missing `{needle}`"));
        }
    }
    validate_css_l4_named_equality_artifact(
        &strict_path,
        "css_l4/at_rules_and_media/direct_to_struct/main",
        "sk-v13-w10-1:fixture-fnv64-83cb4eb20e5253c7",
        false,
    )?;
    validate_css_l4_named_equality_artifact(
        &lightning_eq_path,
    ...
    if resolve_workspace_path(workspace, &row.lightningcss_artifact) != lightning_eq_path {
        return Err("W10.1 CSS lightningcss equality path mismatch".to_string());
    }
    Ok(())
}
...
fn validate_skv13_same_substrate_union_report(
    report: &SkV13SameSubstrateUnionReport,
    workspace: &Path,
) -> Result<(), String> {
    validate_report_artifact_hash(
        workspace,
...
fn validate_skv13_typed_product_report(
    ...
    let spec = skv13_typed_product_criterion_spec(report)?;
    let track1 = read_css_l4_lane_in_group(
        criterion_root,
        spec.criterion_group,
        "track1_real_typed_struct",
    ...
    require_close(
        &format!("{} typed track1_mbps_after", spec.label),
        report.track1_mbps_after,
        track1.mbps,
    )?;
    require_close(
        &format!("{} typed track2_mbps_after", spec.label),
        report.track2_mbps_after,
        track2.mbps,
    )?;
    require_close(
        &format!("{} typed sonic_strict_mbps_after", spec.label),
        report.sonic_strict_mbps_after,
        sonic.mbps,
    )?;
    require_close(
        &format!("{} typed serde_mbps_after", spec.label),
        report.serde_mbps_after,
        serde.mbps,
    )?;
    require_close(
        &format!("{} typed threshold_mbps", spec.label),
        report.threshold_mbps,
        sonic.mbps + 1.0,
    )?;
    if track1.lower_mbps <= sonic.mbps + 1.0 {
        return Err(format!(
            "{} typed Track 1 lower confidence throughput {:.3} <= sonic+1 {:.3}",
            spec.label,
            track1.lower_mbps,
            sonic.mbps + 1.0
        ));
    ...
    validate_report_artifact_hash(
        workspace,
        &format!("{} typed product measurement artifact", spec.label),
        &report.measurement_artifact_path,
        &report.measurement_artifact_sha256,
    )
}

struct TypedProductCriterionSpec {
    label: &'static str,
    criterion_group: &'static str,
    bytes: u64,
}

fn skv13_typed_product_criterion_spec(
    report: &SkV13TypedProductReport,
) -> Result<TypedProductCriterionSpec, String> {
    match (report.wave_id.as_str(), report.corpus.as_str()) {
        ("SK-V13-W13.1", "numbers") => Ok(TypedProductCriterionSpec {
            label: "W13.1",
            criterion_group: "json_numbers",
            bytes: 150_124,
        }),
        ("SK-V13-W13.2", "unicode_basic") => Ok(TypedProductCriterionSpec {
            label: "W13.2",
            criterion_group: "json_unicode_basic",
            bytes: 1_048_586,
        }),
        ("SK-V13-W13.3", "random") => Ok(TypedProductCriterionSpec {
            label: "W13.3",
            criterion_group: "json_random",
            bytes: 510_476,
        }),
        ("SK-V13-W13.4", "instruments") => Ok(TypedProductCriterionSpec {
            label: "W13.4",
...
fn w13_typed_strict_sonic_plus_one_passes(
    bytes: u64,
    track1_ns: Option<f64>,
    track2_ns: Option<f64>,
    sonic_ns: Option<f64>,
...
fn mark_w13_unicode_basic_typed_admission(row: &mut TelemetryRow) {
    ...
    row.parse_utf8 = "measured-row".to_string();
    row.flaw_probe =
        "generated Track 1 typed UnicodeBasicRecord product vs independent serde Track 2/oracle; UTF-8 measured in row"
            .to_string();
    row.signal = format!(
        "PASS W13.2 unicode_basic typed product admission; Track 1 {}, Track 2 oracle {}, sonic {} Mbps",
        format_mbps(row.track1_mbps),
        format_mbps(row.track2_mbps),
        format_mbps(row.competitors.sonic_strict_mbps)
    );
    row.sk_v8.measured_validation_path = "measured-row".to_string();
    row.sk_v8.same_wave_consumer_class = "gate_json_typed_contract".to_string();
    row.sk_v8.redress_entry = "REDRESS-146".to_string();
    row.sk_v8.wave_id = "SK-V13-W13.2".to_string();
    row.sk_v8.sk_v9_open_delta = "typed-row-added".to_string();
}

fn mark_w13_random_typed_admission(row: &mut TelemetryRow) {
    row.strictness = "strict".to_string();
    row.parse_utf8 = "measured-row".to_string();
    row.flaw_probe =
        "generated Track 1 typed RandomDocument product vs independent serde Track 2/oracle; UTF-8 measured in row"
            .to_string();
    row.signal = format!(
        "PASS W13.3 random typed product admission; Track 1 {}, Track 2 oracle {}, sonic {} Mbps",
        format_mbps(row.track1_mbps),
        format_mbps(row.track2_mbps),
        format_mbps(row.competitors.sonic_strict_mbps)
    );
    row.sk_v8.measured_validation_path = "measured-row".to_string();
    row.sk_v8.same_wave_consumer_class = "gate_json_typed_contract".to_string();
    row.sk_v8.redress_entry = "REDRESS-147".to_string();
    row.sk_v8.wave_id = "SK-V13-W13.3".to_string();
    row.sk_v8.sk_v9_open_delta = "typed-row-added".to_string();
}

fn mark_w13_instruments_typed_admission(row: &mut TelemetryRow) {
    row.strictness = "strict".to_string();
    row.parse_utf8 = "measured-row".to_string();
    row.flaw_probe =
        "generated Track 1 typed InstrumentsDocument product vs independent serde Track 2/oracle; UTF-8 measured in row"
            .to_string();
    row.signal = format!(
        "PASS W13.4 instruments typed product admission; Track 1 {}, Track 2 oracle {}, sonic {} Mbps",
        format_mbps(row.track1_mbps),
        format_mbps(row.track2_mbps),
        format_mbps(row.competitors.sonic_strict_mbps)
    );
...
mod tests {
    ...
    fn skv13_css_comparator_oracle_report_arg_allows_multiple_read_only_reports() {
        let mixed = vec![
            ...
            "--skv13-css-visual-functions-report".to_string(),
        ...
        assert_eq!(
            skv13_css_visual_functions_report_path(&mixed).unwrap(),
            Some(PathBuf::from("skv13-css-w4.json"))
        );
        assert_eq!(
    ...
    #[test]
    fn skv13_same_substrate_union_report_arg_allows_json_check_only() {
        let args = vec![
            "--skv13-same-substrate-union-report".to_string(),
            "skv13-w9.json".to_string(),
            "--advisory".to_string(),
    ...
    #[test]
    fn w13_unicode_basic_typed_admits_only_strict_sonic_plus_one_pass() {
        assert_eq!(
            real_typed_row_decision(
                "unicode_basic",
                None,
    ...
    #[test]
    fn w13_random_typed_admits_only_strict_sonic_plus_one_pass() {
        assert_eq!(
            real_typed_row_decision(
                "random",
                None,
    ...
    #[test]
    fn w13_instruments_typed_admits_only_strict_sonic_plus_one_pass() {
        assert_eq!(
            real_typed_row_decision(
                "instruments",
                None,
    ...
    fn w0_real_typed_metadata_expectation_uses_measured_baseline_not_source_fixtures() {
        ...
        assert!(w0_real_typed_metadata_expected("unicode_basic"));
        assert!(w0_real_typed_metadata_expected("random"));
        assert!(w0_real_typed_metadata_expected("instruments"));
    }
