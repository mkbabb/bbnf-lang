...
Output: this file.
Pass Alpha goalset: admit one generated non-JSON direct or typed parser baseline, then admit one measured grammar-generalized intervention on that same row at `ceil(baseline_mbps * 1.01)` or stricter, while guarding the 4 direct and 7 typed JSON A/GO rows; parse-only remains diagnostic and the 13 JSON direct residual rows remain pre-blocked unless reopened with fresh material evidence beyond REDRESS 114-119.
...
(`restart/skinny/tranches/sk-v12/HANDOFF.md:60`-`:70`).

The current result surface is unchanged from SK-V11 close: 16 parse rows are
`S / NO-GO`, `canada/parse_only` is `L / NO-GO`, direct has 4 `A / GO` and 13
`N-direct / NO-GO`, typed has 7 `A / GO`, and overall remains
`N-direct / NoGo` (`skinny/RESULTS.md:143`-`:146`;
`restart/skinny/tranches/sk-v12/HANDOFF.md:41`-`:47`). REDRESS 120 is the seed
close authority: SK-V11 made no behavior source, generated runtime, benchmark
body, gate semantic, or `RESULTS.md` movement, and it routes SK-V12 to solve the
generated non-JSON baseline before revisiting direct residuals
(`skinny/REDRESS.md:3531`-`:3553`).

The S-P2 candidate pool is therefore admissible only through grammar-neutral
generated consumers. P2-A says comparator candidates must stay grammar-neutral,
preserve the single substrate, carry scalar references, and be consumed
immediately by generated parser/product paths
(`restart/skinny/tranches/sk-v12/research/p2/p2a-sota-teardown.md:23`-`:37`).
P2-B makes scalar oracle, strict checkasm, caller micro-proof, and same-wave
consumer the process floor for ASM/SIMD (`restart/skinny/tranches/sk-v12/research/p2/p2b-dav1d-process.md:27`-`:35`).
P2-C keeps AArch64-only candidates in scope and demotes LD4/SHA3 ideas to
inventory until fresh P1 evidence names the missing stream/expression
(`restart/skinny/tranches/sk-v12/research/p2/p2c-arch-esoterica.md:34`-`:42`,
`:122`-`:133`). P2-D contributes no selectable substrate candidate and rejects
the structural class-lane union outright (`restart/skinny/tranches/sk-v12/research/p2/p2d-substrate-tape.md:69`-`:83`).
P2-E lists parse-that primitives only as gaps with same-wave generated
consumers, not as JSON direct authority (`restart/skinny/tranches/sk-v12/research/p2/p2e-parse-that-gaps.md:46`-`:47`,
`:374`-`:394`). P2-F says the generator is still JSON-profiled at runtime
emission and that Lock 14 requires generated per-grammar modules, not generic
crate grammar policy (`restart/skinny/tranches/sk-v12/research/p2/p2f-grammar-neutral.md:14`-`:19`).

The ledger conclusion is not "all primitive work is blocked." It is narrower:
primitive work may admit only when a wave gives it a scalar/oracle reference,
strict parity or equivalent equality, a same-wave generated consumer, a
measurable row gate, and a material differential from the REDRESS route it
touches. Otherwise the route is a pre-blocked retry.

## §2 — Deliverable

### Global Pre-Blocks

| Route family | Binding REDRESS / source authority | Decision |
|---|---|---|
| W3 union/event/class-column/streaming-cursor/class-lane/sidecar substrate, including `UnionTape`, retained structural vectors, parser-owned projections, and W4-through-W3 cascade-lock | REDRESS 96 and 97 measured parity-green W3 class-column and streaming-cursor implementations as row-regressing; REDRESS 98 retires `G-W3-UNION-SUBSTRATE` (`skinny/REDRESS.md:2795`-`:2948`). REDRESS 102 later firewalled parse-only/W3 reopen routes (`skinny/REDRESS.md:3040`-`:3058`). P2-D rejects `structural_class_lane_union` (`restart/skinny/tranches/sk-v12/research/p2/p2d-substrate-tape.md:53`-`:59`, `:79`). | Hard pre-block. No SK-V12 wave may dispatch a renamed union, class column, retained cursor, retained structural vector, `UnionTape`, sidecar, or W4-through-W3 consumer route. |
| Parse-only SOTA close or parse-only row admission | SYNTHESIS classifies `parse_only` as diagnostic only (`restart/skinny/tranches/sk-v12/SYNTHESIS.md:62`-`:64`); REDRESS 102 validates that report/gate rejects parse-only SOTA movement (`skinny/REDRESS.md:3042`-`:3057`). | Hard pre-block. Parse-only can supply profile health and guard diagnostics only. |
| JSON direct residual row movement | REDRESS 119 closes all 13 residual direct rows as measured fixpoint and no row movement (`skinny/REDRESS.md:3497`-`:3525`); REDRESS 120 requires material evidence beyond REDRESS 114-119 before any future reopen (`skinny/REDRESS.md:3542`-`:3553`). | Pre-blocked by default. Reopen only under the material-differential rule below, and never before non-JSON priority succeeds or records an explicit measured block. |
| REDRESS 111 report lane as generated baseline | REDRESS 111 admits a companion non-JSON report/gate lane only (`skinny/REDRESS.md:3282`-`:3309`); REDRESS 112 rejects generated CSS L4 baseline because runtime/codegen remains JSON-profiled (`skinny/REDRESS.md:3311`-`:3338`); REDRESS 113 blocks intervention without a baseline (`skinny/REDRESS.md:3340`-`:3355`). | Hard boundary. A generated baseline wave may consume REDRESS 111's gate shape, but cannot treat its fixture/report as Track 1 runtime evidence. |
...
This ledger names P3-E's wave classes so P3-B/P3-F can bind exact wave numbers
without reopening the route analysis.
...
| W3+ conditional JSON companion | Optional only after W1/W2 succeed, W1 admits and W2 records a measured reject, or the generated non-JSON route records measured `BLOCKED`. | All 13 REDRESS 119 residual rows by default; `number_span_emit_slot`, `container_tail_next`, bounded string span, decoded-byte source fold, output digest host-sink, W0-clamped/docs-only rows, parse-only, W3 substrate. | A residual direct row may reopen only with fresh SK-V12 P1/S-P2 evidence beyond REDRESS 114-119: new hot-leaf evidence, source delta materially different from W3-W7, scalar/oracle proof, same-host microbench, independent Track 2, strict same-run sonic-rs direct floor, same-wave gate consumption, and non-JSON priority already resolved. |
...
| Output sink / digest / oracle (C6, F7) | REDRESS 118 output digest/host-sink block; direct-as-typed proof ban. | Oracle/report support or row-owned generated product only. Cannot be a parser primitive or typed proof substitute. |
| Tape/accounting/capacity (P2-D diagnostics, F8) | REDRESS 50/51/53 side tables; REDRESS 96/97/98 union substrate; REDRESS 102 parse firewall. | Same-tape accounting only. Needs fresh hot-leaf evidence for capacity/flag/view work and may not create a second retained substrate. |
| ISA inventory: LD4 and SHA3/EOR3/BCAX | P2-C marks both non-selectable because S-P1 lacks an interleaved stream or concrete three-input expression (`restart/skinny/tranches/sk-v12/research/p2/p2c-arch-esoterica.md:55`-`:65`, `:122`-`:133`). | May admit only under a future fresh P1 profile naming the exact stream/expression plus scalar oracle, checkasm parity, and generated consumer. |

## §3 — Falsifiability binding

P3-E is a negative ledger, so its falsifiability binding is fail-closed route
classification rather than a throughput lift. The measurable row/threshold facts
that matter to the ledger are:

| Surface | Binding rows / thresholds | Ledger effect |
|---|---|---|
...
also land the same-wave gate consumer. Missing required fields, stale run ids,
producer-only telemetry, oracle coupling, generated Track 1 / Track 2
dishonesty, parse-only SOTA claims, W3 reopen claims, direct digest as typed
proof, and JSON policy leakage fail the route closed
(`restart/skinny/tranches/sk-v12/SYNTHESIS.md:207`-`:226`).

## §4 — Pre-blocked routes

Hard pre-blocks no SK-V12 wave may reopen under the current contract:
...
1. W3 union/event/class-column/streaming-cursor/class-lane/sidecar substrate,
   ...
   and W4-through-W3 cascade-lock.
2. Parse-only SOTA close, parse-only row admission, or parse-only evidence as a
   generated non-JSON or direct product proof.
3. JSON direct residual row movement before the generated non-JSON baseline and
   selected-baseline intervention priority succeeds or records a measured block.
4. JSON direct residual row movement after that priority unless fresh evidence
   exceeds REDRESS 114-119 with a material source delta, scalar/oracle proof,
   same-host microbench, independent Track 2, strict sonic direct floor, and
   same-wave gate consumption.
5. REDRESS 111 report/gate lane treated as generated Track 1 baseline.
6. W0-clamped row admission, docs-only direct admission, or row movement by
   table arithmetic.
7. Direct digest evidence as typed product proof, or output digest/hash as a
   parser primitive without a legal row-owned product.
8. Admission by analogy from another row or output plane.
9. Replays of `number_span_emit_slot`, `container_tail_next`, bounded string
   span, decoded-byte source folds, output-digest host-sink, StringBlock16,
   single-quartet/x4 proof-only, PMULL prefix-XOR default body, or CSSC CTZ bulk
   consumer without material differential and new row/oracle evidence.
10. PMU, cycles, structural-scan, masking probes, Criterion slope, sidecar
...
Routes that may admit only under different framing with fresh P1 evidence:

- Same-tape capacity, sparse-flag, or retained cursor-skip diagnostics may enter
  a later shortlist only if a fresh profile names builder capacity, flag lookup,
  or retained traversal as a hot leaf and the wave keeps the single tape
  substrate (`restart/skinny/tranches/sk-v12/research/p2/p2d-substrate-tape.md:46`-`:51`,
  `:109`-`:111`).
- LD4 interleaved classification may enter only if a fresh profile names an
  existing canonical interleaved stream and the wave supplies scalar
  deinterleave oracle, checkasm parity, and same-loop generated consumer
  (`restart/skinny/tranches/sk-v12/research/p2/p2c-arch-esoterica.md:55`-`:65`).
- SHA3 ternary boolean fold may enter only if a fresh profile names the exact
  hot three-input expression and the wave supplies scalar formula, checkasm
  parity, and generated/runtime consumer
