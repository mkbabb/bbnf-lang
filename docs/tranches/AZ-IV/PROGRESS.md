...
**Close artefact**: [`FINAL.md`](FINAL.md)
...
| W4 - Optimization Substrate Activation | complete_with_misses | `audit/W4-*.{txt,md}` | W4.1 (`8fa9df03` egraph::ruler::* + RuleSet DELETED -1032 LOC + T3 inline-trace TraceSink) + W4.2 (`a63bb7e3` alt_strategy override retired + KeyDispatch singleton domain; T9 unification HALTED — sidecars encode disjoint concepts) + W4.3 (`d8c6dae5` shape_dict_templates + shape_dict_selection DELETED, simd-scan already consumed) + W4.4 (`a6b3af0c` dfa_codegen→regex_scan_adapter rename + dta.rs→grammar_facts.rs rename + `43c313f9` T1 from_rule_id→compound_kind_for_layout registry projection + `686fcd5d` PatternAnnotations retirement plan) + W4.5 (`df08ffe5` analysis/inline.rs 673 LOC → 4 sub-modules + `8ba31a82` PatternAnnotations DELETED, Pratt detect via NodeFacts only). Tailwind perf carry routes to BB rule-discovery. AUDIT-B routed splits for dta.rs + csp_strategy/mod.rs deferred to follow-on (cross-agent collision avoidance). |
| W5 - TS Binding + Value-API + Substrate Audit | complete_with_misses | `audit/W5-*.{txt,md}` | W5.1 (`3adf9d03` bbnf-path-ts cdylib + wasm-bindgen + template-tag + 5/5 isomorphic PathError tests; native + wasm32 builds clean) + W5.2 (`7c5e68bf` ts_node_execute integration test; spawns Node v22.15.0; **TS execution RED — W1 backend-ts gap surfaced**: object/array `value` is span over input bytes, not aggregated array of pairs/elements; documented triumvirate trigger per W5 dispatch; followup carry) + W5.3 (`59350ec8` arena_template + builder_template + 5 simple grammars deduped -301 LOC; 4 outliers JSON/CSS L4/Sheets/BBNF + AUDIT-B css_l4/builder.rs + types/mod.rs splits routed to follow-on per HARD CAP) + W5.4 (`bd72a784` permanent CI-gated substrate_audit.rs test; 886 pub substrates enumerated; **32 zero-caller substrates surfaced** — exceeds >5 threshold per HALT rule, cleanup routed to follow-on; test infrastructure itself is the W5.4 deliverable) + W5-T4 (`6adf38f5` codegen `pub static REGISTRY: LazyLock<StructRegistry>` + sidecar JSON per grammar; bbnf-path proc-macro consumes production sidecar via OnceLock cache); W5-T6 module-split DEFERRED (xtask refactor exceeded cap; documented in `W5-t6-module-split.txt`). |
| W6 - Measurement And Close | complete_with_misses | [`FINAL.md`](FINAL.md), `audit/W6-*.{txt,md,json}`, `docs/benchmarks/post-AZ-IV.json` | W6.1 fat-LTO matrix landed (commit `39e28a50` — `post-AZ-IV.json` with 27 measured rows, sonic-rs paired comparators, AU floor anchored, 3 WATCHDOG_HALT rows with named hotspots); W6.2 workspace gates landed (commit `cc0d8d65` — 1606/1610 passed, 4 known-environmental fails: 2 LSP timeouts + ts_node_execute W1 carry + substrate_audit 32 zero-caller routed; 32-substrate cleanup ledger at `audit/W6-substrate-cleanup-route.md`); W6.3 close docs landed (`1b957e5e`+`3d255680`+`628fb92f` — FINAL.md cites resolving artefact for every Hard Gate; per-wave status updated). **Routed**: bbnf_get_twitter 4196x sonic gap + AU floor regressions → BA rule-discovery + AZ-V substrate-direct optimization tranche per `audit/W6-fat-lto.txt`. Samply 7-artefact contract environmentally-gated to post-AZ-IV measurement cohort. |
...
| 2026-05-02 | W4 close | commits `8fa9df03`+`a63bb7e3`+`d8c6dae5`+`a6b3af0c`+`43c313f9`+`df08ffe5`+`8ba31a82`+`9cef79c5` (regen sync) | W4 substrate activation complete_with_misses: ruler::* + RuleSet DELETED (-1032 LOC); CSP alt_strategy override retired; shape_dict_* fields DELETED; dfa_codegen→regex_scan_adapter + dta→grammar_facts renames; T1 registry projection; PatternAnnotations retirement; analysis/inline.rs split. T9 sidecar unification HALTED (disjoint concepts). AUDIT-B dta.rs + csp_strategy splits deferred (cross-agent collision avoidance). |
| 2026-05-02 | W5 close | commits `7c5e68bf`+`bd72a784`+`3adf9d03`+`59350ec8`+`6adf38f5`+`872b0a9a` | W5 TS binding + value-API + substrate audit complete_with_misses: bbnf-path-ts cdylib + 5/5 isomorphic PathError; ts_node_execute test landed (W1 backend-ts gap surfaced — RED gate, triumvirate trigger, post-AZ-IV follow-on); arena/builder template skeleton dedup on 5 grammars (-301 LOC); permanent substrate_audit.rs CI test (32 zero-caller substrates routed to follow-on per HALT rule); T4 production REGISTRY + sidecar JSON consumption. T6 module-split + AUDIT-B splits + 4 outlier-grammar dedup + W1 TS aggregate emit are explicit follow-on carries. |
| 2026-05-02 | W6.3 close docs + tranche close | this commit | AZ-IV closes `complete_with_misses`. `FINAL.md` lands: every Hard Gate (1-23) cites resolving artefact (MET / MET_WITH_MISSES / ROUTED); every Non-Routable Carry (1-33) resolves to landed evidence or routed follow-on with named destination. Wave statuses on `waves/W{0,1,2,3,4,5,6}.md` match reality. **Routed follow-ons (do not invalidate thesis)**: W3 Sheets Flat-shape lazy → post-W3; W3 Hard Gate 7 sonic-rs ≤ 5x close-matrix → W6.1 measurement cohort + BB rule-discovery; W4 AUDIT-B `dta.rs` + `csp_strategy/mod.rs` splits → follow-on; W4 Tailwind perf → BB rule-discovery; W5 W1 backend-ts aggregate emit (RED Node-execute gate) → post-AZ-IV TS triumvirate; W5 4 outlier-grammar dedup (JSON/CSS L4/Sheets/BBNF) → follow-on; W5 AUDIT-B `css_l4/builder.rs` + `types/mod.rs` splits → follow-on; W5 32 zero-caller substrates → follow-on cleanup (permanent CI-gated test fires until count = 0); W5 T6 module-split generated → follow-on; W6.1 fat-LTO `post-AZ-IV.json` + W6.2 workspace-gates aggregate → post-AZ-IV measurement cohort. BC (bbnf-buddy) tracks alongside per project memory. **Recycled BA opens after this close.** |
