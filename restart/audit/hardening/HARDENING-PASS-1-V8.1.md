...
|---:|---|---|---|---|
| 1 | Lock-Adherence (Lock 4 — DK13 + GADT V1 surface + closure-by-`&'i`) | READY (with simplification opportunity I-2) | Phase 8.3.1 retained Lock 4's GADT V1 user-facing surface + `Pattern @ where T = U` refinement at `restart/locks/14-LOCKS.md:40`; PASS-1 surfaces `BBNF-LOCAL-EQUALITY-ANNOTATION` at `:129` + `Refinement`/`Equality` non-terminals at `:244-246`; the I-2 substrate-deletion alternative was overridden by user FOLD adjudication (Q1) | **READY** |
