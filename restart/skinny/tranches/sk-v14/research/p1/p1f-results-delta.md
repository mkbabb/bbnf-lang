...
**Contracted-deferral framing (V2 reclassification — F-V2-P1F-1).** All four schema-extension columns above — `comparator_plane`, `per_iter_equality`, `audit_overlay_verdict`, `track2_entry_point` — are **contracted-deferrals to the C-2 wave**, NOT S-P1 attribution failures. The framing is explicit and load-bearing:

- **Requirement source (where the column was declared):** `restart/skinny/tranches/sk-v14/SYNTHESIS.md §2` "Telemetry Binding" (lines 232-258) enumerates the schema verbatim; lines 240-242 + 255 introduce the four NEW columns with `**NEW (CH5)**` / `**NEW (R1)**` / `**NEW (R2)**` / `**NEW (audit overlay)**` annotations. The columns are mandatory on every row at SK-V14 close, not at SK-V14 open.
- **Wave deliverable (where the column lands):** `restart/skinny/tranches/sk-v14/SYNTHESIS.md §3` "Candidate Shortlist" row C-2 (line 272) binds R1 + R2 to "comparator rebind + per-iter equality oracle" and names the consumer: "bench harness consumes the rebound comparators on every named JSON row; `xtask gate-json` enforces the schema. ... bench harness emits an equality-pass column per iter". The `track2_entry_point` column is the CH5 hidden-coupling guard that the C-2 schema rewrite populates per row; the `audit_overlay_verdict` column is the audit-overlay scribe that lands alongside the C-2 schema rewrite.
- **Precedent (the analogous contracted-deferral pattern):** `restart/prompts/pass-contracts/PASS-ALPHA.md §4.4` (lines 112-122) declares "Wave-by-wave falsifiability gates ... This layer is authored downstream by skinny pass S-P3 in `sk-v{N+1}/SPEC.md`, consuming the goalset Pass Alpha sets at §4.1–§4.3" — i.e. Pass Alpha §4.4 is a CONTRACTED-DEFERRAL to S-P3, not a Pass Alpha attribution failure. The SK-V14 schema-extension column gap follows the same structural pattern: SYNTHESIS §2 DECLARES the column (analogous to PASS-ALPHA §4.1–§4.3 declaring the goalset); C-2's R1+R2 wave POPULATES the column (analogous to S-P3 authoring §4.4 wave gates). The deliverable boundary is contractually downstream of S-P1.
- **CH5 V1 REVISE → V2 ACCEPT-WITH-NOTE conversion.** `restart/skinny/tranches/sk-v14/research/p1/hardening/V1/CH5.md` Finding CH5-B (lines 104-120) classified the `track2_entry_point` column gap as a "downstream-wave column gap, not a P1 symbol-attribution error" and assigned the population to C-2 (R2 wave). With the contracted-deferral framing made explicit here, the CH5 V1 REVISE on P1-F converts to **ACCEPT-WITH-CONTRACTED-DEFERRAL** at V2 CHALLENGE: S-P1 cannot populate columns the bench harness does not emit; the C-2 schema rewrite is the load-bearing fix, not a P1-F authoring failure. The same framing covers `comparator_plane` (R1), `per_iter_equality` (R2), and `audit_overlay_verdict` (audit-overlay scribe) — all four NEW columns are jointly delivered by the C-2 wave per SYNTHESIS §3 C-2 binding.

This is **S-P1 ACCEPT-WITH-CONTRACTED-DEFERRAL**, NOT S-P1 attribution failure. P1-F correctly identifies the column gap as a per-row population obligation; the C-2 wave plan owns the bench-harness emission that closes the gap.
