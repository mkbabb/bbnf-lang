...
| 16 | Same-harness sonic-rs floor (`bbnf_value_*` parity-or-better; `bbnf_get_*` ≤ 5x) | MET_WITH_MISSES | `audit/W6-sonic-projection.txt` (W6.1 close `39e28a50`); paired sonic-rs comparators measured. **Miss**: `bbnf_get_twitter` 4196x > sonic_get_twitter (Hard Gate 7 carry routed to BA rule-discovery + AZ-V substrate-direct optimization tranche per `audit/W6-fat-lto.txt`). Mechanism shipped (W3 cursor-threading + per-iteration decision consult `33184651`+`715747db`); performance closure is the routed post-AZ-IV deliverable |
