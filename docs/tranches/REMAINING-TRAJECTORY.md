> **STATUS: SUPERSEDED at master cb14970f (2026-05-02).** AZ-IV closed `complete_with_misses` per [`docs/tranches/AZ-IV/FINAL.md`](AZ-IV/FINAL.md); recycled BA (rule discovery) opens next. GESTALT.md is the project synthesis. This file is preserved for archaeology — the optimisation ledger, code-shape sketches, probability table, and competitor-delta trajectory it carries are historical references against which AZ-IV measured (`docs/benchmarks/post-AU.json` is still the authoritative AU floor; the numbers cited here from AY-I onwards are pre-AZ-III). For the active synthesis: read `docs/GESTALT.md`; for the AZ-IV close artefact: read `docs/tranches/AZ-IV/FINAL.md`; for the post-AZ-IV residual (rule discovery): read `docs/tranches/BA/BA.md` (recycled letter).
>
