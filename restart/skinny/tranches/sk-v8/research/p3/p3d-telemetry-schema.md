...
Output: this file.
Pass Alpha goalset: SK-V8 must create `SK-V8-open`, populate required telemetry on all 38 current main rows, make `gate-json` reject missing telemetry, bind CostFacts before behavior admission, preserve typed GO rows, keep direct digest rows as guard rows, and pass Lock 14/Lock 15 at close (`restart/skinny/tranches/sk-v8/SPEC.md:24-42`, `restart/skinny/tranches/sk-v8/HANDOFF.md:133-143`).
Candidate pool: S-P2 substrate-ceiling survivors after V7 consolidation; V7 accepted 6/6 and authorizes S-P3, not any implementation wave (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/hardening/HARDENING-S-P2-V7-CONSOLIDATED.md:7-20`).

## §1 - Synthesis

P3-D is a gate-binding plan, not a source plan. S-P3 is read-only against `skinny/` source, and implementation belongs only to later wave redress phases (`restart/prompts/skinny/PASS-3-SYNTHESIS-PLAN.md:44-46`). This artifact therefore names schema requirements, allowed values, row bindings, fixtures, and failure states only.

The current authority is schema-v3 `skinny/RESULTS.md`: 38 main rows, current overall `N-direct / NoGo`, same-run sonic strict/lossy provenance, sidecar C++ values only where populated, and missing hot-leaf attribution (`skinny/RESULTS.md:1-43`, `skinny/RESULTS.md:216-219`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md:11-40`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md:135-154`). The live rendered header has 26 columns, even though the P3 prompt shorthand says "24-column schema"; this plan preserves the live 26-column header and treats the prompt wording as shorthand for the schema-v3 surface (`skinny/RESULTS.md:3-4`, `restart/prompts/skinny/PASS-3-SYNTHESIS-PLAN.md:239-244`).

The schema has two jobs before behavior waves can dispatch. First, W0 must make every current row observably tied to a run, profile artifact, host/build metadata, comparator plane, strictness, sidecar freshness, and `SK-V8-open` delta (`restart/skinny/tranches/sk-v8/SPEC.md:79-123`, `restart/skinny/tranches/sk-v8/SPEC.md:287-323`). Second, W1 must make CostFacts chosen/rejected evidence consumed by `gate-json --with-cost-facts`, without changing parser behavior or generated JSON output unless a later plan explicitly owns that behavior (`restart/skinny/tranches/sk-v8/SPEC.md:329-372`).

The W3 tape plus structural-projection union needs extra observability but no new directive, BIR variant, or substrate. S-P2 preserves the union as a lead hypothesis only: Tier A is structural-class cursor migration inside the single retained `Tape`; Tier B owns string-boundary, quote/backslash/parity, CostFacts-template, and non-JSON production migration (`restart/skinny/tranches/sk-v8/SYNTHESIS.md:138-172`). W3 remains blocked until W0/W1 close and a fresh plan names owner paths, same-wave production consumer, revert protocol, thresholds, measured-path strict validation, and challenge acceptance (`restart/skinny/tranches/sk-v8/HANDOFF.md:88-96`, `restart/skinny/tranches/sk-v8/SPEC.md:447-452`).

## §2 - Deliverable

### §2.1 Rendered schema-v3 surface

Keep the current rendered `skinny/RESULTS.md` columns unchanged for `SK-V8-open`:
...
| `Workload` | existing | Current main: `parse_only`, `direct_to_struct`, `real_typed_struct`; routed residual only: `tape_vs_tape`; non-main probe: `simd_structural_scan` | `tape_vs_tape` is not default W0/W1 scope and cannot satisfy W3 production-consumer proof (`restart/skinny/tranches/sk-v8/SPEC.md:125-131`). |
| `Outcome` | existing / SPEC 0.3 | `A`, `C`, `G`, `K`, `L`, `N-direct`; optional post-W0 amendment `S` | Reject any other outcome after W0; `K` and `S` never support strict SOTA admission (`restart/skinny/tranches/sk-v8/SPEC.md:57-77`). |
...
| `Strictness` | existing | `strict`, `deferred`, `permissive`, `lossy`, `n/a` | Strict admission requires `strict`; current rows are `deferred` (`skinny/RESULTS.md:5-42`). |
...
| `row_output_plane` / `Output plane` | normalized W0 addition over existing prose | `borrowed_view_offset_tape`, `dom_value_tree`, `digest`, `typed_direct`, `offset_index`, `structural_index`, `sink_only_transient`, `n/a` | Required comparator-plane equality key. Current parse rows are borrowed view over offset tape vs DOM; direct rows are digest; real typed rows are typed direct (`skinny/RESULTS.md:5-42`). |
| `Track 1 Mbps`, `Track 2 Mbps` | existing | Positive numeric Mbps or `n/a` only when workload has no track | W0 telemetry-only rows must remain within +/-1.0 percent versus `SK-V8-open` (`restart/skinny/tranches/sk-v8/SPEC.md:300-314`). |
...
| `comparator_plane` | SPEC 0.4 | `dom_value_tree`, `digest`, `typed_direct`, `offset_index`, `structural_index`, `lazy_iterator`, `sax_dpda`, `none` | Strict admission rejects when this does not equal `row_output_plane` (`restart/skinny/tranches/sk-v8/SPEC.md:117-123`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:233-250`). |
| `comparator_strictness` | SPEC 0.4 | `strict`, `permissive`, `lossy`, `unknown`, `n/a` | Strict admission requires `strict`; sonic lossy and permissive rows remain flaw probes (`restart/skinny/tranches/sk-v8/research/alpha/alpha-B-competitor-deltas.md:20-49`). |
| `Sidecar freshness` | SPEC 0.4 | `same-run`, `same-run-native`, `sidecar-same-run`, `stale:<reason>`, `absent:<reason>`, `historical:<id>`, `n/a` | Strict admission allows same-run native strict anchors or same-run sidecars only; missing sidecars require explicit absence reason (`restart/skinny/tranches/sk-v8/SPEC.md:117-123`, `restart/skinny/tranches/sk-v8/SPEC.md:300-305`). |
| `Profile artifact` | SPEC 0.4 | Existing path under a W0 profile directory, plus symbol/profile format | Reject placeholder `unprofiled`; W0 must name profile command and artifacts (`restart/skinny/tranches/sk-v8/SPEC.md:287-316`). |
...
| `CostFacts rule id` | SPEC 0.4 / W1 | `costfacts:<grammar_id>:<rule_id>` or `none:<reason>` before W1 | Required after W1 for materialized JSON rule rows (`restart/skinny/tranches/sk-v8/SPEC.md:346-366`). |
| `CostFacts chosen shape` | SPEC 0.4 / W1 | Existing five shapes only: `OffsetTape`, `EventTape`, `EagerTape`, `SinkOnly`, `CollapsedStage`; `none:<reason>` before W1 | No `UnionTape` or new `BackendShape` value. The union is a representation replacement, not a sixth shape (`restart/skinny/tranches/sk-v8/SYNTHESIS.md:162-172`). |
...
| `substrate_cardinality` | P3-D addition | `one`, `two_forbidden`, `zero_or_inert`, `unknown` | W3 rejects `two_forbidden` or `unknown`; SC-6's discriminant is cardinality (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-6-lock1-amendment-generalisation.md:641-655`). |
...
| Gate | Binding |
|---|---|
| W0 `SK-V8-open` | All 38 current main rows must have row ids, run ids, profile artifacts, hot leaves, host/build/feature metadata, cycles-per-byte or sample-cost equivalent, sidecar freshness, comparator metadata, and `SK-V8-open delta=baseline`. Parser/scanner/SIMD/asm/codegen behavior changes reject W0 (`restart/skinny/tranches/sk-v8/SPEC.md:277-327`, `restart/skinny/tranches/sk-v8/HANDOFF.md:110-121`). |
| W0 malformed manifest | `gate-json` must reject at least one intentionally malformed sidecar manifest and must reject stale/sidecar-only comparator evidence as strict admission (`restart/skinny/tranches/sk-v8/SPEC.md:300-314`). |
| W1 CostFacts | After W1, `gate-json --with-cost-facts` rejects missing `CostFacts rule id`, chosen shape, rejected alternatives, evidence source, REDRESS reference, or wave id (`restart/skinny/tranches/sk-v8/SPEC.md:346-366`). |
| W2 typed product | New typed rows require explicit host/API schema facts, generated Track 1, structurally independent Track 2/oracle, full-table maintain, and no digest-stressor promotion (`restart/skinny/tranches/sk-v8/SPEC.md:374-407`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-D-validated-invalidated.md:216-236`). |
| W3 union | Selected parse rows must prove strict validation, comparator evidence, structural cursor work, and admitted tape facts occurred in the measured row. The schema rejects sidecar, view-boundary, post-parse, comparator-only, old offset append, and `tape_vs_tape`-as-consumer proofs (`restart/skinny/tranches/sk-v8/SPEC.md:481-497`). |
| W4 direct guard | Direct rows stay guard rows unless a W4 plan names rows and output-plane proof. Digest rows do not become product-plane proof (`restart/skinny/tranches/sk-v8/SPEC.md:506-524`). |
| W5 Lock 14 | Generic-crate edits must pass public API, grammar branch, primitive/table, role/fact boundary, template/provider, and non-JSON proof scans (`restart/skinny/tranches/sk-v8/SPEC.md:247-275`, `restart/skinny/tranches/sk-v8/SPEC.md:527-552`). |
| W6 close | `skinny/RESULTS.md`, `skinny/REDRESS.md`, `restart/skinny/tranches/sk-v8/SPEC.md`, and `restart/skinny/tranches/sk-v8/HANDOFF.md` agree; no row remains with missing required telemetry or unrouted failure (`restart/skinny/tranches/sk-v8/SPEC.md:554-572`). |
...
These rules preserve the SC-5 adjudication: current `parse_only` rows are substrate-guard non-admission telemetry, not strict SOTA scoreboard rows, even when their sonic-strict delta is positive (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:23-69`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:179-216`).
...
| Did generic code remain grammar-neutral? | Lock 14 scans pass; structural classes and fact ids are opaque generated ordinals, not JSON roles (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-3-union-substrate-design.md:324-399`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-6-lock1-amendment-generalisation.md:338-539`). |

Failure to answer any row-level question is a W3 plan failure, not a reason to relax the schema. `tape_vs_tape` may be added later as W0/W1 gate-binding telemetry only with named owner files, tests, LOC, and rerun budget; it is never a W3 production same-wave consumer (`restart/skinny/tranches/sk-v8/SPEC.md:125-131`, `restart/skinny/tranches/sk-v8/HANDOFF.md:93-96`).
...
W0 fixture expectation: all 38 current main rows are present and telemetry-bound. That is 17 `parse_only`, 17 `direct_to_struct`, and 4 `real_typed_struct` rows (`restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md:17-32`, `skinny/RESULTS.md:5-42`).
...
Residual rows that must not remain unprofiled after W0: `twitter parse_only`, `update_center parse_only`, `apache_builds parse_only`, `github_events parse_only`, `unicode_escapes parse_only`, `y_string_unicode parse_only`, `citm_catalog parse_only`, and `instruments parse_only`, matching the Alpha-E telemetry candidate (`restart/skinny/tranches/sk-v8/research/alpha/alpha-E-candidate-shortlist.md:112-157`).

Sidecar expectations: populated simdjson, yyjson, RapidJSON, and asmjson cells need manifest coverage for corpus identity, binary identity, hardware, build flags, run date/run id, comparator plane, comparator strictness, and freshness. Missing sidecar cells use `absent:<reason>` and cannot be promoted by prose. Alpha-B identifies missing or sparse sidecar coverage by comparator (`restart/skinny/tranches/sk-v8/research/alpha/alpha-B-competitor-deltas.md:96-110`).

Throughput expectation for W0/W1 schema work: every current throughput cell remains within +/-1.0 percent of `SK-V8-open` unless the later plan explicitly names a stricter no-throughput-change gate. A larger move means the wave was not telemetry-only and must be rejected or split (`restart/skinny/tranches/sk-v8/SPEC.md:300-314`, `restart/skinny/tranches/sk-v8/SPEC.md:355-366`).

W3 fixture expectation: any Tier A union plan starts from post-W0 data and names selected parse rows plus guards. SC-3's owner/cost table names structural-heavy diagnostic rows `twitter`, `apache_builds`, `gsoc-2018`, `distinct_values`, and `y_string_unicode`, and number-heavy guards `canada`, `mesh`, and `numbers`; thresholds copied from S-P2 must be recomputed from post-W0 same-run strict rows, not stale sidecars (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-3-union-substrate-design.md:469-480`).

## §4 - Pre-Blocked Routes

The schema must prevent old routes from re-entering through relabeling:

- Do not relabel stale sidecars as same-run strict anchors; REDRESS 77 and 78 made strict/lossy provenance explicit (`skinny/REDRESS.md:2130-2182`).
- Do not turn `K` or optional `S` substrate-guard rows into SOTA admission rows; SC-5 requires demotion plus full residual visibility (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:227-282`).
- Do not treat `tape_vs_tape`, `simd_structural_scan`, sidecar manifests, or comparator-only rows as W3 production consumers (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:251-259`, `restart/skinny/tranches/sk-v8/SPEC.md:450-452`).
- Do not introduce `UnionTape`, `BackendShape::Union`, a new BIR variant, a new BBNF directive, a public substrate API, parser-owned cursor/facts, or a parallel substrate (`restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/hardening/HARDENING-S-P2-V7-CONSOLIDATED.md:44-64`).
- Do not reopen REDRESS 28+33, 50-55, 60-72, 80, 82-84, 88, 89, or historical blocked routes without fresh W0 evidence, same-wave consumer, no-regression gate, REDRESS citation, and challenge acceptance (`restart/skinny/tranches/sk-v8/HANDOFF.md:145-158`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-C-redress-digest.md:81-152`).

## §5 - Sources

- `restart/prompts/ORCHESTRATOR.md:74-88`, `restart/prompts/ORCHESTRATOR.md:118-128`, `restart/prompts/ORCHESTRATOR.md:197-212`.
- `restart/prompts/skinny/PASS-3-SYNTHESIS-PLAN.md:56-63`, `restart/prompts/skinny/PASS-3-SYNTHESIS-PLAN.md:231-260`.
- `restart/prompts/pass-contracts/SKINNY-TRIUMVIRATE.md:65-88`, `restart/prompts/pass-contracts/SKINNY-TRIUMVIRATE.md:165-186`, `restart/prompts/pass-contracts/SKINNY-TRIUMVIRATE.md:190-210`.
- `restart/skinny/tranches/sk-v8/SYNTHESIS.md:89-220`, `restart/skinny/tranches/sk-v8/SPEC.md:17-131`, `restart/skinny/tranches/sk-v8/SPEC.md:247-372`, `restart/skinny/tranches/sk-v8/SPEC.md:413-505`.
- `restart/skinny/tranches/sk-v8/HANDOFF.md:56-132`, `restart/skinny/tranches/sk-v8/HANDOFF.md:133-190`.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-1-offset-tape-teardown.md:272-400`.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-2-two-stage-sota.md:300-349` for Tier A/Tier B measured-path, same-wave consumer, scalar/checkasm, and strict-plane requirements.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-3-union-substrate-design.md:98-204`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-3-union-substrate-design.md:403-520`.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-4-string-plane-gap.md:245-312`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-4-string-plane-gap.md:340-394`.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:23-69`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-5-k-classification-adjudication.md:218-373`.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-6-lock1-amendment-generalisation.md:145-239`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-6-lock1-amendment-generalisation.md:338-539`, `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/SC-6-lock1-amendment-generalisation.md:641-666`.
- `restart/skinny/tranches/sk-v8/research/p2-substrate-ceiling/hardening/HARDENING-S-P2-V7-CONSOLIDATED.md:1-69`.
- `restart/skinny/tranches/sk-v8/research/alpha/alpha-A-results-extraction.md:9-154`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-B-competitor-deltas.md:20-110`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-C-redress-digest.md:44-152`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-D-validated-invalidated.md:15-124`, `restart/skinny/tranches/sk-v8/research/alpha/alpha-E-candidate-shortlist.md:112-175`.
- `skinny/RESULTS.md:1-43`, `skinny/RESULTS.md:151-219`, `skinny/REDRESS.md:2130-2182`, `skinny/REDRESS.md:2466-2504`.
