...
exports, and `xctrace_probe` PMU rows; exact samply version not recorded in
the capture logs.
...
The exact samply command line is not embedded in the saved samply logs, so the
reproducible form below is a command shape, not a verbatim transcript:
...
| `numbers` | 2.27 c/B, 5440 Mbps | 34.5% `number_digit_scan` `scan_digit_run` (skinny/crates/parse-that-regex/src/number/mod.rs:106); 19.6% `dispatch_walk` `dispatch_value` (skinny/crates/runtime/src/grammars/json/generated.rs:47); 8.5% `array_walk` `consume_array_next` (skinny/crates/runtime/src/grammars/json/generated.rs:348) | 2.32 c/B, 4625 Mbps | 36.0% `number_digit_scan` `scan_digit_run` (skinny/crates/parse-that-regex/src/number/mod.rs:106); 23.1% `dispatch_walk` `track2::Parser::parse_value_at` (skinny/crates/bbnf-bench/src/track2/json.rs:53); 9.3% `number_scan` `match_number_span_from_first` (skinny/crates/parse-that-regex/src/number/mod.rs:38) |
...
- W3 union/substrate remains pre-blocked. REDRESS 96 and REDRESS 97 are
  falsification evidence for the union-substrate thesis, and REDRESS 98 records
  the route retirement. This profile surfaces no new single-substrate ceiling
  that would reopen that family.
...
- `y_string_unicode` is the worst PMU cycles-per-byte row in this capture
  ...
  signal, not permission to add a sidecar scanner.
