...
|---:|---|---|---|
| P1 | `rg -n 'H\.W5' restart/locks/LOCKS.md` | zero hits | **CLOSED** |
...
| R1 | `rg -n 'Lock 4\|Lock 10' restart/audit/pass-1-substrate/PASS-1.md` | positive at `:73` (Lock 4 amendment cite for higher-rank polymorphism + DK13 + GADT-hidden + closure-by-`&'i` substrate ratification, with `restart/locks/LOCKS.md:40` cross-reference) and `:75` (Lock 10 amendment cite for 6-directive grammar surface + first-class function values, with `restart/locks/LOCKS.md:52` cross-reference) | **CLOSED** |
