...
| R4 | Optional Lock 4 rationale hygiene for egglog-style fusion pressure. | `restart/locks/LOCKS.md`, closed by R3/R4 follow-up. | Closed; Lock 4 now names egglog-style fusion as a known SOTA pressure while preserving per-domain ownership, bridge boundaries, and independent gates. | Lock rationale explains why bridge-vs-fusion survives egglog pressure without changing settled Lock 4 semantics. | No |
