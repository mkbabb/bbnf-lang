...
| G-9 | PROVED (inflection; CSS ratio DIRECTIONAL not re-locked) | We are AT the inflection point: >SOTA is real, the impl is hand-written/forked/replicated; a naive grammar-walk regresses to lightningcss — >SOTA survives ONLY via (a)-(d)-gated named primitives, never a paper-close. INLINE CAVEAT (CH6-F3): the JSON half is bench-row-backed (`RESULTS.md:5-25`); the CSS 1.9–3.3× half is DIRECTIONAL (loadavg 4.35, NOT re-locked, U-4 `1D:239-243`) — the inflection thesis stands, but the CSS ratio is not a re-locked bench row. | `SYNTHESIS-AUDIT-OVERFIT.md:23,36-39`; `SYNTHESIS-RESEARCH.md:215-266`; CSS directional caveat U-4 `1D:239-243` |
