...
| AP-020 | NEW (V2 fold of V1 CH5-004). CSS comparator evidence at `bbnf-bench/src/nonjson_css_l4.rs` routes lightningcss facts through a source-sidecar plane that must be classified — same fenced treatment as 1A gives CSS fact streams — to prevent the sidecar from drifting into runtime substrate authority. | Live `bbnf-bench/src/nonjson_css_l4.rs:222,234` route comparator evidence through `fixture_sidecar_facts`; `:299` writes a `same-plane-source-sidecar` artifact; `:504` validates hardcoded fixture spans inside `fixture_sidecar_facts`. Companion classification: AP-009 already records the same file at `:222-234,298-303` as comparator-sidecar evidence; AP-020 lifts the sidecar-as-anti-pattern row separately so it is not collapsed into AP-009's classification-only entry. | comparator-sidecar coupling; fence required (non-runtime-authoritative; not retained document identity) | 40-120 LOC fencing/classification; medium-high | Fence as comparator-only evidence plane; never accept as runtime substrate; pair with 1A CSS fact-stream fencing; cite-target for any future "runtime CSS substrate" admit. |
...
| AP-020 | 40-120 LOC fence/classification | medium-high | CSS evidence-accounting wave (co-wave with AP-009) | 160 LOC | CSS comparator-sidecar fence consumer | `skinny/crates/bbnf-bench/src/nonjson_css_l4.rs:222,234,299,504` |
