...
Backend IR shape: executable plan nodes. ARCH §7.2 owns the authoritative variant set; PASS-1 carries the listing for substrate-side cross-reference. Proposed variants are `Entry`, `Seq`, `Alt` (`mode: Dispatch | Speculative` discriminator), `RepeatLoop`, `OptionalBranch`, `ByteLiteral`, `RegexProgram`, `SimdScan`, `PrattSpine`, `CallRule`, `CallHost` (chains express as `Seq`-of-`CallHost` rather than a separate `HostChain` variant), `LayoutScope` (`kind: Push | Pop` discriminator), `ErrorRecover`, `SpanMark`, `TapeEmit`, `DirectBuild`, `ValueProject`, `PathEval`, and `DebugMark` — three semantically-redundant pair-collapses (Alt-mode, LayoutScope-kind, CallHost-chain-as-Seq) reduce the prior 22-variant alphabet to 19 without losing lowering distinct-ness.
...
Per-backend lowering obligations, by variant family — Rust V1 and WASM V1 are in scope; TS production is deferred per Q28 but the BIR shape supports TS lower without retrofit when scope opens. PASS-1's per-backend obligations table is consumed by the V1 `RustBackend: Backend` impl per ARCH §7.5 (`restart/ARCHITECTURE.md:1067-1144`); future `WasmBackend` and `TsBackend` impls land V2 without re-architecting BIR or PASS-1's substrate, since the trait surface (and not the obligations table) is the realization layer that grows V2 columns mechanically.
...
|---|---|---|---|
| Host-fn dispatch from grammar | Backend host tables consume typed metadata and the `Seq`-of-`CallHost` chain shape. | host signature inference must be complete | PASS-3 host docs and WASM host diagnostics |
