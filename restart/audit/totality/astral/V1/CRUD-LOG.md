...
| CRUD-5 | SKINNY CORPUS | Update | `restart/skinny/{BENCH,COMPILER,HARDENING,INDEX,SUBSTRATE,WORKSPACE}.md` | pending | pending | CSS row, non-JSON telemetry, Lock 14/16, zero-orphan alignment. |
| CRUD-6 | AUDIT + CLEANUP | Read no-op verification | no files touched beyond this log | pending | pending | Empty delete/archive target inventory for V1; no legacy doc nuke. |
