...
pub fn run(grammar: &str) -> Result<()> {
    for file in render_runtime_files(grammar)? {
...
/// Check one simple root runtime tree without writing generated files.
pub fn check(grammar: &str) -> Result<Vec<PathBuf>> {
    let files = render_runtime_files(grammar)?;
    ...
    if !drift.is_empty() {
        bail!(
            "root runtime projection `{grammar}` has generated drift; rerun `cargo xtask {}`:\n{}",
            regen_command(grammar)?,
            drift
                .iter()
...
fn render_runtime_files(grammar: &str) -> Result<Vec<RenderedRuntimeFile>> {
    ...
    let header = generated_header(&projection)?;
    ...
    files
        ...
        .map(|(relative, source)| {
            ...
            Ok(RenderedRuntimeFile {
                path,
                source: format!("{header}{formatted}"),
            })
        })
        .collect()
}
...
fn validate_projection(projection: &RuntimeProjection) -> Result<()> {
    if projection.module_name != projection.grammar {
        bail!(
            "simple runtime projection `{}` has module_name `{}`",
            projection.grammar,
    ...
    let expected_output_dir = format!("crates/core/src/runtime/{}", projection.module_name);
    if projection.output_dir != expected_output_dir {
        bail!(
            "simple runtime projection `{}` has output_dir `{}`, expected `{expected_output_dir}`",
            projection.grammar,
...
fn generated_header(projection: &RuntimeProjection) -> Result<String> {
    Ok(format!(
        "// @generated by xtask {}; do not edit by hand.\n",
        regen_command(&projection.grammar)?
    ))
}

fn regen_command(grammar: &str) -> Result<&'static str> {
    Ok(match grammar {
        "math" => "regen-math",
        "csv" => "regen-csv",
        "bbnf" => "regen-bbnf",
        "bnf" => "regen-bnf",
        "ebnf" => "regen-ebnf",
        "css_pretty" => "regen-css-pretty",
        "google_sheets" => "regen-google-sheets",
        "json" => "regen-json",
        other => bail!("unknown simple root runtime projection `{other}`"),
    })
}
