...
| H.W1 (typed event cursor over tape projection — load-bearing) | Cost model in `passes::recognizers` derives `LayoutFacts.backend_shape[rule_id]` from existing Grammar IR facts (first-set disjointness, output mode, recovery, host decode, layout scope) per Lock 10 auto-detection mandate. Lowerer at `crates/codegen/src/lower/rust.rs` emits typed `EventCursor` consumption for `OffsetTape`/`EventTape` rules (`event.byte()` instead of `source[pos]`); direct field writes for `SinkOnly`; byte-position fallback for `EagerTape`. Whitespace boundary handling moves into the event cursor (no per-function `skip_ws`). String/number primitive source reads remain inside `parse-that/string` and `parse-that/number` as the only legal source-byte rescans. Parse-time retained projection aux side tables are explicitly rejected by SK-V5 redress item 50; the cursor must consume the existing projection instead of writing another retained column. No new BIR variant; no new BBNF directive. | `parse_value_at` falls below 20% self-time on the current expanded parse blockers (`twitter`, `random`, `unicode_mixed`, `unicode_basic`), or the wave records the exact remaining leaf that replaced it; Class A corpora stay above their S anchors; Unicode rows lose string/Unicode projection as the dominant samply leaf. |
