...
- **`collapse_simple_spans`**: `GrammarIR::collapse_simple_spans: bool`. When true (prettify disabled), Seq nodes where all children are simple Span leaves collapse to a single Span, eliminating arena allocation. Gated to prevent type cascading in prettify grammars.
