...
### §1.2 — Wave manifest (12 waves; ≤12 cap per S-P3 dispatch §2)
...
| W1 | C-5 PRUNE-1 | revert 5 JSON `parse_only` (W14.1–.5) + 6 JSON `direct` + 11 JSON `typed` audit-falsified rows | R3.PRUNE-1 | — |
| W2 | C-2 | comparator rebind (3 plane-correct strict comparators) + per-iter equality oracle in the timing region | R1, R2 | — |
| W3 | C-3 | `cargo xtask regen-css` pipeline + `skinny/corpora/css-l4-sk-v14/` (Bootstrap + Tailwind + Material + Animate; ~960 KB) | R4, R5 | — |
...
| W9 | R7-direct | re-admit each JSON `direct_to_struct` row vs sonic-rs strict per-corpus struct deser | R7 (direct) | 17 (per corpus) |
| W10 | R7-typed | re-admit each JSON `real_typed_struct` row vs per-corpus typed struct deser | R7 (typed) | 17 (per corpus) |
| W11 | R8 | stand up distinct `parse_only` code path in `generated_json` (no full-tape build); wire to sonic-rs Skipper-class comparator; admit | R8 | 17 (per corpus) |

R10 (indefatigable close) is a bracket-level condition (SYNTHESIS §0.1 + §6 close posture), not a wave; SK-V14 closes when W0–W11 dispositions exit + every row family meets the §0.1 bar OR carries architectural-block proof.
...
Throughput floors for the W2 re-baseline are computed post-W2 from rebound comparators; pre-W2 Mbps numbers in current `RESULTS.md` (the SK-V13 fake-admit baseline) MAY NOT be used as comparator references after W2 closes — they are AUDIT-FALSIFIED per SYNTHESIS §1.2.
...
### §2.1 — W1: C-5 PRUNE-1 (JSON audit-falsified revert)

**Same-wave consumer**: REDRESS scribe consumes the revert (29 new row-keyed REDRESS entries per SYNTHESIS §3 C-5 row); `ROLLING-SOTA-DELTA.md` rebase consumes the revert.

**Named corpus rows it must LIFT**: NONE. W1 is a PRUNE wave (admit-baseline restoration).
...
**Mbps threshold (delta vs SK-V14-open)**: NONE for W1 (revert wave); each of the 22 rows above flips to `audit_overlay_verdict=AUDIT-FALSIFIED`, Outcome demoted, Verdict NO-GO.

**Full-table maintain budget**: zero perf regression on any row not in the revert set; ±1.0 % of W0 `SK-V14-open` Track 1 / Track 2 cells for all 51 + 24 rows.

**Exit gate** (measurable from bench):
1. `ROLLING-SOTA-DELTA.md` shows `JSON parse_only: 0/17`, `JSON direct: 0/17`, `JSON typed: 0/17` per SYNTHESIS §1.3.
2. `skinny/REDRESS.md` carries 22 new row-keyed entries citing `audit-overfit/validation/v2-json-validation.md §1`–§4 + `v6-comparator-integrity.md §1, §3` per SYNTHESIS §3 C-5 row.
3. Every reverted row's `audit_overlay_verdict` column = `AUDIT-FALSIFIED` and cites the validation-pack §reference (per SYNTHESIS §2 column rule).
4. `xtask gate-json` accepts the 22-row revert manifest end-to-end.
5. Non-target rows: ±1.0 % vs `SK-V14-open`.

**Revert protocol**: revert the PRUNE-1 commit set as one slice; restore `ROLLING-SOTA-DELTA.md` to pre-PRUNE-1 state; preserve PRUNE-1 research artefacts under `restart/skinny/tranches/sk-v14/research/`; record REDRESS naming the failed revert (e.g., regression detected in non-target row).

**Comparator strict-vs-strict plane (R1)**: not applicable to PRUNE-1 itself (revert wave); R1 binding is consumed by the columns the W0 populates. PRUNE-1 reverts AUDIT-FALSIFIED admits that were granted under the WRONG comparator (`sonic_rs::from_slice::<Value>`); strict comparator rebind lands in W2.

**Per-iter equality column (R2)**: `xtask gate-json` continues to reject the 22 reverted rows on the per-iter equality column being empty (because those rows had startup-only checksum, not per-iter — fail mechanism unchanged from W0).

### §2.2 — W2: C-2 — R1 + R2 (comparator rebind + per-iter equality oracle)

**Same-wave consumer**: bench harness consumes the rebound comparators on every named JSON row; `xtask gate-json` enforces the schema. Per SYNTHESIS §3 C-2 row (line 272).

**Named corpus rows it must LIFT**: NONE strictly admit at W2 close — W2 stands up infrastructure. ALL 51 JSON cells (17 corpora × 3 planes per SYNTHESIS §0.1) MUST acquire populated rebound-comparator columns + per-iter equality columns at W2 close. Per-row Mbps comparison vs `SK-V14-open` is re-baselined.
...
- 51 JSON cells: per-row Track 1 Mbps ≥ 0.95 × `SK-V14-open` Track 1 Mbps; Track 2 Mbps ≥ 0.95 × `SK-V14-open` Track 2 Mbps.
- 24 CSS L4 cells: no perf budget interaction (W2 touches JSON harness only); ±1.0 % vs `SK-V14-open` (default frozen-surface).
...
3. The harness inner-loop call site (cite the file:line in W2 plan) executes equality INSIDE the timed region; the W2 plan quotes the harness line per ORCHESTRATOR-PROMPT R2.
4. Per-row Track 1 / Track 2 Mbps within 0.95 × `SK-V14-open` for all 51 JSON cells.
5. 24 CSS L4 cells within ±1.0 % vs `SK-V14-open`.

**Revert protocol**: revert C-2 comparator rebind + per-iter equality slice as one commit; restore prior `sonic_rs::from_slice::<Value>` only with explicit `audit_overlay_verdict=AUDIT-FALSIFIED` carry-over; record REDRESS naming the failed comparator rebind or harness regression.

**Comparator strict-vs-strict plane (R1)**: W2 IS the R1 binding wave. Each of the three JSON planes acquires a plane-correct strict comparator per the table in §1.3 above. `xtask gate-json` rejects any row admitting under a comparator that does work asymmetric to Track 1 (per SYNTHESIS §2 `comparator_plane` column rule).

**Per-iter equality column (R2)**: W2 IS the R2 binding wave. Bench harness emits `per_iter_equality` column per iteration; `xtask gate-json` rejects rows whose equality column is empty (SYNTHESIS §0.3 R2 verbatim).

### §2.3 — W3: C-3 — R4 + R5 (regen-css xtask + production corpora)

**Same-wave consumer**: runtime regenerated from the 15 `.bbnf` files in the same wave; bench rows wired to the new corpora per SYNTHESIS §3 C-3 row (line 273). MUST land BEFORE PRUNE-2 per audit-overfit §2.1.
...
**Exit gate** (measurable from bench):
1. `cargo xtask regen-css` exists and is invokable from `skinny/xtask/src/main.rs` (cite the registered command path in W3 plan).
...
4. `du -sh skinny/corpora/css-l4-sk-v14` reports ≥ 800 KB (R5 floor).
5. `skinny/corpora/css-l4-sk-v14/` contains Bootstrap, Tailwind, Material, Animate fixtures (each named in the W3 plan; per ORCHESTRATOR-PROMPT R5).
6. CH7 round-trip rule trigger armed: any CSS feature whose claimed Mbps exceeds the same-plane SOTA comparator by ≥ 50× inherits the W10.3 nested_layout 124× round-trip-rule (per SYNTHESIS §0.4 P-1).

**Revert protocol**: revert W3 xtask + corpora slice as one commit; restore prior `include_str!()` template paths only with explicit `audit_overlay_verdict=AUDIT-FALSIFIED` carry-over; record REDRESS naming the failed xtask round-trip or corpora-floor miss.
...
**Per-iter equality column (R2)**: CSS rows acquire `per_iter_equality` column per iter at W3 wiring; `xtask gate-json` rejects CSS rows whose equality column is empty (same rule as JSON per SYNTHESIS §0.3 R2).
...
3. `git grep -l '@generated' crates/core/src/runtime/css_l4/` returns only files produced by `cargo xtask regen-css` (R4 from W3 — sequencing dep enforced).
...
**Comparator strict-vs-strict plane (R1)**: CSS rows bind `comparator_plane=lightningcss full-parse` (no change from W3 wiring); revert preserves the W3 R1 column binding.

**Per-iter equality column (R2)**: revert preserves W3 R2 column wiring; reverted rows continue to fail per-iter equality on the OLD admit data (they never had it; the admit was gate-relabel only per P-4).
...
**Comparator strict-vs-strict plane (R1)**: refactor preserves W2 R1 column bindings; no comparator behavior change.

**Per-iter equality column (R2)**: refactor preserves W2 R2 column bindings; per-iter equality continues to run for every row.
...
3. `cargo xtask regen-{grammar}` round-trips clean for the refactored grammar (R4 from W3 generalizes per SYNTHESIS §3 C-3 row "first instance of the `regen-{grammar}` family").
...
**Comparator strict-vs-strict plane (R1)**: refactor preserves W2 R1 column bindings unchanged.

**Per-iter equality column (R2)**: refactor preserves W2 R2 column bindings unchanged; per-iter equality continues per row.
...
**Comparator strict-vs-strict plane (R1)**: preserves W2 R1 column bindings unchanged.

**Per-iter equality column (R2)**: preserves W2 R2 column bindings unchanged.

### §2.8 — W8: R6 CSS L4 re-admit (24 features)

**Same-wave consumer**: each CSS L4 feature's grammar-derived parser (from W3 R4 regen + W6.2 + W6.3 + W6.4) feeds the W8 admit gate per feature.
...
7. Same-wave consumer present: emitted grammar-derived parser from W3/W6 is the Track 1 path; Track 2 is independent oracle (cssparser); `xtask gate-json` rejects any row whose `track2_entry_point` shares ancestor with `track1_entry_point` beyond public `Tape` / `OffsetFlags`.
...
**Full-table maintain budget**:
- 17 typed cells (W10 not yet admitted): ±1.0 % vs `SK-V14-open` (W9 touches direct plane only on the harness layer; underlying parser unchanged).
- 17 parse_only cells (W11 not yet admitted): ±1.0 % vs `SK-V14-open`.
...
6. Per SYNTHESIS §3 C-2 row + §2.2 W2 exit gate: equality column non-empty AND PASS per iter.
7. Same-wave consumer present: generated `direct_to_struct` parser (from W3-derived `regen-{grammar}` family OR existing W6.1 emitted json runtime) is Track 1 path; Track 2 is independent oracle.
...
**Revert protocol per corpus**: revert the per-corpus admit; row stays at W1 `audit_overlay_verdict=AUDIT-FALSIFIED` posture; REDRESS records the threshold miss.

**Comparator strict-vs-strict plane (R1)**: `comparator_plane=<corpus>::strict_struct_deser` (per-corpus typed deser); `xtask gate-json` rejects any direct row binding `sonic_rs::from_slice::<Value>` or any other eager-DOM comparator per P-2 pre-block.

**Per-iter equality column (R2)**: per-iter equality oracle: bench harness reads back the struct and digest-compares per iter; rejects empty cells.

### §2.10 — W10: R7-typed JSON `real_typed_struct` re-admit (17 corpora)

**Same-wave consumer**: bench harness consumes the W2 rebound `per-corpus typed struct deser` comparator on every typed row.

**Named corpus rows it must LIFT**: all 17 JSON `real_typed_struct` rows (verbatim from `skinny/RESULTS.md`): `json/twitter/real_typed_struct/main`, `json/citm_catalog/real_typed_struct/main`, `json/canada/real_typed_struct/main`, `json/apache_builds/real_typed_struct/main`, `json/github_events/real_typed_struct/main`, `json/update_center/real_typed_struct/main`, `json/mesh/real_typed_struct/main`, `json/random/real_typed_struct/main`, `json/gsoc-2018/real_typed_struct/main`, `json/marine_ik/real_typed_struct/main`, `json/instruments/real_typed_struct/main`, `json/numbers/real_typed_struct/main`, `json/unicode_mixed/real_typed_struct/main`, `json/unicode_escapes/real_typed_struct/main`, `json/unicode_basic/real_typed_struct/main`, `json/distinct_values/real_typed_struct/main`, `json/y_string_unicode/real_typed_struct/main`.

(Note: 2 of these — `json/canada/real_typed_struct/main`, `json/gsoc-2018/real_typed_struct/main`, `json/unicode_mixed/real_typed_struct/main`, `json/unicode_escapes/real_typed_struct/main`, `json/distinct_values/real_typed_struct/main`, `json/y_string_unicode/real_typed_struct/main` — are NOT in current `RESULTS.md` typed plane and represent fresh admit attempts per SYNTHESIS §0.1 "all 17 reopen".)
...
**Full-table maintain budget**:
- 17 direct cells (W9 outcome dependent): each ≥ 0.98 × `SK-V14-open` per row (or post-W9 admitted floor).
- 17 parse_only cells: ±1.0 % vs `SK-V14-open` (W11 not yet admitted).
...
**Revert protocol per corpus**: revert per-corpus admit; row stays at W1 AUDIT-FALSIFIED posture; REDRESS records threshold miss.

**Comparator strict-vs-strict plane (R1)**: `comparator_plane=<corpus>::typed_strict_struct_deser`; rejects any typed row binding eager DOM per P-2.

**Per-iter equality column (R2)**: per-iter equality: typed struct equality comparison per iter inside timing region; rejects empty cells.

### §2.11 — W11: R8 JSON `parse_only` distinct path + admit (17 corpora)
...
- 17 typed cells (W10 outcome dependent): each ≥ 0.98 × post-W10 floor.
- 24 CSS L4 cells: ±1.0 % vs `SK-V14-open`.
...
4. Distinct `parse_only` code path EXISTS in `generated_json` (cite path:line in W11 plan; e.g., `crates/core/src/runtime/json/parse_only.rs`) — NOT a flag-gated branch of the full-tape parser.
...
**Revert protocol for W11 as a whole**: any row that REOPENs blocks SK-V14 close per §0.1 indefatigable clause; SK-V15 opens.
...
- W1: row-revert gate (commit-evidence + ROLLING-SOTA-DELTA cell-state, measurable).
- W2: per-row comparator/equality column + cost-overhead gate (Mbps measurable).
- W3: round-trip xtask exit code + `du` corpora floor (measurable).
...
- W9 + 17 corpora: Mbps threshold vs per-corpus sonic-rs strict struct deser + per-iter equality (measurable).
- W10 + 17 corpora: Mbps threshold vs per-corpus typed struct deser + per-iter equality (measurable).
- W11 + 17 corpora: Mbps threshold vs sonic_rs::Skipper + per-iter equality + no-full-tape `cargo asm` evidence (measurable).
...
- **P-1** — Fake `@generated` header on hand-written templates. W3 + W4 + W5 + W6 + W8 + every CSS-touching wave must NOT re-emit hand-patched templates carrying `@generated`. The CH7 round-trip rule fires at W3 + W8 on any feature whose claimed Mbps exceeds same-plane SOTA by ≥ 50×.
- **P-2** — `sonic_rs::from_slice::<Value>` mislabelled as strict comparator. W2 + W9 + W10 + W11 must NOT re-bind eager-DOM comparator under any plane label.
...
- **P-4** — Gate-relabel as admit. W8 + W9 + W10 + W11 admit requires a parser/codegen source delta cited per row + measurement evidence per REDRESS — NOT a gate.rs / report.rs / lock14_baseline.rs touch only.
...
- **P-7** — Track 1 ≡ Track 2 dishonesty. EVERY admit wave (W8 + W9 + W10 + W11) must keep Track 1 (generated) structurally distinct from Track 2 (independent oracle); `xtask gate-json` rejects rows where `track2_entry_point` shares ancestor with Track 1 entry point beyond public `Tape` / `OffsetFlags` per SYNTHESIS §2 column rule.
...
| W1 | none (revert wave; CREATES new REDRESS entries 22) | — |
| W2 | REDRESS 119, 120 (HISTORY only per HANDOFF.md:131); REDRESS 88, 89 (PMULL prefix-XOR default hot body, CTZ/bulk production consumer as performance evidence — comparator rebind is comparator, not perf claim) | sk-v8/SPEC.md:794-797; sk-v14/HANDOFF.md:131 |
| W3 | REDRESS 28+33 (Class A NEON/TBL tiny-string wiring as parse close — wave is xtask + corpora, not parse close); REDRESS 96-98 (carried-forward CSS-template fake `@generated` route per S-P3 dispatch) | sk-v8/SPEC.md:779-780 |
...
| W9 | REDRESS 50-55 (visitor / aux tables); REDRESS 60-72 (string-boundary / direct source-hook); REDRESS 80, 82-84; REDRESS 88, 89 (perf evidence vs admit gate); REDRESS 119, 120 (HISTORY); REDRESS 126 | sk-v8/SPEC.md:785-797; sk-v14 dispatch §1 |
| W10 | REDRESS 50-55, 60-72, 80, 82-84, 88, 89, 119, 120, 126 (same as W9 — typed-plane shares pre-block surface) | sk-v8/SPEC.md:785-797 |
| W11 | REDRESS 50-55, 60-72, 80, 82-84, 88, 89, 119, 120, 126; ADDITIONALLY REDRESS 49 (no-allocation visitor as parse_only path — must NOT re-open; the W11 distinct parse_only path is a CODEGEN delta, not a visitor) | sk-v8/SPEC.md:783-797 |
