...
CSS L4, BBNF, Sheets, and the five-grammar template cohort (BNF, CSV, EBNF, CSS Pretty, Math) remain on the existing `OpenFrame` emitter path until BB; BA does not migrate them, lest the layered re-org and god-module split cascade through nine grammars at once. Per `docs/tranches/BA/audit/W5-substrate-identity-decision.md` the substrate-identity decision is option (b): JSON only. Lock 1 is **deferred-with-receiver** at BA close, with BB.W1a (CSS L4), BB.W1b (BBNF), BB.W1c (Sheets), BB.W2 (cohort) as the per-grammar receivers. The thesis is surgical excision begotten of demonstration: prove the path on JSON, retire the substrate that begets the 86.07% `Vec<OpenFrame>::clone` pathology (`audit/RESTART-SKETCH-2026-05-03.md:154-220`), and hand BB an emitter substrate that generalises by mechanism.
...
| ID | Gate | SOTA anchor |
|---|---|---|
| BA-G1 | `JsonParser::parse(twitter.json)` ≤ 400 µs on M1 Pro, beating sonic-rs's 436 µs by ≥ 8% | `audit/SOTA-2026-05-03.md:50-58` (sonic-rs `benchmark_aarch64` twitter row; simd-json 424 µs) |

### Engineering gates (non-SOTA)
...
| BA-G10 | Generated-file LOC accounting: per-grammar pre-/post-BA LOC table emitted; per-wave windows (W1: ±0.5%; W3: unchanged; W4: `json.rs ≤ 3,700`, `bbnf.rs ≤ 22,000`, `css_l4.rs ≤ 110,000`, aggregate ≤ +5% from W2; W5: `json.rs ≤ 2,200`); no overflow without justified cause | Lane 06 generated-code budget |

## Wave summary

The wave structure splits per directive §5: W3 → W3a (path triplet rename) + W3b (path-core extraction) + W3c (runtime relocation); W4 → W4a (private parse core + cursor elision) + W4b (public wrappers) + W4c (parse_with deletion, formerly W3.M5).
...
| BA.W5 | JSON direct-to-struct codegen first cut. Direct-projection emit replaces `OpenFrame` stack + `Vec<OpenFrame>::clone` for JSON only per `docs/tranches/BA/audit/W5-generated-parser-shape.md`. Typed enum + per-rule generated `parse_<rule>(...)` functions; byte-disjoint Alt emits direct `match first { ... }` without speculative checkpoint. `OpenFrame` deleted from JSON path only. CSS L4, BBNF, Sheets, cohort STAY on `OpenFrame` (deferred-with-receiver per `docs/tranches/BA/audit/W5-substrate-identity-decision.md`). | Lock 1 (JSON path); Lock 3 (one parse impl); Lock 9 (slice-borrow primary). | BA-G1 (≤ 400 µs); BA-G2 (≤ 2 heap allocs/parse); samply post-BA shows `Vec<OpenFrame>::clone` retired from JSON profile. `rg -n 'enum OpenFrame' crates/core/src/runtime/json/` returns 0 (per surgery #1). |
| BA.W6 | BA close: PROGRESS / FINAL; lock cross-reference table verified; per-grammar generated-LOC table; carry ledger to BB.W0/W1a/W1b/W1c/W2 named explicitly per surgery #24. | Lock-honoured-or-deferred-with-receiver at every gate; carry-tags BA→BB explicit. | `cargo nextest run -p bbnf -p bbnf-ir -p bbnf-analysis` 100% pass for BA-owned surfaces; `substrate_audit` green; BA close artefacts emitted. |
...
Per surgery #24 ("replace `BB.W0/W1` with `BB.W1 for CSS L4/BBNF/Sheets; BB.W2 for the five-grammar cohort`"), and given the substrate-identity decision option (b), the BB receivers are sub-waved per-grammar.

| Tag | Owner wave | Receiving wave | Description | Receiving gate |
|---|---|---|---|---|
| BA→BB.C1a | BA.W5 | BB.W1a | Direct-to-struct codegen path lands for JSON; BB.W1a extends to CSS L4. | `rg -n 'enum OpenFrame' crates/core/src/runtime/css_l4/` returns 0 |
| BA→BB.C1b | BA.W5 | BB.W1b | Direct-to-struct extends to BBNF. | `rg -n 'enum OpenFrame' crates/core/src/runtime/bbnf/` returns 0 |
| BA→BB.C1c | BA.W5 | BB.W1c | Direct-to-struct extends to Google Sheets. | `rg -n 'enum OpenFrame' crates/core/src/runtime/google_sheets/` returns 0 |
| BA→BB.C1d | BA.W5 | BB.W2 | Direct-to-struct extends to the five-grammar cohort (BNF, CSV, EBNF, CSS Pretty, Math) via cohort template. | `rg -n 'enum OpenFrame' crates/core/src/runtime/{bnf,csv,ebnf,css_pretty,math}/` returns 0 |
...
| BA→BB.C3 | BA.W4b | BB.W2 | Cursor-unified `parse_with` + `__EAGER_EMPTY_PATH` substrate; BB extends to all grammars at BB.W2. | `rg -n 'pub fn parse_with' crates/core/src/grammar/generated/` returns ≥ 9 (already met at BA close; BB.W2 verifies post-cohort regen) |
...
| BA→BB.C7 (Lock 10 deferral) | n/a | BB.W3 | Pratt + SIMD auto-detected. | BB-G6 |
...
| BA→BC.C2 | BA.W5 | BC.W0/BC.W1 | Direct-to-struct emitter pattern (one IR walker, leaf emission per backend) is the precursor to BC's `Emitter` trait formalisation across Rust/TS/WASM. |
...
| Lock | Wave | Notes |
|---|---|---|
| L1. Tape + columnar dead | W0 (residue scrub); W5 (JSON OpenFrame retiral) | **Deferred-with-receiver**: CSS L4 → BB.W1a; BBNF → BB.W1b; Sheets → BB.W1c; cohort → BB.W2 (per `docs/tranches/BA/audit/W5-substrate-identity-decision.md`). The `enum OpenFrame` declarations at `crates/core/src/runtime/{css_l4,bbnf,google_sheets,bnf,csv,ebnf,css_pretty,math}/builder.rs` survive BA close. |
...
| L3. Cursor + byte-skip unified | W4a (private parse core + eager elision); W4b (public wrappers); W4c (legacy deletion) | `__EAGER_EMPTY_PATH` LazyLock at BA.W4a is the unification point; W4b reroutes `parse` and `Document::get<T>`; W4c retires the four legacy `parse_with.rs` files. BB.W2 extends to all grammars via carry BA→BB.C3. |
...
| L5. IR + per-backend lower | W5 demonstrates the pattern (Rust emitter consumes IR-shape) | BB.W1a/W1b/W1c/W2 generalise across grammars; BC.W0 codifies the IR contract. The Rust emitter consumes IR-shape in BA.W5; BC.W0 documents the contract. |
...
| L8. Surpass sonic-rs / simdjson / lightning-css | G1 (≤ 400 µs twitter beating sonic-rs 436 µs) | The only parse-throughput gate at BA close is BA-G1; BA-G9's `get<T>` ratio is internal (non-SOTA) until SOTA admits sonic-rs `get_unchecked` measurement. BB and BC tighten the gates further. |
| L9. Slice-borrow primary; bumpalo + owned escape hatches | W5 (JSON parse returns slice-borrowed `JsonValue<'p>`) | Default surface is `&'i str` slice + `Cow<'i, str>` per `audit/SOTA-2026-05-03.md:122-123`. The arena is opaque collection storage (per W5.M0 emission shape); bumpalo opt-in via `parse_in` routes to BB.W4. |
...
| L13. No god directories; cohesive encapsulation at every level | W0 (9-directory layered re-org); W2 (god-module splits, 23 → 0 production source); W3a/b/c (path crate triplet) | sonic-rs / lightningcss / simdjson cohesion is the standard. Files >500 LOC outside `generated/` (under `crates/*/src/`) forbidden after BA.W2 per `docs/tranches/BA/audit/W2-file-size-distribution.md`. >10-children dirs mixing concerns forbidden after BA.W0. Test fixtures DEFER per CENSUS:122. |
...
| BA.W5 direct-to-struct path regresses CSS L4 / Sheets / BBNF (whose paths are not migrated per option (b)) | High | BA.W5 explicitly does NOT migrate non-JSON grammars; metadata-driven `EmitStrategy::for_grammar` retains `OpenFrame` for non-JSON until BB.W1a/W1b/W1c/W2. The substrate-without-consumer concern is mitigated because BA.W5's consumer (JSON) lands in the SAME wave. |
...
| BA.W5 | JSON direct-to-struct | `json.rs ≤ 2,200` (post-W4 → post-W5 net); other grammars unchanged |
...
|---|---|---|---|
| 1 | Delete CSS L4/BBNF/Sheets `OpenFrame` preservation OR move to BA.W6 | BA.W5 (JSON only; `rg -n 'enum OpenFrame' crates/core/src/runtime/json/` returns 0) | W5 closer-gate |
| 2 | Lock 1 deferred-with-receiver (option (b)) | BA.md §13-Lock + `docs/tranches/BA/audit/W5-substrate-identity-decision.md` | BA.W6.M1 lock cross-reference |
...
| 11 | "Every parse-throughput gate cites..." + non-SOTA engineering table | BA.md §Hard gates rewrite | BA.md §Hard gates |
| 12 | BA-G9 marked non-SOTA OR add sonic-rs `get_unchecked` measurement | BA-G9 marked non-SOTA | BA.md §Hard gates row BA-G9 |
| 15 | CSS host fns to per-grammar host namespace | BA.W0.M1 (relocate to `crates/core/src/grammar/host/css_l4/css_types.rs`) | W0.M1 exit-criteria |
...
| 17 | Inverse-layout-audit gate | BA.W2.M5 (added in W2.md) | W2.M5 exit-criteria |
| 18 | Fail-explicit table | BA.W2.M4 + `docs/tranches/BA/audit/W2-fail-explicit-table.md` | W2.M4 exit-criteria |
| 19 | BBNF aggregator `pub use bbnf::*` deletion | BA.W2.M4 + `docs/tranches/BA/audit/W6-bbnf-aggregator-disposition.md` | W2.M4 exit-criteria |
| 20 | Generated LOC gates per W1/W3/W4 | per-wave §1 / §3 / §7 windows in W1, W3a/b/c, W4a/b/c | per-wave gates |
| 24 | BA.md C1 receiver = BB.W1 (CSS L4/BBNF/Sheets) + BB.W2 (cohort) | BA.md §Carry-tags TO BB sub-waved per-grammar | §Carry-tags TO BB |
...
Hereupon BA opens. The substrate is the typed-enum + slice-borrow; the demonstration grammar is JSON; the carry to BB is the renamed canonical surface, the cursor-unified parse, the consolidated path-crate triplet, the grammar-agnostic IR, the direct-to-struct emitter scaffolding (JSON only), and the four deferred-with-receiver locks (Lock 1 per-grammar; Locks 4, 10, 11). The 13 locks are settled; BA verifies the BA-owned cells in eleven waves (W0, W1, W2, W3a, W3b, W3c, W4a, W4b, W4c, W5, W6) and hands BB a ratified foundation.
